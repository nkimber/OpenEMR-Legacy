...
const missingPayerAnchorPatientId = "MOD-PAT-0005";
const diagnosisPointer = "K21.9";
const validCptCode = "99214";
const validModifierText = "25";
const missingPayerIssue = "missing-payer";
...
test.describe("claim missing payer parity @slice549 @workflow-claim-missing-payer @mutation @billing", () => {
  test("creates, flags, renders, and removes a missing-payer claim scrub report", async ({
    page,
    target,
    targetDb,
    workflow
  }, testInfo) => {
    const patient = await targetDb.findPatientByCanonicalId(missingPayerAnchorPatientId);
    expect(patient).not.toBeNull();
    if (!patient) {
      throw new Error(`Anchor patient ${missingPayerAnchorPatientId} was not found.`);
    }
    ...
    const encounterInput = {
      ...
      reason: "Missing payer parity encounter",
      facilityId: 10,
      facilityName: "OpenEMR Modernization Clinic",
      billingFacilityId: 10,
      billingNote: "Created by the Slice 549 claim missing payer parity suite."
    };
    ...
    try {
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-549-claim-missing-payer-precondition",
        description: "Captures the Slice 549 missing-payer anchor, baseline counts, proposed temporary encounter, valid diagnosis line, CPT4 line, and queued claim with no payer selected.",
        expected: {
          patient: { pubpid: missingPayerAnchorPatientId },
          validModifierText,
          missingPayerIssue,
          countChange: {
        ...
        context: {
          canonicalId: missingPayerAnchorPatientId,
          suite: "workflow-claim-missing-payer",
          workflow: "claim-missing-payer-precondition"
        }
      });
      ...
      cptLineId = await workflow.createBillingLine({
        ...
        codeText: "Missing payer parity charge",
        fee: "115.00",
        units: 1,
        justify: diagnosisPointer
      });
      ...
      const expectedScrub = buildClaimScrubReport(claimId, encounterId, "", 0, patient.pubpid, linesWithValidClaimLines);
      expect(expectedScrub.report).toContain("SCRUB-FAIL");
      expect(expectedScrub.report).toContain(missingPayerIssue);
      ...
      if (target.type === "modernized-openemr") {
        ...
        await expect(page.locator("body")).toContainText(`${missingPayerAnchorPatientId} / PID ${patient.pid}`);
        const claimCard = page.locator("article.billing-line-card")
          .filter({ hasText: `Version ${createdClaim!.version} / Primary Payer 0` })
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-549-claim-missing-payer-flagged",
        description: "Captures the temporary Slice 549 claim after missing-payer local claim scrubbing, including deterministic missing-payer report content, process-file metadata, and count stability.",
        expected: {
          claim: {
            patientId: patient.pid,
            encounter: encounterId,
          ...
          scrub: { status: "FAIL", issues: [missingPayerIssue] }
        },
        ...
        context: {
          ...
          encounter: encounterId,
          suite: "workflow-claim-missing-payer",
          workflow: "claim-missing-payer-flagged"
        }
      });
    } finally {
    ...
    await attachDatabaseProbeEvidence(testInfo, {
      target: target.type,
      probe: "slice-549-claim-missing-payer-cleanup",
      description: "Captures the final Slice 549 cleanup state for the temporary missing-payer claim, billing lines, and encounter.",
      expected: {
      ...
      context: {
        ...
        encounter: encounterId,
        suite: "workflow-claim-missing-payer",
        workflow: "claim-missing-payer-cleanup"
      }
    });
