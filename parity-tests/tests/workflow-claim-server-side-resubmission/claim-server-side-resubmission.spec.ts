...
const serverSideResubmissionAnchorPatientId = "MOD-PAT-0005";
const serverSideResubmissionEncounter = 1000052;

test.describe("claim server-side resubmission parity @slice558 @workflow-claim-server-side-resubmission @mutation @billing", () => {
  test("creates, server-resubmits, renders, and removes a temporary denied claim", async ({
    page,
    target,
    targetDb,
    workflow
  }, testInfo) => {
    const patient = await targetDb.findPatientByCanonicalId(serverSideResubmissionAnchorPatientId);
    ...
    const createInput = {
      ...
      encounter: serverSideResubmissionEncounter,
      payerId: 9005,
      ...
      processFile: `CLAIM-${serverSideResubmissionEncounter}-DENIAL-RESUBMIT-SEED-835.txt`,
      target: "X12",
      x12PartnerId: 1,
    ...
    try {
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-558-claim-server-side-resubmission-precondition",
        description: "Captures the Slice 558 server-side claim-resubmission anchor, baseline claim count, existing claims, and proposed temporary denied claim.",
        expected: {
          patient: {
            pubpid: serverSideResubmissionAnchorPatientId
          },
          encounter: serverSideResubmissionEncounter,
          deniedClaim: {
            payerName: "Northstar HMO",
        ...
        context: {
          canonicalId: serverSideResubmissionAnchorPatientId,
          encounter: serverSideResubmissionEncounter,
          suite: "workflow-claim-server-side-resubmission",
          workflow: "claim-server-side-resubmission-precondition"
        }
      });
      ...
      const expectedResubmission = buildClaimResubmissionPayload(
        claimId,
        serverSideResubmissionEncounter,
        "Northstar HMO",
        9005,
        patient!.pubpid,
        created!.statusLabel
      ...
      if (target.type === "modernized-openemr") {
        ...
        await expect(page.locator("body")).toContainText(`${serverSideResubmissionAnchorPatientId} / PID ${patient!.pid}`);
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-558-claim-server-side-resubmission-resubmitted",
        description: "Captures the temporary Slice 558 claim after backend resubmission, including queued status, X12 target metadata, backend-generated deterministic resubmission payload, modernized UI action evidence, and claim-count stability.",
        expected: {
          claim: {
            patientId: patient!.pid,
            encounter: serverSideResubmissionEncounter,
        ...
        context: {
          ...
          encounter: serverSideResubmissionEncounter,
          suite: "workflow-claim-server-side-resubmission",
          workflow: "claim-server-side-resubmission-resubmitted"
        }
      });
    } finally {
      if (claimId !== null) {
    ...
    if (claimId !== null) {
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-558-claim-server-side-resubmission-cleanup",
        description: "Captures the final Slice 558 cleanup state for the temporary resubmitted claim.",
        expected: {
          counts: {
        ...
        context: {
          ...
          encounter: serverSideResubmissionEncounter,
          suite: "workflow-claim-server-side-resubmission",
          workflow: "claim-server-side-resubmission-cleanup"
        }
      });
