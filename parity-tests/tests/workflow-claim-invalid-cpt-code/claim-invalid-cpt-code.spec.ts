...
const invalidCptAnchorPatientId = "MOD-PAT-0005";
const validDiagnosisPointer = "K21.9";
const invalidCptCode = "99A14";
...
test.describe("claim invalid CPT code parity @slice545 @workflow-claim-invalid-cpt-code @mutation @billing", () => {
  test("creates, flags, renders, and removes an invalid-CPT-code claim scrub report", async ({
    page,
    target,
    targetDb,
    workflow
  }, testInfo) => {
    const patient = await targetDb.findPatientByCanonicalId(invalidCptAnchorPatientId);
    expect(patient).not.toBeNull();
    if (!patient) {
      throw new Error(`Anchor patient ${invalidCptAnchorPatientId} was not found.`);
    }
    ...
    const encounterInput = {
      ...
      dateTime: "2026-06-18 16:30:00",
      reason: "Invalid CPT code parity encounter",
      facilityId: 10,
      facilityName: "OpenEMR Modernization Clinic",
      billingFacilityId: 10,
      billingNote: "Created by the Slice 545 claim invalid CPT code parity suite."
    };
    ...
    try {
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-545-claim-invalid-cpt-code-precondition",
        description: "Captures the Slice 545 invalid CPT-code anchor, baseline counts, proposed temporary encounter, valid diagnosis line, malformed CPT4 line, and queued claim.",
        expected: {
          patient: { pubpid: invalidCptAnchorPatientId },
          invalidCptCode,
          countChange: {
        ...
        context: {
          canonicalId: invalidCptAnchorPatientId,
          suite: "workflow-claim-invalid-cpt-code",
          workflow: "claim-invalid-cpt-code-precondition"
        }
      });
      ...
      cptLineId = await workflow.createBillingLine({
        ...
        providerId: patient.providerId,
        encounter: encounterId,
        dateTime: "2026-06-18 16:42:00",
        codeType: "CPT4",
        code: invalidCptCode,
        modifier: "25",
        codeText: "Invalid CPT code parity charge",
        fee: "115.00",
        units: 1,
        justify: validDiagnosisPointer
      });
      ...
      const linesWithInvalidCpt = await targetDb.getBillingLinesForEncounter(patient.pid, encounterId);
      expect(linesWithInvalidCpt).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ codeType: "ICD10", code: validDiagnosisPointer }),
      ...
      const expectedScrub = buildClaimScrubReport(claimId, encounterId, "Northstar HMO", 9005, patient.pubpid, linesWithInvalidCpt);
      expect(expectedScrub.report).toContain("SCRUB-FAIL");
      expect(expectedScrub.report).toContain(`invalid-cpt-code:${invalidCptCode}`);
      ...
      if (target.type === "modernized-openemr") {
        ...
        await expect(page.locator("body")).toContainText(`${invalidCptAnchorPatientId} / PID ${patient.pid}`);
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-545-claim-invalid-cpt-code-flagged",
        description: "Captures the temporary Slice 545 claim after invalid-CPT-code local claim scrubbing, including deterministic invalid-cpt-code report content, process-file metadata, and count stability.",
        expected: {
          claim: {
            patientId: patient.pid,
            encounter: encounterId,
          ...
          scrub: { status: "FAIL", issues: [`invalid-cpt-code:${invalidCptCode}`] }
        },
        actual: { patient, beforeCounts, afterScrubCounts, encounterId, claimId, diagnosisLineId, cptLineId, createdClaim, scrubbed, linesWithInvalidCpt },
        context: {
          canonicalId: invalidCptAnchorPatientId,
          encounter: encounterId,
          suite: "workflow-claim-invalid-cpt-code",
          workflow: "claim-invalid-cpt-code-flagged"
        }
      });
    } finally {
    ...
    await attachDatabaseProbeEvidence(testInfo, {
      target: target.type,
      probe: "slice-545-claim-invalid-cpt-code-cleanup",
      description: "Captures the final Slice 545 cleanup state for the temporary invalid-CPT-code claim, billing lines, and encounter.",
      expected: {
      ...
      context: {
        ...
        encounter: encounterId,
        suite: "workflow-claim-invalid-cpt-code",
        workflow: "claim-invalid-cpt-code-cleanup"
      }
    });
