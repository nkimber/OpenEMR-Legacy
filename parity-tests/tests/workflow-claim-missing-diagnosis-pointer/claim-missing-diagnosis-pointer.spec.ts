...
const missingDiagnosisPointerAnchorPatientId = "MOD-PAT-0005";
const diagnosisPointer = "K21.9";
const validCptCode = "99214";
const validModifierText = "25";
const missingDiagnosisPointerIssue = "missing-diagnosis-pointer";
...
test.describe("claim missing diagnosis pointer parity @slice553 @workflow-claim-missing-diagnosis-pointer @mutation @billing", () => {
  test("creates, flags, renders, and removes a missing-diagnosis-pointer claim scrub report", async ({
    page,
    target,
    targetDb,
    workflow
  }, testInfo) => {
    const patient = await targetDb.findPatientByCanonicalId(missingDiagnosisPointerAnchorPatientId);
    expect(patient).not.toBeNull();
    if (!patient) {
      throw new Error(`Anchor patient ${missingDiagnosisPointerAnchorPatientId} was not found.`);
    }
    ...
    const encounterInput = {
      ...
      reason: "Missing diagnosis pointer parity encounter",
      facilityId: 10,
      facilityName: "OpenEMR Modernization Clinic",
      billingFacilityId: 10,
      billingNote: "Created by the Slice 553 claim missing diagnosis pointer parity suite."
    };
    ...
    try {
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-553-claim-missing-diagnosis-pointer-precondition",
        description: "Captures the Slice 553 missing-diagnosis-pointer anchor, baseline counts, proposed temporary encounter, valid diagnosis line, CPT4 line, and queued claim with blank CPT diagnosis pointer.",
        expected: {
          patient: { pubpid: missingDiagnosisPointerAnchorPatientId },
          validModifierText,
          missingDiagnosisPointerIssue,
          countChange: {
        ...
        context: {
          canonicalId: missingDiagnosisPointerAnchorPatientId,
          suite: "workflow-claim-missing-diagnosis-pointer",
          workflow: "claim-missing-diagnosis-pointer-precondition"
        }
      });
      ...
      cptLineId = await workflow.createBillingLine({
        ...
        codeText: "Missing diagnosis pointer parity charge",
        fee: "115.00",
        units: 1,
        justify: ""
      });
      ...
      expect(expectedScrub.report).toContain(missingDiagnosisPointerIssue);
      expect(expectedScrub.report).toContain("diagnosisPointers=none");
      ...
      if (target.type === "modernized-openemr") {
        ...
        await expect(page.locator("body")).toContainText(`${missingDiagnosisPointerAnchorPatientId} / PID ${patient.pid}`);
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-553-claim-missing-diagnosis-pointer-flagged",
        description: "Captures the temporary Slice 553 claim after missing-diagnosis-pointer local claim scrubbing, including deterministic missing-diagnosis-pointer report content, process-file metadata, and count stability.",
        expected: {
          claim: {
            patientId: patient.pid,
            encounter: encounterId,
          ...
          scrub: { status: "FAIL", issues: [missingDiagnosisPointerIssue] }
        },
        ...
        context: {
          ...
          encounter: encounterId,
          suite: "workflow-claim-missing-diagnosis-pointer",
          workflow: "claim-missing-diagnosis-pointer-flagged"
        }
      });
    } finally {
    ...
    await attachDatabaseProbeEvidence(testInfo, {
      target: target.type,
      probe: "slice-553-claim-missing-diagnosis-pointer-cleanup",
      description: "Captures the final Slice 553 cleanup state for the temporary missing-diagnosis-pointer claim, billing lines, and encounter.",
      expected: {
      ...
      context: {
        ...
        encounter: encounterId,
        suite: "workflow-claim-missing-diagnosis-pointer",
        workflow: "claim-missing-diagnosis-pointer-cleanup"
      }
    });
