...
const inactiveDiagnosisLineAnchorPatientId = "MOD-PAT-0005";
const diagnosisPointer = "K21.9";
const validCptCode = "99214";
const validModifierText = "25";
const inactiveDiagnosisLineIssue = "missing-diagnosis-code";
...
test.describe("claim inactive diagnosis line parity @slice555 @workflow-claim-inactive-diagnosis-line @mutation @billing", () => {
  test("creates, deactivates, flags, renders, and removes an inactive-diagnosis claim scrub report", async ({
    page,
    target,
    targetDb,
    workflow
  }, testInfo) => {
    const patient = await targetDb.findPatientByCanonicalId(inactiveDiagnosisLineAnchorPatientId);
    expect(patient).not.toBeNull();
    if (!patient) {
      throw new Error(`Anchor patient ${inactiveDiagnosisLineAnchorPatientId} was not found.`);
    }
    ...
    const encounterInput = {
      ...
      reason: "Inactive diagnosis line parity encounter",
      facilityId: 10,
      facilityName: "OpenEMR Modernization Clinic",
      billingFacilityId: 10,
      billingNote: "Created by the Slice 555 claim inactive diagnosis line parity suite."
    };
    ...
    try {
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-555-claim-inactive-diagnosis-line-precondition",
        description: "Captures the Slice 555 inactive-diagnosis anchor, baseline counts, proposed temporary encounter, valid diagnosis line that will be marked inactive, active CPT4 line, and queued claim.",
        expected: {
          patient: { pubpid: inactiveDiagnosisLineAnchorPatientId },
          validModifierText,
          inactiveDiagnosisLineIssue,
          countChange: {
        ...
        context: {
          canonicalId: inactiveDiagnosisLineAnchorPatientId,
          suite: "workflow-claim-inactive-diagnosis-line",
          workflow: "claim-inactive-diagnosis-line-precondition"
        }
      });
      ...
      await workflow.updateBillingLineStatus(diagnosisLineId, 1, 0);
      const inactiveDiagnosisLine = await workflow.getBillingLine(diagnosisLineId);
      expect(inactiveDiagnosisLine).toMatchObject({
        codeType: "ICD10",
        code: diagnosisPointer,
        fee: "0.00",
        justify: diagnosisPointer,
      ...
      cptLineId = await workflow.createBillingLine({
        ...
        codeText: "Inactive diagnosis line parity charge",
        fee: "115.00",
        units: 1,
        justify: diagnosisPointer
      });
      ...
      const activeLinesWithoutDiagnosisLines = await targetDb.getBillingLinesForEncounter(patient.pid, encounterId);
      expect(activeLinesWithoutDiagnosisLines).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ codeType: "CPT4", code: validCptCode, modifier: validModifierText, justify: diagnosisPointer })
        ])
      );
      expect(activeLinesWithoutDiagnosisLines.some((line) => (line.codeType || "").toUpperCase() === "ICD10")).toBe(false);

      const expectedScrub = buildClaimScrubReport(claimId, encounterId, "Northstar HMO", 9005, patient.pubpid, activeLinesWithoutDiagnosisLines);
      expect(expectedScrub.report).toContain("SCRUB-FAIL");
      expect(expectedScrub.report).toContain(inactiveDiagnosisLineIssue);
      expect(expectedScrub.report).not.toContain("missing-payer");
      expect(expectedScrub.report).not.toContain("missing-cpt-line");
      ...
      if (target.type === "modernized-openemr") {
        ...
        await expect(page.locator("body")).toContainText(`${inactiveDiagnosisLineAnchorPatientId} / PID ${patient.pid}`);
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-555-claim-inactive-diagnosis-line-flagged",
        description: "Captures the temporary Slice 555 claim after inactive-diagnosis-aware local claim scrubbing, including deterministic missing-diagnosis-code report content, inactive diagnosis status, process-file metadata, and count stability.",
        expected: {
          claim: {
            patientId: patient.pid,
            encounter: encounterId,
          ...
          inactiveDiagnosisLine: { billed: 1, activity: 0 },
          scrub: { status: "FAIL", issues: [inactiveDiagnosisLineIssue] }
        },
        actual: { patient, beforeCounts, afterScrubCounts, encounterId, claimId, diagnosisLineId, cptLineId, inactiveDiagnosisLine, createdClaim, scrubbed, activeLinesWithoutDiagnosisLines },
        context: {
          canonicalId: inactiveDiagnosisLineAnchorPatientId,
          encounter: encounterId,
          suite: "workflow-claim-inactive-diagnosis-line",
          workflow: "claim-inactive-diagnosis-line-flagged"
        }
      });
    } finally {
    ...
    await attachDatabaseProbeEvidence(testInfo, {
      target: target.type,
      probe: "slice-555-claim-inactive-diagnosis-line-cleanup",
      description: "Captures the final Slice 555 cleanup state for the temporary inactive-diagnosis claim, billing lines, and encounter.",
      expected: {
      ...
      context: {
        ...
        encounter: encounterId,
        suite: "workflow-claim-inactive-diagnosis-line",
        workflow: "claim-inactive-diagnosis-line-cleanup"
      }
    });
