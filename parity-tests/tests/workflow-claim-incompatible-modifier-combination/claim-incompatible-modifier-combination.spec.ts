...
const incompatibleModifierAnchorPatientId = "MOD-PAT-0005";
const diagnosisPointer = "K21.9";
const validCptCode = "99214";
const incompatibleModifierText = "25,59";
const incompatibleModifierCombination = "25+59";
...
test.describe("claim incompatible modifier combination parity @slice548 @workflow-claim-incompatible-modifier-combination @mutation @billing", () => {
  test("creates, flags, renders, and removes an incompatible-modifier-combination claim scrub report", async ({
    page,
    target,
    targetDb,
    workflow
  }, testInfo) => {
    const patient = await targetDb.findPatientByCanonicalId(incompatibleModifierAnchorPatientId);
    expect(patient).not.toBeNull();
    if (!patient) {
      throw new Error(`Anchor patient ${incompatibleModifierAnchorPatientId} was not found.`);
    }
    ...
    const encounterInput = {
      ...
      reason: "Incompatible modifier combination parity encounter",
      facilityId: 10,
      facilityName: "OpenEMR Modernization Clinic",
      billingFacilityId: 10,
      billingNote: "Created by the Slice 548 claim incompatible modifier combination parity suite."
    };
    ...
    try {
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-548-claim-incompatible-modifier-combination-precondition",
        description: "Captures the Slice 548 incompatible modifier-combination anchor, baseline counts, proposed temporary encounter, valid diagnosis line, CPT4 line with supported but incompatible modifiers, and queued claim.",
        expected: {
          patient: { pubpid: incompatibleModifierAnchorPatientId },
          incompatibleModifierText,
          incompatibleModifierCombination,
          countChange: {
        ...
        context: {
          canonicalId: incompatibleModifierAnchorPatientId,
          suite: "workflow-claim-incompatible-modifier-combination",
          workflow: "claim-incompatible-modifier-combination-precondition"
        }
      });
      ...
      cptLineId = await workflow.createBillingLine({
        ...
        codeType: "CPT4",
        code: validCptCode,
        modifier: incompatibleModifierText,
        codeText: "Incompatible modifier parity charge",
        fee: "115.00",
        units: 1,
        justify: diagnosisPointer
      });
      ...
      const linesWithIncompatibleModifiers = await targetDb.getBillingLinesForEncounter(patient.pid, encounterId);
      expect(linesWithIncompatibleModifiers).toEqual(
      ...
      const expectedScrub = buildClaimScrubReport(claimId, encounterId, "Northstar HMO", 9005, patient.pubpid, linesWithIncompatibleModifiers);
      expect(expectedScrub.report).toContain("SCRUB-FAIL");
      expect(expectedScrub.report).toContain(`incompatible-modifier-combination:${incompatibleModifierCombination}`);
      ...
      if (target.type === "modernized-openemr") {
        ...
        await expect(page.locator("body")).toContainText(`${incompatibleModifierAnchorPatientId} / PID ${patient.pid}`);
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-548-claim-incompatible-modifier-combination-flagged",
        description: "Captures the temporary Slice 548 claim after incompatible-modifier-combination local claim scrubbing, including deterministic incompatible-modifier-combination report content, process-file metadata, and count stability.",
        expected: {
          claim: {
            patientId: patient.pid,
            encounter: encounterId,
          ...
          scrub: { status: "FAIL", issues: [`incompatible-modifier-combination:${incompatibleModifierCombination}`] }
        },
        actual: { patient, beforeCounts, afterScrubCounts, encounterId, claimId, diagnosisLineId, cptLineId, createdClaim, scrubbed, linesWithIncompatibleModifiers },
        context: {
          canonicalId: incompatibleModifierAnchorPatientId,
          encounter: encounterId,
          suite: "workflow-claim-incompatible-modifier-combination",
          workflow: "claim-incompatible-modifier-combination-flagged"
        }
      });
    } finally {
    ...
    await attachDatabaseProbeEvidence(testInfo, {
      target: target.type,
      probe: "slice-548-claim-incompatible-modifier-combination-cleanup",
      description: "Captures the final Slice 548 cleanup state for the temporary incompatible-modifier-combination claim, billing lines, and encounter.",
      expected: {
      ...
      context: {
        ...
        encounter: encounterId,
        suite: "workflow-claim-incompatible-modifier-combination",
        workflow: "claim-incompatible-modifier-combination-cleanup"
      }
    });
