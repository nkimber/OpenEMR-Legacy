...
const multiDiagnosisAnchorPatientId = "MOD-PAT-0005";
...
test.describe("claim multi-diagnosis pointer parity @slice541 @workflow-claim-multi-diagnosis-pointer @mutation @billing", () => {
  test("creates, validates, renders, and removes a multi-diagnosis claim scrub report", async ({
    page,
    target,
    targetDb,
    workflow
  }, testInfo) => {
    const patient = await targetDb.findPatientByCanonicalId(multiDiagnosisAnchorPatientId);
    expect(patient).not.toBeNull();
    if (!patient) {
      throw new Error(`Anchor patient ${multiDiagnosisAnchorPatientId} was not found.`);
    }
    ...
    const encounterInput = {
      ...
      reason: "Multi diagnosis pointer parity encounter",
      facilityId: 10,
      facilityName: "OpenEMR Modernization Clinic",
      billingFacilityId: 10,
      billingNote: "Created by the Slice 541 claim multi diagnosis pointer parity suite."
    };
    ...
    try {
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-541-claim-multi-diagnosis-pointer-precondition",
        description: "Captures the Slice 541 multi-diagnosis pointer anchor, baseline counts, proposed temporary encounter, two ICD10 diagnosis lines, CPT line with comma-separated pointers, and queued claim.",
        expected: {
          patient: {
            pubpid: multiDiagnosisAnchorPatientId
          },
        ...
        context: {
          canonicalId: multiDiagnosisAnchorPatientId,
          suite: "workflow-claim-multi-diagnosis-pointer",
          workflow: "claim-multi-diagnosis-pointer-precondition"
        }
      });
      ...
      cptLineId = await workflow.createBillingLine({
        ...
        code: "99214",
        modifier: "25",
        codeText: "Multi diagnosis pointer parity charge",
        fee: "115.00",
        units: 1,
      ...
      const linesWithMultipleDiagnoses = await targetDb.getBillingLinesForEncounter(patient.pid, encounterId);
      expect(linesWithMultipleDiagnoses).toEqual(
      ...
      if (target.type === "modernized-openemr") {
        ...
        await expect(page.locator("body")).toContainText(`${multiDiagnosisAnchorPatientId} / PID ${patient.pid}`);
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-541-claim-multi-diagnosis-pointer-passed",
        description: "Captures the temporary Slice 541 claim after multi-diagnosis-pointer-aware local claim scrubbing, including deterministic SCRUB-PASS report content, process-file metadata, and count stability.",
        expected: {
          claim: {
            patientId: patient.pid,
            encounter: encounterId,
        ...
        context: {
          ...
          encounter: encounterId,
          suite: "workflow-claim-multi-diagnosis-pointer",
          workflow: "claim-multi-diagnosis-pointer-passed"
        }
      });
    } finally {
      if (claimId !== null) {
    ...
    await attachDatabaseProbeEvidence(testInfo, {
      target: target.type,
      probe: "slice-541-claim-multi-diagnosis-pointer-cleanup",
      description: "Captures the final Slice 541 cleanup state for the temporary multi-diagnosis-pointer claim, billing lines, and encounter.",
      expected: {
        counts: {
      ...
      context: {
        ...
        encounter: encounterId,
        suite: "workflow-claim-multi-diagnosis-pointer",
        workflow: "claim-multi-diagnosis-pointer-cleanup"
      }
    });
