...
test.describe("claim invalid fee parity @slice550 @workflow-claim-invalid-fee @mutation @billing", () => {
  test("creates, flags, renders, and removes an invalid-fee claim scrub report", async ({
    page,
    target,
    targetDb,
    workflow
  }, testInfo) => {
    const patient = await targetDb.findPatientByCanonicalId(invalidFeeAnchorPatientId);
    expect(patient).not.toBeNull();
    if (!patient) {
      throw new Error(`Anchor patient ${invalidFeeAnchorPatientId} was not found.`);
    }
    ...
    const encounterInput = {
      ...
      reason: "Invalid fee parity encounter",
      facilityId: 10,
      facilityName: "OpenEMR Modernization Clinic",
      billingFacilityId: 10,
      billingNote: "Created by the Slice 550 claim invalid fee parity suite."
    };
    ...
    try {
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-550-claim-invalid-fee-precondition",
        description: "Captures the Slice 550 invalid-fee anchor, baseline counts, proposed temporary encounter, valid diagnosis line, CPT4 line, and queued claim with zero CPT fee.",
        expected: {
          patient: { pubpid: invalidFeeAnchorPatientId },
          validModifierText,
          invalidFeeIssue,
          countChange: {
        ...
        context: {
          canonicalId: invalidFeeAnchorPatientId,
          suite: "workflow-claim-invalid-fee",
          workflow: "claim-invalid-fee-precondition"
        }
      });
      ...
      cptLineId = await workflow.createBillingLine({
        ...
        codeText: "Invalid fee parity charge",
        fee: "0.00",
        units: 1,
        justify: diagnosisPointer
      });
      ...
      expect(expectedScrub.report).toContain(invalidFeeIssue);
      ...
      if (target.type === "modernized-openemr") {
        ...
        await expect(page.locator("body")).toContainText(`${invalidFeeAnchorPatientId} / PID ${patient.pid}`);
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-550-claim-invalid-fee-flagged",
        description: "Captures the temporary Slice 550 claim after invalid-fee local claim scrubbing, including deterministic invalid-fee report content, process-file metadata, and count stability.",
        expected: {
          claim: {
            patientId: patient.pid,
            encounter: encounterId,
          ...
          scrub: { status: "FAIL", issues: [invalidFeeIssue] }
        },
        ...
        context: {
          ...
          encounter: encounterId,
          suite: "workflow-claim-invalid-fee",
          workflow: "claim-invalid-fee-flagged"
        }
      });
    } finally {
    ...
    await attachDatabaseProbeEvidence(testInfo, {
      target: target.type,
      probe: "slice-550-claim-invalid-fee-cleanup",
      description: "Captures the final Slice 550 cleanup state for the temporary invalid-fee claim, billing lines, and encounter.",
      expected: {
      ...
      context: {
        ...
        encounter: encounterId,
        suite: "workflow-claim-invalid-fee",
        workflow: "claim-invalid-fee-cleanup"
      }
    });
