...
const claimModifierCompatibilityAnchorPatientId = "MOD-PAT-0005";
const claimModifierCompatibilityEncounter = 1000052;
const validDiagnosisPointer = "K21.9";
const duplicateModifierValue = "25";
const duplicateModifierText = "25,25";
...
test.describe("claim modifier compatibility parity @slice538 @workflow-claim-modifier-compatibility @mutation @billing", () => {
  test("creates, flags, renders, and removes a duplicate-modifier claim scrub report", async ({
    page,
    target,
    targetDb,
    workflow
  }, testInfo) => {
    const patient = await targetDb.findPatientByCanonicalId(claimModifierCompatibilityAnchorPatientId);
    expect(patient).not.toBeNull();
    if (!patient) {
      throw new Error(`Anchor patient ${claimModifierCompatibilityAnchorPatientId} was not found.`);
    }

    const beforeCounts = await targetDb.getPatientWorkflowCounts(patient.pid);
    const beforeClaims = await targetDb.getClaimsForPatient(patient.pid);
    const beforeLines = await targetDb.getBillingLinesForEncounter(patient.pid, claimModifierCompatibilityEncounter);
    const createClaimInput = {
      patientId: patient.pid,
      encounter: claimModifierCompatibilityEncounter,
      payerId: 9005,
      payerName: "Northstar HMO",
      payerType: 1,
      status: 1,
      billProcess: 1,
      billTime: "2026-06-18 12:45:00",
      processFile: "",
      target: "HCFA",
      x12PartnerId: 0,
      submittedClaim: ""
    ...
    try {
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-538-claim-modifier-compatibility-precondition",
        description: "Captures the Slice 538 claim modifier-compatibility anchor, baseline counts, existing encounter billing lines, and proposed temporary diagnosis plus duplicate-modifier CPT line and queued claim.",
        expected: {
          patient: {
            pubpid: claimModifierCompatibilityAnchorPatientId
          },
          encounter: claimModifierCompatibilityEncounter,
          duplicateModifierText,
          countChange: {
        ...
        context: {
          canonicalId: claimModifierCompatibilityAnchorPatientId,
          encounter: claimModifierCompatibilityEncounter,
          suite: "workflow-claim-modifier-compatibility",
          workflow: "claim-modifier-compatibility-precondition"
        }
      });
      ...
      cptLineId = await workflow.createBillingLine({
        ...
        providerId: patient.providerId,
        encounter: claimModifierCompatibilityEncounter,
        dateTime: "2026-06-18 13:18:00",
        codeType: "CPT4",
        code: "99214",
        modifier: duplicateModifierText,
        codeText: "Duplicate modifier parity charge",
        fee: "115.00",
        units: 1,
        justify: validDiagnosisPointer
      });
      ...
      const linesWithDuplicateModifier = await targetDb.getBillingLinesForEncounter(patient.pid, claimModifierCompatibilityEncounter);
      expect(linesWithDuplicateModifier).toEqual(
      ...
      const expectedScrub = buildClaimScrubReport(
        claimId,
        claimModifierCompatibilityEncounter,
        "Northstar HMO",
        9005,
        patient.pubpid,
        linesWithDuplicateModifier
      );
      expect(expectedScrub.report).toContain("SCRUB-FAIL");
      expect(expectedScrub.report).toContain(`duplicate-modifier:${duplicateModifierValue}`);
      expect(expectedScrub.report).not.toContain(`invalid-modifier:${duplicateModifierText}`);

      if (target.type === "modernized-openemr") {
        await openAuthenticatedModernizedFees(page, target, patient.pubpid);
        await expect(page.locator("body")).toContainText(`${claimModifierCompatibilityAnchorPatientId} / PID ${patient.pid}`);
      ...
      expect(scrubbed!.submittedClaim).toContain(`duplicate-modifier:${duplicateModifierValue}`);
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-538-claim-modifier-compatibility-flagged",
        description: "Captures the temporary Slice 538 claim after modifier-compatibility local claim scrubbing, including deterministic duplicate-modifier scrub report content, process-file metadata, and count stability.",
        expected: {
          claim: {
            patientId: patient.pid,
            encounter: claimModifierCompatibilityEncounter,
          ...
          scrub: {
            status: "FAIL",
            issues: [`duplicate-modifier:${duplicateModifierValue}`]
          }
        },
        actual: {
          patient,
        ...
        context: {
          ...
          encounter: claimModifierCompatibilityEncounter,
          suite: "workflow-claim-modifier-compatibility",
          workflow: "claim-modifier-compatibility-flagged"
        }
      });
    } finally {
      if (claimId !== null) {
    ...
    await attachDatabaseProbeEvidence(testInfo, {
      target: target.type,
      probe: "slice-538-claim-modifier-compatibility-cleanup",
      description: "Captures the final Slice 538 cleanup state for the temporary duplicate-modifier claim and billing lines.",
      expected: {
        counts: {
      ...
      context: {
        ...
        encounter: claimModifierCompatibilityEncounter,
        suite: "workflow-claim-modifier-compatibility",
        workflow: "claim-modifier-compatibility-cleanup"
      }
    });
