...
const missingCptLineAnchorPatientId = "MOD-PAT-0005";
const diagnosisPointer = "K21.9";
const missingCptLineIssue = "missing-cpt-line";
...
test.describe("claim missing CPT line parity @slice552 @workflow-claim-missing-cpt-line @mutation @billing", () => {
  test("creates, flags, renders, and removes a missing-cpt-line claim scrub report", async ({
    page,
    target,
    targetDb,
    workflow
  }, testInfo) => {
    const patient = await targetDb.findPatientByCanonicalId(missingCptLineAnchorPatientId);
    expect(patient).not.toBeNull();
    if (!patient) {
      throw new Error(`Anchor patient ${missingCptLineAnchorPatientId} was not found.`);
    }
    ...
    const encounterInput = {
      ...
      reason: "Missing CPT line parity encounter",
      facilityId: 10,
      facilityName: "OpenEMR Modernization Clinic",
      billingFacilityId: 10,
      billingNote: "Created by the Slice 552 claim missing CPT line parity suite."
    };
    ...
    try {
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-552-claim-missing-cpt-line-precondition",
        description: "Captures the Slice 552 missing-cpt-line anchor, baseline counts, proposed temporary encounter, valid diagnosis line, and queued claim with no CPT service line.",
        expected: {
          patient: { pubpid: missingCptLineAnchorPatientId },
          missingCptLineIssue,
          countChange: {
        ...
        context: {
          canonicalId: missingCptLineAnchorPatientId,
          suite: "workflow-claim-missing-cpt-line",
          workflow: "claim-missing-cpt-line-precondition"
        }
      });
      ...
      const linesWithoutCptLines = await targetDb.getBillingLinesForEncounter(patient.pid, encounterId);
      expect(linesWithoutCptLines).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ codeType: "ICD10", code: diagnosisPointer })
        ])
      );
      expect(linesWithoutCptLines.some((line) => (line.codeType || "").toUpperCase() === "CPT4")).toBe(false);

      const expectedScrub = buildClaimScrubReport(claimId, encounterId, "Northstar HMO", 9005, patient.pubpid, linesWithoutCptLines);
      expect(expectedScrub.report).toContain("SCRUB-FAIL");
      expect(expectedScrub.report).toContain(missingCptLineIssue);
      ...
      if (target.type === "modernized-openemr") {
        ...
        await expect(page.locator("body")).toContainText(`${missingCptLineAnchorPatientId} / PID ${patient.pid}`);
      ...
      await attachDatabaseProbeEvidence(testInfo, {
        target: target.type,
        probe: "slice-552-claim-missing-cpt-line-flagged",
        description: "Captures the temporary Slice 552 claim after missing-cpt-line local claim scrubbing, including deterministic missing-cpt-line report content, process-file metadata, and count stability.",
        expected: {
          claim: {
            patientId: patient.pid,
            encounter: encounterId,
          ...
          scrub: { status: "FAIL", issues: [missingCptLineIssue] }
        },
        actual: { patient, beforeCounts, afterScrubCounts, encounterId, claimId, diagnosisLineId, createdClaim, scrubbed, linesWithoutCptLines },
        context: {
          canonicalId: missingCptLineAnchorPatientId,
          encounter: encounterId,
          suite: "workflow-claim-missing-cpt-line",
          workflow: "claim-missing-cpt-line-flagged"
        }
      });
    } finally {
    ...
    await attachDatabaseProbeEvidence(testInfo, {
      target: target.type,
      probe: "slice-552-claim-missing-cpt-line-cleanup",
      description: "Captures the final Slice 552 cleanup state for the temporary missing-cpt-line claim, diagnosis line, and encounter.",
      expected: {
      ...
      context: {
        ...
        encounter: encounterId,
        suite: "workflow-claim-missing-cpt-line",
        workflow: "claim-missing-cpt-line-cleanup"
      }
    });
