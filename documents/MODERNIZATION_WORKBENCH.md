...
The shared encounter fee-sheet entry plan runs against both targets and verifies temporary `CPT4 99499` and `ICD10 R73.03` fee-sheet rows on `MOD-PAT-0001` encounter `1000013`, legacy Fee Sheet rendering, modernized Encounters workspace fee-sheet entry behavior, Fee Sheet Linkage rendering, Diagnosis Coding rendering, billed/inactive hiding, and hard-delete cleanup. The prior encounter diagnosis coding mutation plan remains available and verifies a temporary `ICD10 R73.03` fee-sheet diagnosis row on the same anchor.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-74 encounter fee-sheet entry readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
