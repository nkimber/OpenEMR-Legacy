...
The reusable parity test harness lives in `parity-tests/` and is launched by `scripts/Run-OpenEmrParityTests.ps1`. It currently provides database, HTTP, Playwright UI, workflow mutation, named run plans, a side-by-side slice-1 readiness plan through a side-by-side slice-188 billing mutation authorization-policy readiness plan, and full-suite legacy runs.
...
Slice 188 adds billing mutation authorization-policy evidence: the shared plan proves a temporary Clinicians `acct:bill view` grant can read billing facts while fee-sheet line, claim, payment, and collections follow-up mutations receive 403 requiring Billing write before the grant is revoked. Slice 187 remains available for message mutation authorization-policy evidence: the shared ACL matrix proves Administrators have Patient Notes write while Clinicians have Patient Notes add-only but not write, and the modernized side proves clinician message read/create success plus 403 status/content/assignment/reply/archive/delete denial before admin cleanup. Slice 186 remains available for document mutation authorization-policy evidence: the shared ACL matrix proves Administrators have Documents write and Documents Delete write while Clinicians have Documents add-only but not write/delete, and the modernized side proves clinician document read/create success plus 403 metadata/content/sign/archive/delete denial before admin cleanup. Slice 185 remains available for encounter mutation authorization-policy evidence: the shared ACL matrix proves Administrators have Authorize Any Encounter write while Clinicians do not permanently carry Authorize Any Encounter, and the modernized side temporarily grants Clinicians view-only encounter access to prove read success plus 403 create/update/sign/delete denial before revoking the grant. Slice 184 remains available for patient mutation authorization-policy evidence: the shared ACL matrix proves Administrators and Front Office have Demographics write while Clinicians have Demographics add-only but not write, and the modernized side proves a clinician can read and register a temporary patient but receives 403 for existing chart contact, demographics, and delete operations while admin can clean up the temporary patient. Slice 183 remains available for procedure mutation authorization-policy evidence: the shared ACL matrix proves Administrators have Lab Results write and Sign Lab Results write while Clinicians have Lab Results add-only but not write/sign, and the modernized side proves a clinician can read and create procedure orders/reports but receives 403 for complete, sign, and delete while admin can complete, sign, and delete the temporary order tree. Slice 182 remains available for procedure authorization-policy evidence: the legacy side proves the ACL matrix grants Lab Results access to Administrators and not Front Office, while the modernized side proves an authenticated front-desk session receives 403 for procedure result retrieval, order-catalog retrieval, and procedure-order create while an admin session still retrieves procedure result/catalog facts through API and Procedures UI. Slice 181 remains available for billing authorization-policy evidence: the legacy side proves the ACL matrix grants Billing access to Administrators/Accounting and not Front Office, while the modernized side proves an authenticated front-desk session receives 403 for billing retrieval, statement batch, and billing-line create while an admin session still retrieves fee-sheet and revenue-cycle facts through API and Fees UI. Earlier protection, mutation, report, queue, provider, catalog, administration, and authorization-policy evidence remains available through the existing Workbench cards and comparison drill-ins.
