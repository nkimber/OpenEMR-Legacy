...
Slice 135 extends the Workbench-managed procedure family with the shared procedure report review queue plan. That plan verifies received/unreviewed queue membership before sign-off, reviewed queue membership after sign-off, UI/API/database facts, and cleanup against both legacy and modernized targets. Slice 134 report sign-off remains available for signed reviewer/timestamp metadata, reviewed status, and linked result preservation. Slice 133 report correction remains available for corrected collected date, report date, specimen number, report status, review status, notes, and linked result preservation. Slice 132 order correction remains available for temporary lab order metadata facts, and Slice 131 specimen detail remains available for temporary order-level specimen facts.
