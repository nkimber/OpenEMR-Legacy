...
- The API can read recent side-by-side comparison artifacts from `parity-tests/artifacts/comparisons/`, and the Test Runs page renders matched/different status, run IDs, suites, metrics, differences, artifact paths, and expandable drill-ins with run artifact paths, comparison JSON paths, artifact directories, selected suites, and full difference detail.
