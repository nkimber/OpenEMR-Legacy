...
Slice 132 extends the Workbench-managed procedure family with the shared procedure order correction plan. That plan verifies corrected order date, procedure code/name/type, diagnosis, priority, status, instructions, UI/API/database facts, post-correction result visibility, and cleanup against both legacy and modernized targets. Slice 131 specimen detail remains available for temporary order-level specimen facts.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-132 procedure order correction readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
