...
The shared encounter diagnosis coding mutation plan runs against both targets and verifies a temporary `ICD10 R73.03` fee-sheet diagnosis row on `MOD-PAT-0001` encounter `1000013`, legacy Fee Sheet rendering, modernized Encounter detail API/UI Diagnosis Coding rendering, billed/inactive hiding, and hard-delete cleanup.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-73 encounter diagnosis coding mutation readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
