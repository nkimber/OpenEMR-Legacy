...
Slice 143 adds the functionality progress ledger to the Progress page. Slice 146 adds explicit scope-adjusted completion estimates to that ledger. Slice 152 adds weighted overall progress, Git-backed progress history, and rough remaining-time forecasting. Slice 155 adds direct comparison-side links to run JSON, Playwright JSON, JUnit XML, and HTML reports. Slice 210 adds managed patient portal secure-message inbox plan actions for both legacy and modernized targets; Slice 209 patient portal home actions, Slice 208 patient portal session actions, Slice 207 patient portal authentication actions, Slice 206 patient portal access actions, Slice 205 patient portal reset actions, Slice 204 patient portal account actions, Slice 203 insurance subscriber actions, Slice 202 patient history/lifestyle actions, Slice 201 patient care-team contact actions, Slice 200 patient care-team members actions, Slice 199 patient care-team actions, Slice 198 patient provider assignment actions, Slice 197 patient employer core actions, Slice 196 patient social-details actions, Slice 195 patient guardian-details actions, Slice 194 patient guardian-contact actions and Slice 193 patient deceased-status actions, Slice 192 patient registration validation actions, Slice 191 patient duplicate detection actions, Slice 190 encounter amendment history actions, Slice 189 appointment mutation authorization-policy actions, Slice 188 billing mutation authorization-policy actions, Slice 187 message mutation authorization-policy actions, Slice 186 document mutation authorization-policy actions, Slice 185 encounter mutation authorization-policy actions, Slice 184 patient mutation authorization-policy actions, and Slice 183 procedure mutation authorization-policy actions remain available; earlier protection and authorization-policy actions remain available.
...
The `/api/progress` response also reconstructs a committed progress history from Git by reading `modernization-workbench/config/functionality-progress.json` as it existed at each first-parent commit that changed the ledger after it was introduced. It prepends a Git-derived timeline anchor for the first first-parent commit that touched `modernized-openemr` so the chart starts when modernized application implementation began, even though progress estimates were not captured yet. In the current history that anchor is commit `6586a4a0`, `Bootstrap modernized OpenEMR slice one`. Each measured history point exposes the Git commit completion timestamp as `snapshotCompletedAt`, and the Progress page plots the weighted completion line chart on that timestamp-based x-axis so longer gaps between completed snapshots remain visible. The API enriches history points with matching `PROJECT_CHANGELOG.md` entry titles, summaries, and key outcomes when their explicit `Commit:` or `Changeset:` metadata matches the progress commit, and falls back to Git commit-message body text if present. The chart renders the pre-ledger anchor span as a dashed segment, supports brush-to-zoom range selection, resettable zoom, rich hover details, and click-to-inspect details. Anchor details explain that the percentage was not captured yet and measured snapshot details show commit identity, weighted percent, delta, elapsed time since the prior timeline point, remaining weighted work, the unweighted comparison signal, and the changelog narrative for what was accomplished. Commits before scope weights existed fall back to equal weighting for their historical point.
...
- The API can read recent side-by-side comparison artifacts from `parity-tests/artifacts/comparisons/`, and the Test Runs page renders matched/different status, run IDs, suites, metrics, differences, artifact paths, expandable drill-ins, safe links to run/comparison artifact files, and direct links to comparison-side run JSON, Playwright JSON, JUnit XML, and HTML reports.
...
- The API can run the shared slice-18 admin facility mutation readiness parity plan, the shared slice-19 admin user mutation readiness parity plan, the shared slice-20 access-control readiness parity plan, the shared slice-21 access-permission mutation readiness parity plan, the shared slice-22 user group membership mutation readiness parity plan, the shared slice-159 through slice-208 authentication, protection, authorization-policy, mutation authorization-policy, encounter amendment history, patient duplicate detection, patient registration validation, patient deceased status, patient guardian contact, patient guardian details, patient social details, patient employer, patient provider assignment, patient care-team, patient care-team members, patient care-team contact, patient history, insurance subscriber, patient portal account, patient portal reset, patient portal access, patient portal authentication, and patient portal session readiness parity plans for both legacy and modernized targets.
...
The reusable parity test harness lives in `parity-tests/` and is launched by `scripts/Run-OpenEmrParityTests.ps1`. It currently provides database, HTTP, Playwright UI, workflow mutation, named run plans, a side-by-side slice-1 readiness plan through a side-by-side slice-209 patient portal home readiness plan, and full-suite legacy runs.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-210 patient portal secure-message readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
