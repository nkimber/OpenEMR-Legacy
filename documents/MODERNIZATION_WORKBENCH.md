...
- The API can run the shared slice-18 admin facility mutation readiness parity plan, the shared slice-19 admin user mutation readiness parity plan, the shared slice-20 access-control readiness parity plan, the shared slice-21 access-permission mutation readiness parity plan, the shared slice-22 user group membership mutation readiness parity plan, the shared slice-159 through slice-196 authentication, protection, authorization-policy, mutation authorization-policy, encounter amendment history, patient duplicate detection, patient registration validation, patient deceased status, patient guardian contact, patient guardian details, and patient social details readiness parity plans for both legacy and modernized targets.
...
The reusable parity test harness lives in `parity-tests/` and is launched by `scripts/Run-OpenEmrParityTests.ps1`. It currently provides database, HTTP, Playwright UI, workflow mutation, named run plans, a side-by-side slice-1 readiness plan through a side-by-side slice-196 patient social details readiness plan, and full-suite legacy runs.
