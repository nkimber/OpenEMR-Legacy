...
The reusable parity test harness lives in `parity-tests/` and is launched by `scripts/Run-OpenEmrParityTests.ps1`. It currently provides database, HTTP, Playwright UI, workflow mutation, named run plans, a side-by-side slice-1 readiness plan through a side-by-side slice-141 procedure lab provider lifecycle readiness plan, and full-suite legacy runs.
...
Slice 141 extends the harness to a side-by-side procedure lab provider lifecycle readiness plan and adds temporary create/deactivate/include-inactive/delete evidence to the UI/parity evidence surfaced by the Workbench. Slice 140 remains available for permanent provider-directory evidence, Slice 139 remains available for permanent lab-provider catalog ownership evidence, Slice 138 remains available for lab-filter evidence, Slice 137 remains available for provider-filter evidence, Slice 136 remains available for patient/date-filter evidence, and Slice 135 remains available for procedure report review queue transition evidence.
