...
- The API can run the shared slice-88 patient image document preview readiness, slice-89 patient image document thumbnail readiness, and slice-90 patient PDF document inline-preview readiness parity plans for both legacy and modernized targets.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-90 patient PDF document inline-preview readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
