...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness, slice-2 scheduling readiness, slice-3 encounters readiness, slice-4 clinical-lists readiness, slice-5 messaging readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
