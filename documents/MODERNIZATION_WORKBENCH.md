...
- The API can run the shared slice-11 appointment mutation readiness, slice-93 appointment reschedule readiness, and slice-94 appointment arrival readiness parity plans for both legacy and modernized targets.
...
- The API can run the shared slice-93 appointment reschedule readiness and slice-94 appointment arrival readiness parity plans for both legacy and modernized targets.
...
The shared patient scanned attachment plan runs against both targets and verifies temporary scanned PDF document create/archive/delete behavior, derived scan status, capture source, scanned page count, OCR status, and modernized Documents card and viewer scan-readiness rendering. The shared appointment reschedule plan runs against both targets and verifies temporary future appointment create/update/delete behavior, changed date/time/duration/status/room facts, and modernized Calendar reschedule rendering. The shared appointment arrival plan runs against both targets and verifies temporary future appointment create/mark-arrived/delete behavior, arrived status facts, and modernized Calendar `Mark arrived` rendering.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-94 appointment arrival readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
