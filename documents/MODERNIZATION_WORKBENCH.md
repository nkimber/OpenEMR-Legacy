...
- The API can run the shared slice-12 encounter mutation readiness, slice-168 encounter protection readiness, slice-169 document protection readiness, slice-170 message protection readiness, and slice-171 billing protection readiness parity plans for both legacy and modernized targets.
...
- The API can run the shared slice-18 admin facility mutation readiness parity plan, the shared slice-19 admin user mutation readiness parity plan, the shared slice-20 access-control readiness parity plan, the shared slice-21 access-permission mutation readiness parity plan, the shared slice-22 user group membership mutation readiness parity plan, the shared slice-159 admin login readiness parity plan, the shared slice-160 admin login audit readiness parity plan, the shared slice-161 admin session readiness parity plan, the shared slice-162 admin audit protection readiness parity plan, the shared slice-163 admin directory protection readiness parity plan, the shared slice-164 reports protection readiness parity plan, the shared slice-165 patient protection readiness parity plan, the shared slice-166 clinical-list protection readiness parity plan, the shared slice-167 appointment protection readiness parity plan, the shared slice-168 encounter protection readiness parity plan, the shared slice-169 document protection readiness parity plan, and the shared slice-170 message protection readiness parity plan for both legacy and modernized targets.
...
The reusable parity test harness lives in `parity-tests/` and is launched by `scripts/Run-OpenEmrParityTests.ps1`. It currently provides database, HTTP, Playwright UI, workflow mutation, named run plans, a side-by-side slice-1 readiness plan through a side-by-side slice-170 message protection readiness plan, and full-suite legacy runs.
...
The modernized target test command is `modernized-openemr/scripts/Test-ModernizedBaseline.ps1`, which writes `modernized-openemr/artifacts/latest-modernized-smoke-test.json`. It checks API health, admin login readiness, admin session readiness, login audit readiness, protected login-audit access, protected administration directory access, protected operational reports and CSV export access, protected patient search access, protected clinical-list access, protected appointment search access, protected encounter search access, protected patient document access, protected patient message access, deterministic anchor patient search for `MOD-PAT-0001`, the anchor chart summary response, patient demographics update/restore lifecycle for `MOD-PAT-0010`, temporary patient registration create/search/load/delete lifecycle, anchor insurance coverage and insurance coverage mutation lifecycle for `MOD-PAT-0005`, anchor immunization history for `MOD-PAT-0007`, appointment search/detail for `MOD-PAT-0003`, appointment create/cancel/delete lifecycle cleanup, appointment reschedule lifecycle cleanup, appointment arrival lifecycle cleanup, appointment check-out lifecycle cleanup, appointment no-show lifecycle cleanup, appointment category lifecycle cleanup, appointment pending-status lifecycle cleanup, appointment provider reassignment lifecycle cleanup, appointment facility reassignment lifecycle cleanup, appointment billing-location reassignment lifecycle, appointment comments lifecycle cleanup, appointment recurrence metadata lifecycle cleanup, appointment monthly recurrence lifecycle cleanup, appointment recurrence unit matrix lifecycle cleanup, appointment days-of-week recurrence lifecycle cleanup, appointment monthly repeat-on recurrence lifecycle cleanup, appointment series recurrence update propagation, appointment provider overlap tolerance, appointment patient overlap tolerance, appointment room overlap tolerance, appointment recurring-series expansion, appointment recurrence-exception expansion, appointment occurrence cancellation exception, appointment occurrence restore exception, appointment occurrence reschedule exception, appointment recurrence exception-list edit, appointment series root update propagation, appointment series root metadata propagation, encounter co-signature readiness, encounter document revision readiness, encounter document replacement revision readiness, encounter scanned attachment readiness, patient message reply update, patient message portal metadata rendering, patient message update metadata readiness, patient binary document content replacement lifecycle, procedure result correction lifecycle, procedure specimen metadata through the procedure lifecycle, procedure specimen detail lifecycle, procedure order correction lifecycle, procedure report correction lifecycle, procedure report sign-off lifecycle, procedure report review queue lifecycle, procedure report review queue filter lifecycle, procedure report review queue provider filter lifecycle, procedure report review queue lab filter lifecycle, procedure lab provider catalog/directory/lifecycle/configuration readiness, and the existing encounter, clinical-list, message, document, prescription, immunization, procedure, billing, administration, access-control, operational-report, and CSV export smoke checks. The reusable side-by-side commands include the `slice-1-readiness` through `slice-170-message-protection-readiness` parity plans, which write latest summaries for both `legacy-openemr` and `modernized-openemr` under `parity-tests/artifacts/` and can be compared with the parity comparison runner.
