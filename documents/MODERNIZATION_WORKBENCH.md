...
The legacy workflow mutation run covers deterministic demographics, appointment, encounter-detail, clinical-list, patient-message, prescription, billing, and lab procedure lifecycles. The shared patient contact, appointment, encounter, and clinical-list mutation plans now run against both legacy and modernized targets, updating/rendering/restoring the same anchor patient contact record, creating/cancelling/deleting the same anchor patient's future appointment lifecycle, creating/updating/rendering/deleting the same anchor patient's encounter with vitals and SOAP details, and creating/rendering/deactivating/deleting the same anchor patient's allergy list entry. The Workbench mutation commands use per-test reseeding for stronger isolation, while the tests also perform cleanup so they can safely run inside broader plans.
