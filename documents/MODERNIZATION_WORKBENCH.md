...
The `/api/progress` response also reconstructs a committed progress history from Git by reading `modernization-workbench/config/functionality-progress.json` as it existed at each first-parent commit that changed the ledger after it was introduced. Each history point exposes the Git commit completion timestamp as `snapshotCompletedAt`, and the Progress page plots the weighted completion line chart on that timestamp-based x-axis so longer gaps between completed snapshots remain visible. The chart supports brush-to-zoom range selection, resettable zoom, rich hover details for each snapshot, and click-to-inspect snapshot details that show commit identity, weighted percent, delta, elapsed time since the prior snapshot, remaining weighted work, and the unweighted comparison signal. Commits before scope weights existed fall back to equal weighting for their historical point.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-208 patient portal session readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
