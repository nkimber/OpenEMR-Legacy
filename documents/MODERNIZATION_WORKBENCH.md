...
The `/api/progress` response also reconstructs a committed progress history from Git by reading `modernization-workbench/config/functionality-progress.json` as it existed at each first-parent commit that changed the ledger after it was introduced. Each history point exposes the Git commit completion timestamp as `snapshotCompletedAt`, and the Progress page plots the weighted completion line chart on that timestamp-based x-axis so longer gaps between completed snapshots remain visible. Commits before scope weights existed fall back to equal weighting for their historical point.
