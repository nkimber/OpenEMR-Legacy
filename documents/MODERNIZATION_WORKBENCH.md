...
- The API can run the shared slice-86 encounter document lifecycle timeline readiness parity plan for both legacy and modernized targets.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-86 encounter document lifecycle timeline readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
