...
The shared patient scanned attachment plan runs against both targets and verifies temporary scanned PDF document create/archive/delete behavior, derived scan status, capture source, scanned page count, OCR status, and modernized Documents card and viewer scan-readiness rendering.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-92 patient scanned attachment readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
