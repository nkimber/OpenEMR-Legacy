...
The `/api/progress` response also reconstructs a committed progress history from Git by reading `modernization-workbench/config/functionality-progress.json` as it existed at each first-parent commit that changed the ledger after it was introduced. It prepends a Git-derived timeline anchor for the first first-parent commit that touched `modernized-openemr` so the chart starts when modernized application implementation began, even though progress estimates were not captured yet. In the current history that anchor is commit `6586a4a0`, `Bootstrap modernized OpenEMR slice one`. Each measured history point exposes the Git commit completion timestamp as `snapshotCompletedAt`, and the Progress page plots the weighted completion line chart on that timestamp-based x-axis so longer gaps between completed snapshots remain visible. The chart renders the pre-ledger anchor span as a dashed segment, supports brush-to-zoom range selection, resettable zoom, rich hover details, and click-to-inspect details. Anchor details explain that the percentage was not captured yet and measured snapshot details show commit identity, weighted percent, delta, elapsed time since the prior timeline point, remaining weighted work, and the unweighted comparison signal. Commits before scope weights existed fall back to equal weighting for their historical point.
