...
Slice 143 adds the functionality progress ledger to the Progress page. Slice 146 adds explicit scope-adjusted completion estimates to that ledger. Slice 152 adds weighted overall progress, Git-backed progress history, and rough remaining-time forecasting. Slice 155 adds direct comparison-side links to run JSON, Playwright JSON, JUnit XML, and HTML reports. Slice 182 adds managed procedure authorization-policy plan actions for both legacy and modernized targets; Slice 181 billing authorization-policy, Slice 180 patient message authorization-policy, and Slice 179 patient document authorization-policy, Slice 178 patient chart authorization-policy, Slice 177 encounter authorization-policy, Slice 176 appointment authorization-policy, Slice 175 clinical-list authorization-policy, Slice 174 reports authorization-policy, Slice 173 admin authorization-policy, Slice 172 procedure protection, Slice 171 billing protection, Slice 170 message protection, Slice 169 document protection, Slice 168 encounter protection, Slice 167 appointment protection, Slice 166 clinical-list protection, Slice 165 patient chart protection, Slice 164 operational reports protection, Slice 163 admin directory protection, Slice 162 admin audit protection, Slice 161 admin session readiness, Slice 160 admin login audit readiness, and Slice 159 admin login readiness actions remain available.
...
- The API can run the shared slice-18 admin facility mutation readiness parity plan, the shared slice-19 admin user mutation readiness parity plan, the shared slice-20 access-control readiness parity plan, the shared slice-21 access-permission mutation readiness parity plan, the shared slice-22 user group membership mutation readiness parity plan, the shared slice-159 admin login readiness parity plan, the shared slice-160 admin login audit readiness parity plan, the shared slice-161 admin session readiness parity plan, the shared slice-162 admin audit protection readiness parity plan, the shared slice-163 admin directory protection readiness parity plan, the shared slice-173 admin authorization-policy readiness parity plan, the shared slice-164 reports protection readiness parity plan, the shared slice-174 reports authorization-policy readiness parity plan, the shared slice-175 clinical-list authorization-policy readiness parity plan, the shared slice-176 appointment authorization-policy readiness parity plan, the shared slice-177 encounter authorization-policy readiness parity plan, the shared slice-178 patient chart authorization-policy readiness parity plan, the shared slice-179 patient document authorization-policy readiness parity plan, the shared slice-180 patient message authorization-policy readiness parity plan, the shared slice-181 billing authorization-policy readiness parity plan, the shared slice-182 procedure authorization-policy readiness parity plan, the shared slice-165 patient protection readiness parity plan, the shared slice-166 clinical-list protection readiness parity plan, the shared slice-167 appointment protection readiness parity plan, the shared slice-168 encounter protection readiness parity plan, the shared slice-169 document protection readiness parity plan, the shared slice-170 message protection readiness parity plan, the shared slice-171 billing protection readiness parity plan, and the shared slice-172 procedure protection readiness parity plan for both legacy and modernized targets.
...
The reusable parity test harness lives in `parity-tests/` and is launched by `scripts/Run-OpenEmrParityTests.ps1`. It currently provides database, HTTP, Playwright UI, workflow mutation, named run plans, a side-by-side slice-1 readiness plan through a side-by-side slice-182 procedure authorization-policy readiness plan, and full-suite legacy runs.
...
Slice 182 adds procedure authorization-policy evidence: the legacy side proves the ACL matrix grants Lab Results access to Administrators and not Front Office, while the modernized side proves an authenticated front-desk session receives 403 for procedure result retrieval, order-catalog retrieval, and procedure-order create while an admin session still retrieves procedure result/catalog facts through API and Procedures UI. Slice 181 remains available for billing authorization-policy evidence: the legacy side proves the ACL matrix grants Billing access to Administrators/Accounting and not Front Office, while the modernized side proves an authenticated front-desk session receives 403 for billing retrieval, statement batch, and billing-line create while an admin session still retrieves fee-sheet and revenue-cycle facts through API and Fees UI. Slice 180 remains available for patient message authorization-policy evidence: the legacy side proves the ACL matrix grants Patient Notes access to Administrators and not Front Office, while the modernized side proves an authenticated front-desk session receives 403 for message list/create and an admin session still retrieves the same messages through API and Messages UI. Slice 179 remains available for patient document authorization-policy evidence: the legacy side proves the ACL matrix grants Documents access to Administrators and not Front Office, while the modernized side proves an authenticated front-desk session receives 403 for document list/content/create and an admin session still retrieves the same documents through API and Documents UI. Slice 178 remains available for patient chart authorization-policy evidence: the legacy side proves the ACL matrix grants Demographics access to Administrators and Front Office, while the modernized side proves both admin and front-desk sessions can retrieve the same patient chart through API and Patient/Client UI after ACL enforcement. Slice 177 remains available for encounter authorization-policy evidence: the legacy side proves the ACL matrix grants Authorize Any Encounter access to Administrators and not Front Office, while the modernized side proves an authenticated front-desk session receives 403 for encounter search and mutation and an admin session still retrieves the same encounter through API and Encounters UI. Slice 176 remains available for appointment authorization-policy evidence: the legacy side proves the ACL matrix grants Appointment access to Administrators and Front Office, while the modernized side proves the seeded front-desk session can retrieve the same appointment through API and Calendar UI after its Front Office membership is applied. Slice 175 remains available for clinical-list authorization-policy evidence: the legacy side proves the ACL matrix grants Medical/History access to Administrators and not Front Office, while the modernized side proves an authenticated front-desk session receives 403 for clinical-list retrieval and allergy mutation and an admin session still succeeds. Slice 174 remains available for operational reports authorization-policy evidence: the legacy side proves the ACL matrix grants Patient Report access to Administrators and not Front Office, while the modernized side proves an authenticated front-desk session receives 403 for operational report JSON and CSV export and an admin session still succeeds. Slice 173 remains available for administration authorization-policy evidence: the legacy side proves the ACL matrix grants `admin:acl write` to Administrators and not Front Office, while the modernized side proves an authenticated front-desk session receives 403 for administration directory and mutation access and an admin session still succeeds. Slice 166 extends the harness to a side-by-side clinical-list protection plan and adds legacy patient-summary no-session/authenticated-session clinical-list evidence, modernized `/api/clinical-lists` no-session/active-session evidence, modernized Lists workspace sign-in gating, and comparison evidence to the UI/parity evidence surfaced by the Workbench. Slice 165 extends the harness to a side-by-side patient chart protection plan and adds legacy patient-summary no-session/authenticated-session evidence, modernized `/api/patients` search and chart no-session/active-session evidence, modernized Patient/Client sign-in gating, and comparison evidence to the UI/parity evidence surfaced by the Workbench. Slice 164 remains available for operational reports protection evidence, legacy `patient_list.php` / `clinical_reports.php` no-session/authenticated-session evidence, modernized `/api/reports/operational` and `/api/reports/operational/export` no-session/active-session evidence, modernized Reports workspace sign-in gating, and comparison evidence. Slice 163 remains available for admin directory protection evidence, legacy `usergroup_admin.php` / `facilities.php` no-session/authenticated-session evidence, modernized `/api/administration/directory` no-session/active-session evidence, modernized administration mutation no-session rejection evidence, modernized Admin directory sign-in gating, and comparison evidence. Slice 162 remains available for admin audit protection evidence, legacy `logview.php` no-session/authenticated-session evidence, modernized `/api/auth/login-audit` no-session/active-session/ended-session evidence, modernized Admin Login Audit sign-in gating, and comparison evidence. Slice 161 remains available for admin session readiness evidence, legacy `OpenEMR` cookie/logout evidence, modernized `auth_sessions` evidence, `/api/auth/session` and `/api/auth/logout` evidence, modernized Admin Session Readiness panel rendering, and comparison evidence. Slice 160 remains available for admin login audit readiness evidence, legacy decoded `log` row evidence, modernized `auth_audit_events` evidence, authenticated `/api/auth/login-audit` evidence, modernized Admin Login Audit panel rendering, and comparison evidence. Slice 159 remains available for admin login readiness evidence, legacy admin login success/failure evidence, modernized `/api/auth/login` evidence, modernized Admin Login Readiness panel rendering, and comparison evidence. Slice 158 remains available for patient message update metadata evidence, legacy `pnotes.update_by` / `update_date` evidence, modernized PostgreSQL/API update metadata evidence, modernized Messages update-user rendering, cleanup, and comparison evidence. Slice 157 remains available for patient message portal metadata evidence, legacy `pnotes.portal_relation` / `is_msg_encrypted` evidence, modernized PostgreSQL/API metadata evidence, modernized Messages card rendering, and comparison evidence. Slice 156 remains available for pnotes-compatible reply append evidence, unchanged message counts, legacy patient-notes rendering, modernized Messages reply controls, and comparison evidence. Slice 154 remains available for signed-to-received state evidence, reviewed-queue exclusion, unreviewed-queue inclusion, blank reviewer metadata, legacy `list_reports.php` option `3` rendering, modernized Procedures/Reports workspace controls, and comparison evidence. Slice 153 remains available for two-report unreviewed queue evidence, one-operation reviewed-state mutation evidence, legacy `list_reports.php` option `2` rendering, modernized procedure report bulk-sign API/UI controls, and comparison evidence. Slice 151 remains available for temporary reportless-order transmit marking evidence, sent-awaiting-results queue evidence, legacy `list_reports.php` option `4` rendering, modernized `procedure order queue` API/UI transmit controls, and comparison evidence. Slice 149 remains available for temporary reportless-order ready-to-send evidence, reported queue transition evidence after report attachment, legacy `list_reports.php` rendering, modernized `procedure order queue` API/UI controls, and comparison evidence. Slice 148 remains available for temporary catalog group import evidence, PathGroup-style order/result CSV behavior, legacy-compatible deactivate/reactivate semantics, legacy `procedure_type` rendering, and modernized `lab_order_catalog` API/UI controls. Slice 147 remains available for temporary catalog item lifecycle evidence, Slice 145 remains available for permanent order catalog evidence, Slice 144 remains available for temporary provider address-book linkage evidence, Slice 142 remains available for temporary provider configuration evidence, Slice 141 remains available for temporary provider lifecycle evidence, Slice 140 remains available for permanent provider-directory evidence, Slice 139 remains available for permanent lab-provider catalog ownership evidence, Slice 138 remains available for lab-filter evidence, Slice 137 remains available for provider-filter evidence, Slice 136 remains available for patient/date-filter evidence, and Slice 135 remains available for procedure report review queue transition evidence.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-170 message protection readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
...
The modernized target test command is `modernized-openemr/scripts/Test-ModernizedBaseline.ps1`, which writes `modernized-openemr/artifacts/latest-modernized-smoke-test.json`. It checks API health, admin login/session/audit readiness, protected access for audit, administration, reports, patient search, clinical lists, appointments, encounters, documents, messages, billing, and procedures, deterministic anchor patient search for `MOD-PAT-0001`, the anchor chart summary response, cleanup-backed patient, appointment, encounter, document, message, revenue-cycle, and procedure/lab mutation lifecycles, and the existing administration, access-control, operational-report, and CSV export smoke checks. The reusable side-by-side commands include the `slice-1-readiness` through `slice-181-billing-authorization-policy-readiness` parity plans, which write latest summaries for both `legacy-openemr` and `modernized-openemr` under `parity-tests/artifacts/` and can be compared with the parity comparison runner.
