...
- The API can run the shared slice-18 admin facility mutation readiness parity plan, the shared slice-19 admin user mutation readiness parity plan, the shared slice-20 access-control readiness parity plan, the shared slice-21 access-permission mutation readiness parity plan, the shared slice-22 user group membership mutation readiness parity plan, the shared slice-159 through slice-206 authentication, protection, authorization-policy, mutation authorization-policy, encounter amendment history, patient duplicate detection, patient registration validation, patient deceased status, patient guardian contact, patient guardian details, patient social details, patient employer, patient provider assignment, patient care-team, patient care-team members, patient care-team contact, patient history, insurance subscriber, patient portal account, patient portal reset, and patient portal access readiness parity plans for both legacy and modernized targets.
...
The reusable parity test harness lives in `parity-tests/` and is launched by `scripts/Run-OpenEmrParityTests.ps1`. It currently provides database, HTTP, Playwright UI, workflow mutation, named run plans, a side-by-side slice-1 readiness plan through a side-by-side slice-206 patient portal access readiness plan, and full-suite legacy runs.
...
Slice 206 adds patient portal access evidence: the shared plan reuses the existing `MOD-PAT-0004` provisioned account, revokes portal access, verifies legacy `patient_data.allow_patient_portal`, verifies the modernized API/UI access projection through the Patient/Client Portal Account panel, grants access again, and restores the original enabled state while preserving CMS login and onsite account facts. Slice 205 remains available with patient portal reset evidence: the shared plan issues one-time reset state, verifies legacy `patient_access_onsite.portal_pwd_status` / `portal_onetime`, verifies the modernized API/UI reset projection through the Patient/Client Portal Account panel, clears the reset, and restores the original patient-managed state. Slice 204 remains available with patient portal account evidence: the shared plan reads the existing `MOD-PAT-0004` account anchor, verifies legacy OpenEMR `patient_data.cmsportal_login` and `patient_access_onsite` fields plus demographics-page rendering, and verifies the modernized API/UI projection through the Patient/Client Portal Account panel. Slice 203 remains available with insurance subscriber evidence: the shared plan reads the existing `MOD-PAT-0005` secondary subscriber anchor, verifies legacy OpenEMR `insurance_data` subscriber fields and insurance-detail rendering, verifies the modernized API/UI projection through the Patient/Client insurance panel, and creates/updates/deletes a temporary tertiary coverage row with subscriber details. Slice 202 remains available with patient history/lifestyle evidence: the shared plan reads the existing `MOD-PAT-0010` anchor history row, verifies legacy OpenEMR `history_data` rendering on the History and Lifestyle page, verifies the modernized API/UI projection through the Patient/Client History and Lifestyle panel, and keeps the run read-only. Slice 201 remains available with patient contact-backed care-team evidence: the shared plan updates the existing `MOD-PAT-0010` anchor to a mixed provider/contact care team with provider `103` / `Alex Chen` at facility `12` / `East County Care Center` and related contact `3200010` / `Casey Brooks`, verifies legacy OpenEMR `care_teams` / `care_team_member.contact_id` behavior, verifies the modernized API/UI projection through the provider/contact Care Team panel, and restores the original members. Slice 200 remains available with patient care-team members evidence: the shared plan updates the existing `MOD-PAT-0010` anchor to a two-member care team with provider `103` / `Alex Chen` at facility `12` / `East County Care Center` and provider `104` / `Robin Morris` at facility `10` / `Modernization Family Medicine`, verifies the legacy OpenEMR `care_teams` / `care_team_member` contract, verifies the modernized API/UI projection through the multi-member Care Team panel, and restores the original members. Slice 199 remains available for patient care-team lead-member evidence. Slice 198 remains available for patient provider assignment evidence. Slice 197 remains available for patient employer evidence. Slice 196 remains available for patient social-detail evidence. Slice 195 remains available for patient guardian demographic/address evidence. Slice 194 remains available for patient guardian-contact evidence. Slice 193 remains available for patient deceased-status evidence, Slice 192 remains available for patient registration validation evidence, Slice 191 remains available for patient duplicate detection evidence, Slice 190 remains available for encounter amendment history evidence, Slice 189 remains available for appointment mutation authorization-policy evidence, Slice 188 remains available for billing mutation authorization-policy evidence, Slice 187 remains available for message mutation authorization-policy evidence, Slice 186 remains available for document mutation authorization-policy evidence, Slice 185 remains available for encounter mutation authorization-policy evidence, and Slice 184 remains available for patient mutation authorization-policy evidence. Earlier protection, mutation, report, queue, provider, catalog, administration, and authorization-policy evidence remains available through the existing Workbench cards and comparison drill-ins.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-206 patient portal access readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
