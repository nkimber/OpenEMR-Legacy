...
- The API can run the shared slice-18 admin facility mutation readiness parity plan, the shared slice-19 admin user mutation readiness parity plan, the shared slice-20 access-control readiness parity plan, the shared slice-21 access-permission mutation readiness parity plan, the shared slice-22 user group membership mutation readiness parity plan, the shared slice-159 through slice-189 authentication, protection, authorization-policy, and mutation authorization-policy readiness parity plans for both legacy and modernized targets.
...
The reusable parity test harness lives in `parity-tests/` and is launched by `scripts/Run-OpenEmrParityTests.ps1`. It currently provides database, HTTP, Playwright UI, workflow mutation, named run plans, a side-by-side slice-1 readiness plan through a side-by-side slice-189 appointment mutation authorization-policy readiness plan, and full-suite legacy runs.
...
Slice 189 adds appointment mutation authorization-policy evidence: the shared plan temporarily downgrades Clinicians from `patients:appt write` to `patients:appt view`, proves the clinician can still read appointment facts while appointment create, update, status, recurrence restore, occurrence reschedule, and delete mutations receive 403 requiring Appointment write, then restores the permanent Appointment write grant. Slice 188 remains available for billing mutation authorization-policy evidence, Slice 187 remains available for message mutation authorization-policy evidence, Slice 186 remains available for document mutation authorization-policy evidence, Slice 185 remains available for encounter mutation authorization-policy evidence, Slice 184 remains available for patient mutation authorization-policy evidence, and Slice 183 remains available for procedure mutation authorization-policy evidence. Earlier protection, mutation, report, queue, provider, catalog, administration, and authorization-policy evidence remains available through the existing Workbench cards and comparison drill-ins.
