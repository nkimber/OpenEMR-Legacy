...
Slice 133 extends the Workbench-managed procedure family with the shared procedure report correction plan. That plan verifies corrected collected date, report date, specimen number, report status, review status, notes, UI/API/database facts, linked result preservation, and cleanup against both legacy and modernized targets. Slice 132 order correction remains available for temporary lab order metadata facts, and Slice 131 specimen detail remains available for temporary order-level specimen facts.
...
The Workbench now exposes curated plan actions for legacy readiness, slice-1 side-by-side readiness through slice-133 procedure report correction readiness, isolated mutations, and the full legacy parity contract. Plan evidence displays the selected suites so an operator can distinguish a plan run from an individual suite run.
