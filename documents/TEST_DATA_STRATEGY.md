...
Current status: implemented as `openemr-shared-synthetic-v1` and verified against the legacy MariaDB baseline and the modernized PostgreSQL seed through the implemented read-only slices plus patient contact, patient demographics, patient registration, patient insurance, appointment, encounter, clinical-list allergy, problem-list, medication-list, patient-message, text patient-document, binary patient-document, prescription, immunization, billing, procedure, administration facility, administration user, access-permission, and user group membership mutation slices, including the read-only default ACL access-control matrix slice, the read-only pending/scheduled procedure-order slice, and the read-only patient documents slice.
...
- Patient document metadata, OpenEMR document categories, encounter links, and deterministic text payload previews.
