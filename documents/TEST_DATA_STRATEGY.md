...
Current status: implemented as `openemr-shared-synthetic-v1` and verified against the legacy MariaDB baseline and the modernized PostgreSQL seed through the implemented read-only slices plus patient contact, appointment, encounter, clinical-list allergy, patient-message, prescription, billing, procedure, administration facility, and administration user mutation slices, including the read-only default ACL access-control matrix slice.
...
The administration access-control slice uses OpenEMR's default ACL topology rather than synthetic patient-specific records. The modernized seed mirrors 7 access groups, 65 visible permission objects, and 203 group-permission assignments from the legacy default phpGACL data. The `slice-20-access-control-readiness` plan verifies group counts, key permission objects, representative group-permission assignments, and browser-visible access-control surfaces against both legacy MariaDB and modernized PostgreSQL.
