...
- Claim status, payment posting, and account balance coverage, with deeper revenue-cycle behavior later once the project selects those workflows.
...
The account balance readiness slice reuses the same seeded billing, claim, and AR payment rows rather than adding new fixture data. The `MOD-PAT-0005` account anchor proves patient-level totals of `$635.00` charges, `$206.00` paid, `$64.25` adjusted, and `$364.75` balance, plus encounter `1000052` totals of `$186.00` charges, `$126.00` paid, `$42.00` adjusted, and `$18.00` balance. The `slice-49-account-balance-readiness` plan verifies those rollups against both target databases and modernized Fees rendering. Statement generation, aging buckets, collection workflows, and payment reversal behavior remain future revenue-cycle slices.
