...
Current implementation state as of Slice 157: the modernized target has side-by-side parity coverage through the Slice 157 patient message portal metadata readiness slice. The shared gold dataset includes permanent procedure order catalog rows that seed legacy OpenEMR `procedure_type` and modernized PostgreSQL `lab_order_catalog`, and permanent patient-message metadata that seeds legacy `pnotes.portal_relation` / `is_msg_encrypted` and modernized `messages.portal_relation` / `is_encrypted`; the parity harness now verifies ready-to-send/reportless, sent-awaiting-results, reported, reviewed, reopened/unreviewed, replied-message, and portal-metadata workflow states without changing permanent counts except for intentional seed projection metadata. The Modernization Workbench can run the Slice 65, Slice 66, Slice 156, and Slice 157 message plans plus the Slice 145, Slice 147, Slice 148, Slice 149, Slice 151, Slice 153, and Slice 154 procedure plans against both targets, open direct comparison-side run/report evidence from Test Runs drill-ins, show weighted scope-adjusted completion, show Git-backed progress history and rough active-time forecasting for the curated functionality ledger, and show generated source inventory statistics for code/schema scale comparison.
