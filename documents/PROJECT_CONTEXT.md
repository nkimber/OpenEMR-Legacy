...
Current update: Slice 128 adds patient binary document content replacement readiness. The latest Workbench-specific slice remains Slice 125, which adds safe artifact links; the latest modernized OpenEMR workflow slice is Slice 128, and the target now covers thirty-eight read-only vertical slices plus eighty-eight mutation-capable slices through Slice 128.
