...
Current update: Slice 141 adds procedure lab provider lifecycle readiness. The latest Workbench-specific slice remains Slice 125, which adds safe artifact links; the latest modernized OpenEMR workflow slice is Slice 141, and the target now covers forty read-only vertical slices plus ninety-nine mutation-capable slices through Slice 141.
