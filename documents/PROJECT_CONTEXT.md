...
Recent Workbench progress continuation: Slice 143 adds a Workbench functionality progress ledger on the Progress page so operators can see completed, outstanding, and deferred modernization scope by domain area. Slice 146 extends that ledger with scope-adjusted percent-complete estimates and rationale text so the Progress page can show a directional modernization completion estimate while the known scope continues to evolve. Slice 154 extends the ledger with scope weights, a weighted overall estimate, Git-backed committed progress history, and a rough active-time forecast derived from weighted remaining scope and recent changelog slice durations.
...
Current update: Slice 154 adds Workbench weighted progress analytics. The Progress page now treats the weighted overall completion estimate as the main project progress signal, keeps the simple average visible, charts committed progress snapshots from Git, and estimates rough active time remaining from weighted remaining scope and recent slice duration cadence.
...
Current implementation state as of Slice 154: the modernized target has side-by-side parity coverage through the procedure order transmit readiness slice. The shared gold dataset includes permanent procedure order catalog rows that seed legacy OpenEMR `procedure_type` and modernized PostgreSQL `lab_order_catalog`; the parity harness now adds temporary order queue tests so ready-to-send/reportless, sent-awaiting-results, and reported lab-order queue states can be verified without changing permanent counts. The Modernization Workbench can run the Slice 145, Slice 147, Slice 148, Slice 149, and Slice 151 procedure plans against both targets, the Progress page shows weighted scope-adjusted completion, Git-backed progress history, and rough active-time forecasting for the curated functionality ledger, and the Architecture page shows generated source inventory statistics for code/schema scale comparison.
