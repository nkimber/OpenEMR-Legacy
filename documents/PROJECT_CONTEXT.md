...
Current update: Slice 123 adds encounter document replacement revision readiness. The plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, captures current revision facts, replaces the payload, verifies the current revision hash changes while remaining the current Version 1 row, confirms modernized API/UI replacement rendering, and hard-deletes the document. The matched comparison artifact records side-by-side parity, and the target now covers thirty-eight read-only vertical slices plus eighty-five mutation-capable slices through Slice 123.
