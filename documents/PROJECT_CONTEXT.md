...
Current update: Slice 138 adds procedure report review queue lab filter readiness. The latest Workbench-specific slice remains Slice 125, which adds safe artifact links; the latest modernized OpenEMR workflow slice is Slice 138, and the target now covers thirty-eight read-only vertical slices plus ninety-eight mutation-capable slices through Slice 138.
