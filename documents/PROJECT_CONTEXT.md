...
Current update: Slice 140 adds procedure lab provider directory readiness. The latest Workbench-specific slice remains Slice 125, which adds safe artifact links; the latest modernized OpenEMR workflow slice is Slice 140, and the target now covers forty read-only vertical slices plus ninety-eight mutation-capable slices through Slice 140.
