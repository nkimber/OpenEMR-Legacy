...
Current update: Slice 131 adds procedure specimen detail readiness. The latest Workbench-specific slice remains Slice 125, which adds safe artifact links; the latest modernized OpenEMR workflow slice is Slice 131, and the target now covers thirty-eight read-only vertical slices plus ninety-one mutation-capable slices through Slice 131.
