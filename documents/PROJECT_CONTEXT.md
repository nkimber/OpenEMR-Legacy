...
Current update: Slice 151 follows the procedure order queue slice by adding focused procedure order transmit readiness. Temporary reportless lab orders can be marked transmitted on both targets, leave the ready-to-send queue, appear in the sent-awaiting-results queue through legacy `list_reports.php` option `4` and the modernized Reports workspace, clean up, and compare side-by-side. The slice intentionally proves the `date_transmitted` state transition only; actual HL7/SFTP/filesystem/web-service transport remains future scope.
