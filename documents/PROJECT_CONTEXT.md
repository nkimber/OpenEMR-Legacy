...
Current update: Slice 150 adds Workbench Architecture source inventory statistics. The Architecture page now shows source-file counts, physical line counts, non-blank line counts, component breakdowns, and schema-signal counts for the legacy OpenEMR baseline, the Workbench, and the modernized target, using a generated snapshot refreshed by `npm run generate:source-inventory`.
