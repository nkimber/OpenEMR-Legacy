...
Current update: Slice 147 follows the permanent procedure order catalog slice by adding focused procedure order catalog lifecycle behavior. Temporary orderable catalog items can be created under the permanent provider group, updated, rendered on legacy OpenEMR and the modernized Reports workspace, deactivated, deleted, and compared side-by-side. Slice 146 remains the latest Workbench-specific slice and the current Workbench estimate is 53% complete overall across the 11 tracked functionality areas.
...
Current implementation state as of Slice 147: the modernized target has side-by-side parity coverage through the procedure order catalog lifecycle readiness slice. The shared gold dataset includes permanent procedure order catalog rows that seed legacy OpenEMR `procedure_type` and modernized PostgreSQL `lab_order_catalog`, the Modernization Workbench can run the Slice 145 and Slice 147 procedure order catalog plans against both targets, and the Workbench Progress page shows scope-adjusted completion estimates for the curated functionality ledger.
