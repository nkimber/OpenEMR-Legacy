...
Recent specimen continuation: Slice 130 follows the focused procedure result correction slice by proving that report-level collected date and specimen number are preserved in the modernized PostgreSQL schema, procedure API, Encounter/Procedures UI cards, workflow probes, and side-by-side parity tests.
...
Current update: Slice 130 adds procedure specimen readiness. The latest Workbench-specific slice remains Slice 125, which adds safe artifact links; the latest modernized OpenEMR workflow slice is Slice 130, and the target now covers thirty-eight read-only vertical slices plus ninety mutation-capable slices through Slice 130.
