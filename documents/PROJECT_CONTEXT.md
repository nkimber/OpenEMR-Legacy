...
Current update: Slice 137 adds procedure report review queue provider filter readiness. The latest Workbench-specific slice remains Slice 125, which adds safe artifact links; the latest modernized OpenEMR workflow slice is Slice 137, and the target now covers thirty-eight read-only vertical slices plus ninety-seven mutation-capable slices through Slice 137.
