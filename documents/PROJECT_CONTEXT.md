...
Current update: Slice 136 adds procedure report review queue patient/date filter readiness. The latest Workbench-specific slice remains Slice 125, which adds safe artifact links; the latest modernized OpenEMR workflow slice is Slice 136, and the target now covers thirty-eight read-only vertical slices plus ninety-six mutation-capable slices through Slice 136.
