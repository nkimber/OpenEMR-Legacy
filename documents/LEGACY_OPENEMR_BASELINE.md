...
- Shared procedure order catalog plan now runs against the legacy target as the legacy half of Slice 145 side-by-side parity, using permanent `procedure_type` rows `9000` through `9999` to verify the `Gold Lab Order Catalog` root, five provider groups, three orderable panels per provider, native `interface/orders/types.php` rendering, and `types_ajax.php` child-row expansion.
- Shared procedure order catalog lifecycle plan now runs against the legacy target as the legacy half of Slice 147 side-by-side parity, using temporary `procedure_type` rows under provider group `9040` to verify create, update, activity-state change, native `types_ajax.php` rendering, delete, and cleanup behavior.
- Shared procedure vendor compendium import plan now runs against the legacy target as the legacy half of Slice 148 side-by-side parity, using a temporary `procedure_type` group under root `9000` to verify PathGroup-style order/result CSV import, OpenEMR-compatible order deactivation/reactivation on re-import, native `types_ajax.php` rendering, subtree delete, and cleanup behavior without changing the permanent 21-row catalog.
- Shared procedure order queue plan now runs against the legacy target as the legacy half of Slice 149 side-by-side parity, creating a temporary reportless lab order for `MOD-PAT-0009`, verifying ready-to-send rendering through native `interface/orders/list_reports.php` option `5`, attaching a temporary report, verifying reported rendering through option `3`, and deleting the temporary order tree and encounter during cleanup.
- Shared procedure order transmit plan now runs against the legacy target as the legacy half of Slice 151 side-by-side parity, creating a temporary reportless lab order for `MOD-PAT-0009`, marking `procedure_order.date_transmitted`, verifying sent-not-received rendering through native `interface/orders/list_reports.php` option `4`, and deleting the temporary order tree and encounter during cleanup.
- Shared procedure report bulk sign-off plan now runs against the legacy target as the legacy half of Slice 153 side-by-side parity, creating two temporary unreviewed `procedure_report` rows for `MOD-PAT-0009`, bulk updating both to reviewed through OpenEMR-compatible `procedure_report.review_status`, `source`, and `date_report` fields, verifying reviewed rendering through native `interface/orders/list_reports.php` option `2`, and deleting both temporary order trees plus the encounter during cleanup.
- Shared procedure report reopen review plan now runs against the legacy target as the legacy half of Slice 154 side-by-side parity, creating one temporary signed `procedure_report` row for `MOD-PAT-0009`, updating `procedure_report.review_status` back to `received`, verifying unreviewed rendering through native `interface/orders/list_reports.php` option `3`, verifying normalized blank reviewer metadata, and deleting the temporary order tree plus encounter during cleanup.
...
- Shared patient portal appointment request plan now runs against the legacy target as the legacy half of Slice 220 side-by-side parity, creating a temporary pending `^` `openemr_postcalendar_events` row plus provider-facing `Patient Reminders` pnote from the signed-in `MOD-PAT-0004` portal account, verifying portal rendering, and hard-deleting both rows during cleanup.
...
- Patient portal appointment request parity creates temporary `MOD-PAT-0004` pending request rows only: a `^` appointment request and matching `Patient Reminders` provider note, both removed during cleanup so the 2,800-appointment baseline remains stable.
...
- Slice 154 extends both adapter families with procedure report reopen review readiness for temporary `procedure_report` / `lab_reports` rows, including signed-to-received state transition, reviewed queue exclusion, unreviewed queue inclusion, blank reviewer metadata, UI rendering, and cleanup verification.
...
- Slice 145 adds permanent procedure order catalog seed coverage, including legacy `procedure_type` IDs `9000` through `9999` and modernized `lab_order_catalog` rows for the same root, provider groups, and orderable panel facts.
- Slice 147 adds temporary procedure order catalog lifecycle coverage, including legacy `procedure_type` temporary order rows, modernized `lab_order_catalog` temporary rows, create/update/deactivate/delete behavior, rendering, and cleanup while preserving the permanent 21-row catalog baseline.
