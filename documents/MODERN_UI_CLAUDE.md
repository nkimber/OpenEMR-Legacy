...
- Registered in `modernization-workbench/config/apps.json` as a managed Docker Compose application. The Workbench Dashboard website launcher lists Modern UI Claude beside the legacy baseline and Modernized OpenEMR so operators can choose which website to open, start, or restart from the first Workbench screen. The Workbench Applications page can show whether the `modern-ui-claude` service is stopped/running, check `http://localhost:3100/`, start it with `docker compose up -d --build`, stop/restart it, and load recent compose logs.
- Deployed by the Workbench Demo Deployment page as its own Azure Container App when the `modern-ui-claude` target is selected. The Azure image is built from `infra/azure/demo/modern-ui-claude-demo.Dockerfile`, serves the static SPA through Nginx on port 8080, exposes `/health`, and proxies `/api/` to the deployed modernized OpenEMR public URL so the browser can use same-origin API calls.
