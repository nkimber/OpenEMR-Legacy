...
Active implementation as of 2026-06-24. The login + landing slice described above exists and runs in `modern-ui-claude/`. The app is now visible in the Modernization Workbench Applications page as a frontend-only managed Docker Compose service. `modern-ui-claude/README.md` has been updated to match. The patient portal landing page (`/portal/home`) is now a full dashboard (see above) including a health summary view and inbox message-thread/reply view; the clinician landing page (`/home`) is still a minimal welcome shell. Broader clinician-side screens (patient chart, scheduling, etc., per the approved Calm Clinical mockups) are not yet built.
