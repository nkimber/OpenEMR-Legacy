...
Slice 138 adds procedure report review queue lab filter readiness to the same modernized target contract with the `workflow-procedure-report-review-queue-lab-filters` suite and `slice-138-procedure-report-review-queue-lab-filters-readiness` plan. Slice 137 procedure report review queue provider filter readiness remains available through the `workflow-procedure-report-review-queue-provider-filters` suite and `slice-137-procedure-report-review-queue-provider-filters-readiness` plan, Slice 136 procedure report review queue patient/date filter readiness remains available through the `workflow-procedure-report-review-queue-filters` suite and `slice-136-procedure-report-review-queue-filters-readiness` plan, Slice 135 procedure report review queue readiness remains available through the `workflow-procedure-report-review-queue` suite and `slice-135-procedure-report-review-queue-readiness` plan, and Slice 134 procedure report sign-off remains available through the `workflow-procedure-report-signoff` suite and `slice-134-procedure-report-signoff-readiness` plan.
...
The Workbench Test Runs page also renders recent side-by-side comparison artifacts. Its `/api/parity-comparisons` route reads bounded `comparison.json` summaries from `parity-tests/artifacts/comparisons/`, normalizes left/right run metadata, exposes difference counts and previews, and leaves command execution to the existing runner/compare scripts. Slice 124 adds expandable card drill-ins so operators can review legacy/modernized run artifact paths, comparison JSON paths, artifact directories, selected suites, full difference detail, and matched-state confirmation from the Workbench without manually opening the artifact directory. Slice 125 adds safe artifact links through `/api/artifacts/file`, restricted to `parity-tests/artifacts/`, `legacy-openemr/artifacts/`, `modernized-openemr/artifacts/`, and `modernization-workbench/artifacts/`, so run and comparison JSON evidence can be opened directly from drill-ins while non-artifact paths are rejected.

## Modernized Target Parity Path

The modernized target now exists and currently includes thirty-eight read-only slices plus ninety-eight mutation-capable slices through Slice 138. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, insurance coverage, immunization history, scheduling, encounter, revenue-cycle, document, administration, access-control, reporting, and clinical-list workflows, and perform safe cleanup-backed mutation lifecycles for patient, scheduling, encounter, document, revenue-cycle, procedure, administration, and ACL workflow slices.

The slice readiness plans from `slice-1-readiness` through `slice-138-procedure-report-review-queue-lab-filters-readiness` prove the same normalized database facts, browser-visible behavior, mutation post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. The newest procedure report review queue lab filter plan creates temporary lab providers, a temporary encounter, lab order, and received report for `MOD-PAT-0009`, verifies lab-filtered unreviewed queue inclusion, verifies outside-lab exclusion, signs the report as `admin`, verifies lab-filtered reviewed queue membership, and hard-deletes the temporary lab-provider, order tree, and encounter rows. Slice 137 procedure report review queue provider filters remain available for provider-filtered unreviewed inclusion, outside-provider exclusion, filtered reviewed membership, and cleanup parity. Slice 136 procedure report review queue filters remain available for patient/date-filtered unreviewed inclusion, outside-date exclusion, filtered reviewed membership, and cleanup parity. Slice 135 procedure report review queue readiness remains available for unreviewed/reviewed queue membership, queue counts, reviewer transition, and cleanup parity. Slice 134 procedure report sign-off remains available for reviewed status, reviewer/timestamp, report/result visibility, and cleanup parity. Slice 133 procedure report correction remains available for collected date, report date, specimen number, report status, review status, notes, and linked result parity. Slice 132 procedure order correction remains available for order date, procedure code/name/type, diagnosis, priority, status, instructions, and post-correction result parity. Slice 131 procedure specimen detail remains available for order-level specimen parity. Slice 130 procedure specimen metadata remains available for report collected-date and specimen-number parity. Slice 129 procedure result correction remains available for in-place corrected lab results. Slice 128 patient binary replacement remains available for patient-scoped PDF byte replacement. Slice 127 encounter binary replacement remains available for encounter-attached PDF byte replacement. Slice 126 encounter scanned attachment readiness remains available for scan-source/OCR metadata parity, Slice 123 encounter document replacement revision remains available for temporary current-revision mutation parity, Slice 122 encounter document revision remains available for seeded `DOC-MOD-PAT-0001-1` and `DOC-MOD-PAT-0001-2` current-version metadata, and Slice 121 encounter co-signature plus Slice 120 appointment reminders remain available. Earlier appointment plans remain stable for create/cancel/delete, reschedule/update, arrival/check-in, check-out, no-show, category, pending-status, provider reassignment, facility reassignment, billing-location reassignment, comments behavior, recurrence metadata behavior, recurring-series read expansion, recurrence-exception read expansion, occurrence-cancel mutation, occurrence-restore mutation, occurrence-reschedule mutation, recurrence exception-list editing, recurring root title/time propagation, recurring root metadata propagation, monthly recurrence, recurrence unit matrix behavior, days-of-week recurrence behavior, monthly repeat-on recurrence behavior, seeded series recurrence update behavior, provider-overlap tolerance behavior, patient-overlap tolerance behavior, and room-overlap tolerance behavior.

For the one-hundred-thirty-eighth modernized procedure report review queue lab filter readiness slice, compare the current side-by-side readiness plan with:

```powershell
npm run compare -- --left-target legacy-openemr --right-target modernized-openemr --plan slice-138-procedure-report-review-queue-lab-filters-readiness
```
