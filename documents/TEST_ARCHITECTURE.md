...
Slice 216 adds patient portal secure-message batch-archive readiness with the `workflow-patient-portal-batch-archive` suite and `slice-216-patient-portal-batch-archive-readiness` plan. The suite creates cleanup-backed inbound portal messages, archives selected rows together through the legacy portal Actions menu and the modernized `POST /api/patient-portal/messages/archive` endpoint, verifies `Delete` status and active-folder removal, and keeps the per-test reset contract used by the prior portal-message mutation slices.
...
Slice 216 extends these allow-lists with `workflow-patient-portal-batch-archive` and `slice-216-patient-portal-batch-archive-readiness`.
