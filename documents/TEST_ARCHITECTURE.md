...
The modernized target now exists and includes implemented workflow slices through Slice 525. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 525 extends the billing surface by verifying deterministic patient-payment receipt PDF export from the modernized Fees payment row.

The slice readiness plans from `slice-1-readiness` through `slice-525-patient-payment-receipt-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 525 verifies patient-payment receipt readiness by creating a cleanup-backed patient payment, deriving the deterministic receipt number from patient/date/sequence, inspecting the modernized receipt PDF bytes and headers, confirming the Fees payment row exposes a `Receipt` action, and deleting the temporary payment after comparison. Slice 524 verifies charge-template readiness by loading the modernized Fees page, replacing ad hoc CPT draft values through the `Office visit` template, saving the generated `CPT4 99213` fee-sheet line, and deleting the temporary billing line after comparison. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
