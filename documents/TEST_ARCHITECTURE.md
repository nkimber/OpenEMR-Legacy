...
The modernized target now exists and includes implemented workflow slices through Slice 526. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 526 extends the billing surface by verifying focused patient-payment refund reversal from the modernized Fees payment-posting workflow.

The slice readiness plans from `slice-1-readiness` through `slice-526-patient-payment-refund-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 526 verifies patient-payment refund readiness by creating a cleanup-backed negative `patient_refund` posting, proving payment-session/activity increments, net payment reduction, balance increase, positive refund ledger impact, modernized Fees refund rendering, and cleanup. Slice 525 verifies patient-payment receipt readiness by creating a cleanup-backed patient payment, deriving the deterministic receipt number from patient/date/sequence, inspecting the modernized receipt PDF bytes and headers, confirming the Fees payment row exposes a `Receipt` action, and deleting the temporary payment after comparison. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
