...
Slice 217 adds patient portal secure-message All-folder readiness with the `workflow-patient-portal-all-messages` suite and `slice-217-patient-portal-all-messages-readiness` plan. The suite creates cleanup-backed inbound and sent portal messages, verifies active owner-scoped All-folder inclusion through the legacy portal All tab and modernized Portal `All` folder, archives an inbound row, verifies archived-row exclusion, and keeps the per-test reset contract used by the prior portal-message mutation slices. Slice 216 remains available for selected-message batch archive readiness with the `workflow-patient-portal-batch-archive` suite and `slice-216-patient-portal-batch-archive-readiness` plan.
...
Slice 217 extends these allow-lists with `workflow-patient-portal-all-messages` and `slice-217-patient-portal-all-messages-readiness`; Slice 216 remains available with `workflow-patient-portal-batch-archive` and `slice-216-patient-portal-batch-archive-readiness`.
