...
The modernized target now exists and includes implemented workflow slices through Slice 548. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 548 extends the billing validation surface by verifying incompatible modifier-combination claim scrubbing for CPT4 lines that combine supported modifiers `25` and `59`.

The slice readiness plans from `slice-1-readiness` through `slice-568-patient-payment-server-side-posting-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 568 verifies focused patient-payment server-side posting readiness by creating a cleanup-backed positive `patient_payment` posting against the seeded `MOD-PAT-0005` billing encounter `1000052`, driving the modernized Fees `Patient` payment source through the backend `POST /api/billing/payments/patient-payments` operation or equivalent legacy patient-payment workflow, proving patient payer identity, positive payment amount, zero adjustment, blank reason/account codes, blank payer claim number, account balance and ledger movement, void rollback, modernized rendering, and cleanup while moving patient payment capture rules out of the React UI. Slice 567 verifies focused insurance-payment server-side posting readiness by creating a cleanup-backed positive `insurance_payment` posting against the seeded `MOD-PAT-0005` billing encounter `1000052` and Northstar HMO payer, driving the modernized Fees `Insurance` payment source through the backend `POST /api/billing/payments/insurance-payments` operation or equivalent legacy payment workflow, proving insurer payer identity, positive payment amount, non-negative adjustment amount, reason/account-code handling, payer claim number, account balance and ledger movement, void rollback, modernized rendering, and cleanup while moving insurance payment posting rules out of the React UI. Slice 566 verifies focused adjustment-reversal server-side posting readiness by creating a cleanup-backed negative `adjustment_reversal` posting against the seeded `MOD-PAT-0005` billing encounter `1000052` and Northstar HMO payer, driving the modernized Fees `Adjustment reversal` mode through the backend `POST /api/billing/payments/adjustment-reversals` operation or equivalent legacy adjustment reversal workflow, proving insurer payer identity, zero payment amount, negative adjustment reversal amount, payer claim number, account balance and ledger movement, modernized rendering, and cleanup while moving adjustment reversal posting rules out of the React UI. Slice 565 verifies focused insurance-reversal server-side posting readiness by creating a cleanup-backed negative `insurance_reversal` posting against the seeded `MOD-PAT-0005` billing encounter `1000052` and Northstar HMO payer, driving the modernized Fees `Insurance reversal` mode through the backend `POST /api/billing/payments/insurance-reversals` operation or equivalent legacy reversal workflow, proving insurer payer identity, negative reversal amount, zero adjustment, payer claim number, account balance and ledger movement, modernized rendering, and cleanup while moving insurance reversal posting rules out of the React UI. Slice 564 verifies focused patient-refund server-side posting readiness by creating a cleanup-backed negative `patient_refund` posting against the seeded `MOD-PAT-0005` billing encounter `1000052`, driving the modernized Fees `Patient refund` mode through the backend `POST /api/billing/payments/patient-refunds` operation or equivalent legacy refund workflow, proving patient payer identity, negative refund amount, zero adjustment, account balance and ledger movement, modernized rendering, and cleanup while moving patient-refund posting rules out of the React UI. Slice 563 verifies focused server-side charge-template catalog readiness by applying Office visit and Complex follow-up fee-sheet templates against the seeded `MOD-PAT-0001` billing encounter, driving the modernized Fees template buttons through the backend `GET /api/billing/charge-templates/{templateId}` operation or equivalent legacy CPT-line workflow, proving CPT, modifier, fee, units, justification, saved-line payloads, modernized rendering, count movement, and cleanup while moving charge-template catalog business rules out of the React UI. Slice 562 verifies focused server-side EOB batch import readiness by importing a cleanup-backed two-line Northstar HMO remittance batch against the seeded billing encounter `1000052`, driving the modernized Fees `Import EOB` action through the backend `POST /api/billing/eob-batches/import` operation or equivalent legacy payment-posting workflow, proving payer-claim numbers, reason codes, account balance and ledger movement, modernized rendering, and cleanup while moving EOB batch import business rules out of the React UI. Slice 561 verifies focused server-side claim adjudication readiness by creating a cleanup-backed generated Northstar HMO claim against the seeded billing encounter `1000052`, driving the modernized Fees `Adjudicate` action through the backend `POST /api/billing/claims/{claimId}/adjudicate` operation or equivalent legacy payment-plus-claim update, proving linked EOB payment and contractual adjustment rows, payer-claim number, X12 835 process-file metadata, account balance and ledger movement, modernized rendering, and cleanup while moving adjudication business rules out of the React UI. Slice 560 verifies focused server-side claim clearing readiness by creating a cleanup-backed denied Northstar HMO claim against the seeded billing encounter `1000052`, driving the modernized Fees `Clear` action through the backend `POST /api/billing/claims/{claimId}/clear` operation or equivalent legacy update, proving marked-as-cleared status, HCFA target metadata, blank process-file state, submitted-claim fallback content, modernized rendering, claim-count stability, and cleanup while moving clearing business rules out of the React UI. Slice 559 verifies focused server-side claim denial readiness by creating a cleanup-backed queued Northstar HMO claim against the seeded billing encounter `1000052`, driving the modernized Fees `Deny` action through the backend `POST /api/billing/claims/{claimId}/deny` operation or equivalent legacy update, proving denied status, X12 target metadata, deterministic process-file and denial payload fallback content, modernized rendering, claim-count stability, and cleanup while moving denial business rules out of the React UI. Slice 558 verifies focused server-side claim resubmission readiness by creating a cleanup-backed denied Northstar HMO claim against the seeded billing encounter `1000052`, driving the modernized Fees `Resubmit` action through the backend `POST /api/billing/claims/{claimId}/resubmit` operation or equivalent legacy update, proving queued status, X12 target metadata, deterministic process-file and resubmission payload content, modernized rendering, claim-count stability, and cleanup while moving resubmission business rules out of the React UI. Slice 557 verifies focused server-side claim generation readiness by creating a cleanup-backed queued Northstar HMO claim against the seeded billing encounter `1000052`, driving the modernized Fees `Generate` action through the backend `POST /api/billing/claims/{claimId}/generate` operation or equivalent legacy update, proving deterministic 837P-style submitted-claim payload content, X12 target metadata, process-file metadata, modernized rendering, claim-count stability, and cleanup while moving generation business rules out of the React UI. Slice 556 verifies focused server-side claim scrubbing readiness by creating a cleanup-backed queued Northstar HMO claim against the seeded billing encounter `1000052`, driving the modernized Fees `Scrub` action through the backend `POST /api/billing/claims/{claimId}/scrub` operation or equivalent legacy update, proving deterministic `SCRUB-PASS` / `issues=none` report content, process-file metadata, modernized rendering, claim-count stability, and cleanup while moving scrub business rules out of the React UI. Slice 555 verifies focused inactive-diagnosis-line readiness by creating a cleanup-backed same-day encounter with ICD10 diagnosis `K21.9`, marking that diagnosis row billed/inactive before scrubbing, creating a valid active CPT4 `99214` line carrying supported modifier `25`, positive fee and units, and diagnosis pointer `K21.9`, and queuing a Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `missing-diagnosis-code` report content from the active-line projection without payer, CPT-line, fee, unit, CPT-code, diagnosis-pointer, modifier, future-date, or invalid-pointer misclassification, inactive-row status evidence, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 554 verifies focused inactive-CPT-line readiness by creating a cleanup-backed same-day encounter with valid ICD10 diagnosis `K21.9`, a valid CPT4 `99214` line carrying supported modifier `25`, positive fee and units, marking that CPT row billed/inactive before scrubbing, and queuing a Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `missing-cpt-line` report content from the active-line projection without payer, fee, unit, CPT-code, diagnosis-code, diagnosis-pointer, modifier, future-date, or missing-code misclassification, inactive-row status evidence, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 553 verifies focused missing-diagnosis-pointer readiness by creating a cleanup-backed same-day encounter with valid ICD10 diagnosis `K21.9`, a valid CPT4 `99214` line carrying supported modifier `25`, positive fee and units, a blank CPT diagnosis pointer, and a queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `missing-diagnosis-pointer` report content without payer, CPT-line, fee, unit, CPT-code, diagnosis-code, invalid-pointer, modifier, future-date, or missing-code misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 552 verifies focused missing-CPT-line readiness by creating a cleanup-backed same-day encounter with valid ICD10 diagnosis `K21.9`, no CPT4 service line, and a queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `missing-cpt-line` report content without payer, fee, unit, CPT-code, diagnosis-code, diagnosis-pointer, modifier, future-date, or missing-code misclassification, process-file metadata, modernized rendering, claim and single diagnosis-line count stability, and cleanup. Slice 551 verifies focused invalid-units readiness by creating a cleanup-backed same-day encounter with valid ICD10 diagnosis `K21.9`, a valid CPT4 `99214` line carrying supported modifier `25`, positive fee, zero CPT units, and a queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `invalid-units` report content without payer, fee, CPT-code, diagnosis-code, diagnosis-pointer, modifier, future-date, or missing-code misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 550 verifies focused invalid-fee readiness by creating a cleanup-backed same-day encounter with valid ICD10 diagnosis `K21.9`, a valid CPT4 `99214` line carrying supported modifier `25`, zero CPT fee, positive units, and a queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `invalid-fee` report content without payer, CPT-code, diagnosis-code, diagnosis-pointer, modifier, unit, future-date, or missing-code misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 549 verifies focused missing-payer readiness by creating a cleanup-backed same-day encounter with valid ICD10 diagnosis `K21.9`, a valid CPT4 `99214` line carrying supported modifier `25`, positive fee and units, and a queued claim with no payer selected, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `missing-payer` report content without CPT-code, diagnosis-code, diagnosis-pointer, modifier, fee, unit, future-date, or missing-code misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 548 verifies focused incompatible modifier-combination readiness by creating a cleanup-backed same-day encounter with valid ICD10 diagnosis `K21.9`, a valid CPT4 `99214` line carrying supported-but-incompatible modifier text `25,59`, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `incompatible-modifier-combination` report content without invalid-modifier, duplicate-modifier, modifier-count, CPT-code, diagnosis-pointer, or missing-code misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 547 verifies focused future service-date readiness by creating a cleanup-backed future-dated encounter with valid ICD10 diagnosis `K21.9`, a future-dated valid CPT4 `99214` line, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `future-service-date` report content without CPT-code, diagnosis-code, diagnosis-pointer, or missing-code misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 546 verifies focused invalid ICD10 diagnosis-code readiness by creating a cleanup-backed encounter with malformed ICD10 diagnosis code `BADCODE`, a valid CPT4 `99214` line justified by that same diagnosis, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `invalid-diagnosis-code` report content without diagnosis-pointer, missing-code, or CPT-code misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 545 verifies focused invalid CPT-code readiness by creating a cleanup-backed encounter with one active ICD10 diagnosis line, a malformed CPT4 code `99A14`, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `invalid-cpt-code` report content without diagnosis-pointer or missing-code misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 544 verifies focused duplicate diagnosis-code readiness by creating a cleanup-backed encounter with two active ICD10 `K21.9` diagnosis lines, a CPT line justified by `K21.9`, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `duplicate-diagnosis-code` report content without invalid-pointer, missing-code, or duplicate-pointer misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 543 verifies focused duplicate diagnosis-pointer readiness by creating a cleanup-backed encounter with one ICD10 diagnosis line, a CPT line justified by duplicate comma-separated diagnosis pointers, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `duplicate-diagnosis-pointer` report content without invalid-pointer, missing-code, or pointer-count misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 542 verifies focused claim diagnosis-pointer-count readiness by creating a cleanup-backed encounter with five ICD10 diagnosis lines, a CPT line justified by five comma-separated diagnosis pointers, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `diagnosis-pointer-count-exceeded` report content without invalid-pointer or missing-code misclassification, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 541 verifies focused claim multi-diagnosis-pointer readiness by creating a cleanup-backed encounter with two ICD10 diagnosis lines, a comma-separated multi-diagnosis CPT line, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-PASS` report content, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 540 verifies focused claim missing-diagnosis-code readiness by creating a cleanup-backed CPT line and queued Northstar HMO claim on a temporary encounter with no ICD10 diagnosis row, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `missing-diagnosis-code` report content, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 539 verifies focused claim modifier-count readiness by creating a cleanup-backed ICD10 diagnosis line, five-modifier CPT line, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `modifier-count-exceeded` report content, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 538 verifies focused claim modifier-compatibility readiness by creating a cleanup-backed ICD10 diagnosis line, duplicate-modifier CPT line, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `duplicate-modifier` report content, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 537 verifies focused claim diagnosis-pointer scrubbing readiness by creating a cleanup-backed ICD10 diagnosis line, unsupported-pointer CPT line, and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `invalid-diagnosis-pointer` report content, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 536 verifies focused claim modifier-scrubbing readiness by creating a cleanup-backed unsupported-modifier CPT line and queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic `SCRUB-FAIL` / `invalid-modifier` report content, process-file metadata, modernized rendering, claim and billing-line count stability, and cleanup. Slice 535 verifies expanded charge-template catalog readiness by applying the modernized Fees `Complex follow-up` template or equivalent legacy create, proving CPT `99214`, modifier `25`, two units, fee, diagnosis pointer, modernized rendering, billing-line count movement, and cleanup. Slice 534 verifies focused claim-resubmission readiness by creating a cleanup-backed denied Northstar HMO claim, driving the modernized Fees `Resubmit` action or equivalent legacy update, proving queued status, X12 target, deterministic process-file and resubmission payload content, modernized rendering, claim-count stability, and cleanup. Slice 533 verifies focused claim-scrubbing readiness by creating a cleanup-backed queued Northstar HMO claim, driving the modernized Fees `Scrub` action or equivalent legacy update, proving deterministic payer/CPT/diagnosis-pointer/fee/unit scrub report content, process-file metadata, modernized rendering, claim-count stability, and cleanup. Slice 532 verifies focused claim-generation readiness by creating a cleanup-backed queued Northstar HMO claim, driving the modernized Fees `Generate` action or equivalent legacy update, proving deterministic 837P-style submitted-claim payload content, X12 target, process-file metadata, modernized rendering, claim-count stability, and cleanup. Slice 531 verifies EOB batch import readiness by importing a cleanup-backed two-line Northstar HMO remittance batch, proving electronic payment method, payer identity, payer-claim numbers, reason codes, payment/adjustment balance and ledger effects, modernized Fees `Import EOB` action/rendering, and cleanup. Slice 530 verifies claim-adjudication readiness by creating a cleanup-backed generated Northstar HMO claim, posting a linked EOB payment plus contractual adjustment, marking the claim cleared with X12 835 metadata, proving payer identity, payer-claim number, reason-code, balance and ledger effects, modernized Fees `Adjudicate` action/rendering, and cleanup. Slice 529 verifies claim-denial readiness by creating a cleanup-backed queued Northstar HMO claim, updating it to denied status with X12 denial response metadata, proving payer identity, denial file/payload facts, claim-count stability, modernized Fees denial rendering, and cleanup. Slice 528 verifies adjustment-reversal readiness by creating a cleanup-backed negative `adjustment_reversal` posting for Northstar HMO, proving payer identity, payment-session/activity increments, net adjustment reduction, balance increase, positive reversal ledger impact, modernized Fees reversal rendering, and cleanup. Slice 527 verifies insurance-payment reversal readiness by creating a cleanup-backed negative `insurance_reversal` posting for Northstar HMO, proving payer identity, payment-session/activity increments, net payment reduction, balance increase, positive reversal ledger impact, modernized Fees reversal rendering, and cleanup. Slice 526 verifies patient-payment refund readiness by creating a cleanup-backed negative `patient_refund` posting, proving payment-session/activity increments, net payment reduction, balance increase, positive refund ledger impact, modernized Fees refund rendering, and cleanup. Slice 525 verifies patient-payment receipt readiness by creating a cleanup-backed patient payment, deriving the deterministic receipt number from patient/date/sequence, inspecting the modernized receipt PDF bytes and headers, confirming the Fees payment row exposes a `Receipt` action, and deleting the temporary payment after comparison. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
