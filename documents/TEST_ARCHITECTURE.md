...
The modernized target now exists and includes implemented workflow slices through Slice 241. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, duplicate detection, registration validation, deceased status, guardian contact, guardian details, social details, employer details, provider assignment, care-team membership, patient history/lifestyle, insurance coverage and subscriber details, patient portal account/reset/access/authentication/session/home/clinical-summary/lab-results/medical-report/generated-report/appointment/request/secure-message/document readiness, immunization history, scheduling, encounter, revenue-cycle, procedure lab-provider ownership, directory, lifecycle, configuration, address-book, order-catalog, and clinical order queue views, document, administration, access-control, reporting, and clinical-list workflows, create/validate/end an admin session, record login audit evidence, enforce protected login-audit access, enforce protected administration directory access, enforce ACL-backed administration authorization, enforce protected operational reports and CSV export access, enforce ACL-backed operational reports authorization, enforce protected patient search/chart access, enforce ACL-backed patient chart authorization, enforce patient duplicate detection, enforce patient registration validation row safety, enforce protected clinical-list access, enforce ACL-backed clinical-list authorization, enforce protected appointment search access, enforce ACL-backed appointment authorization, enforce view/write appointment mutation authorization, enforce protected encounter search access, enforce ACL-backed encounter authorization, enforce encounter amendment history projection, enforce protected patient document access, enforce protected patient message access, enforce ACL-backed patient message authorization, enforce protected billing access, enforce ACL-backed billing authorization, enforce protected procedure access, enforce ACL-backed procedure authorization, enforce add/write/sign procedure mutation authorization, enforce add/write patient mutation authorization, enforce view/write encounter mutation authorization, enforce add/write/delete document mutation authorization, enforce add/write message mutation authorization, enforce view/write billing mutation authorization, and perform safe cleanup-backed mutation lifecycles for patient, patient portal reset, patient portal access, patient portal session logout, patient portal secure-message composition/recipient-directory/subject-presets/HTML body rendering/pagination/reply/thread view/archive/read-status/batch-archive/all-folder/lifecycle audit/encrypted-body protection/forward-to-practice/deleted-folder, patient portal document list/download, patient portal appointment list, patient portal appointment request, patient portal appointment request options, patient portal clinical summary, patient portal lab results, patient portal medical report, patient portal generated report, patient portal generated report PDF export, patient portal generated report issue selection, patient portal generated report encounter-form selection, patient portal generated report procedure-order selection, patient portal generated report procedure-order artifact delivery, patient portal generated report printable template metadata, patient portal generated report package delivery, patient portal generated report lifecycle audit, patient deceased status, patient guardian contact, patient guardian details, patient social details, patient employer details, patient provider assignment, patient care-team membership, patient contact-backed care-team membership, patient insurance subscriber coverage, patient-message reply/content/assignment, scheduling, encounter, document, revenue-cycle, procedure lab-provider lifecycle/configuration/address-book, procedure order catalog lifecycle/import, procedure order queue/transmit, procedure report bulk sign-off/reopen review, administration, and ACL workflow slices.

The slice readiness plans from `slice-1-readiness` through `slice-241-patient-portal-message-pagination-readiness` prove the same normalized database facts, browser-visible behavior, mutation/session/audit/encrypted-body/forwarding/deleted-folder/selected-procedure-order, selected-artifact, recipient-routing, subject-preset, sanitized HTML body, and folder-pagination post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. Slice 241 verifies patient portal secure-message pagination readiness on both targets; the legacy side checks OpenEMR's 20-message page size and next-page button in the portal message list, while the modernized side renders bounded Inbox/Sent/All/Deleted pagers with the same page size and proves the second-page inbox message becomes visible only after paging forward. Slice 240 verifies patient portal secure-message HTML body rendering readiness on both targets; the legacy side checks OpenEMR's DOMPurify-rendered message detail body, while the modernized side renders message cards and thread bodies through DOMPurify with the same HTML profile and `a`/`img` stripping policy while preserving raw message bodies at the workflow boundary. Slice 239 verifies patient portal secure-message subject preset readiness on both targets; the legacy side checks OpenEMR's compose `#title` input and `#listid` datalist values, while the modernized side calls `GET /api/patient-portal/messages/compose-options`, renders the same datalist-backed subject input in the Portal secure-message compose form, and preserves free-text subject entry. Slice 238 verifies patient portal secure-message recipient-directory readiness on both targets; the legacy side normalizes OpenEMR's portal `authrecips` lookup from active `users.portal_user` rows with admin fallback and checks the legacy compose select, while the modernized side calls `GET /api/patient-portal/messages/recipients`, validates compose routing against that directory, and renders the same selected recipient in the Portal secure-message compose selector. Slice 237 verifies patient portal generated medical-report procedure-order artifact readiness on both targets; the legacy side posts selected `procedures[]` IDs to printable/PDF `portal/report/portal_custom_report.php` source artifact requests, while the modernized side sends selected procedure-order IDs to `POST /api/patient-portal/medical-report/pdf` and `/package`, inspects PDF, package manifest, summary, and packaged PDF contents, and renders selected-report download controls in the Portal generated report panel. Slice 236 verifies patient portal generated medical-report procedure-order selection readiness on both targets; the legacy side posts selected `procedures[]` IDs to `portal/report/portal_custom_report.php`, while the modernized side sends selected procedure-order IDs to `POST /api/patient-portal/medical-report/generate` and renders the selected procedure-order evidence in the Portal generated report panel. Slice 235 verifies patient portal secure-message Deleted-folder readiness on both targets; the legacy side uses `onsite_mail.deleted = 1` plus the portal Archive tab, while the modernized side exposes `deletedMessages` from `GET /api/patient-portal/messages` and renders the Portal Deleted secure-message section. Slice 234 verifies patient portal secure-message forward-to-practice readiness on both targets; the legacy side updates `onsite_mail` and creates a pnotes patient message, while the modernized side uses `POST /api/patient-portal/messages/{messageId}/forward`, `messages.portal_relation = 'portal:forwarded'`, and Portal `Forward to practice` rendering. Slice 233 verifies patient portal secure-message encrypted-body readiness on both targets; the legacy side normalizes encrypted `onsite_mail` rows to the protected placeholder, while the modernized side protects encrypted portal message bodies at the API boundary and renders encrypted-message status plus the placeholder in the Portal secure-message area. Slice 232 verifies patient portal secure-message lifecycle audit readiness on both targets; the legacy side normalizes secure-message source actions, while the modernized side records and exposes compose, reply, read, single-message archive, and selected-message archive rows through `patient_portal_message_audit_events`, `GET /api/patient-portal/messages/audit`, and Portal Message Audit rendering. Slice 231 verifies patient portal generated medical-report lifecycle audit readiness on both targets; the legacy side normalizes generated-report source actions from `portal/report/portal_custom_report.php`, while the modernized side records and exposes generated-report, PDF-download, and package-download rows through `patient_portal_report_audit_events`, `GET /api/patient-portal/medical-report/audit`, and Portal Report Audit rendering. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
