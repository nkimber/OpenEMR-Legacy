...
For the eighty-sixth modernized encounter document lifecycle timeline readiness slice, compare the current side-by-side readiness plan with:

```powershell
npm run compare -- --left-target legacy-openemr --right-target modernized-openemr --plan slice-86-encounter-document-lifecycle-readiness
```
