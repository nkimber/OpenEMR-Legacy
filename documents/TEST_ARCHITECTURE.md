...
- Slice 598 procedure result version-history plan for side-by-side procedure result correction source facts, modernized current-plus-prior result-history API/UI evidence, legacy overwrite-baseline evidence, and cleanup of the temporary lab workflow.
- Slice 599 procedure report reviewer-assignment plan for side-by-side received report source facts, assigned reviewer/timestamp queue metadata, modernized Procedures card rendering, and cleanup of the temporary lab workflow.
- Slice 600 appointment waitlist deferral plan for side-by-side portal appointment-request source facts, deferred Patient Reminders status/body evidence, Calendar waitlist rendering, and cleanup of the temporary request plus provider reminder.
- Slice 601 appointment reminder retry plan for side-by-side due-reminder source facts, deterministic retry audit metadata, Calendar retry rendering, and dispatch-history verification.
- Slice 602 patient portal reminder preferences plan for side-by-side Portal profile SMS/email consent submission, staff profile-review acceptance, contact permission persistence, reminder channel derivation from `SMS + Email` to `Email`, modernized Portal/Calendar rendering, and cleanup of the temporary due appointment.
- Slice 603 appointment reminder template administration plan for side-by-side due-reminder source facts, protected template-catalog evidence, selected compatible template dispatch, incompatible template rejection, and modernized Calendar selected-template rendering.
...
The modernized target now exists and includes implemented workflow slices through Slice 603. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 603 extends scheduling operations with appointment reminder template administration.

The slice readiness plans from `slice-1-readiness` through `slice-603-appointment-reminder-template-administration-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 603 verifies appointment reminder template administration by using the existing `MOD-PAT-0191` due reminder anchor, proving the protected catalog contains default and alternate compatible templates, proving incompatible template selection returns `400`, dispatching the selected arrival-instructions template, and checking modernized Calendar selected-template rendering. Slice 602 verifies patient-facing reminder preference readiness by creating a cleanup-backed due appointment for `MOD-PAT-0004`, proving reminder derivation starts as `SMS + Email`, submitting and accepting a Portal profile change that disables SMS while keeping email enabled, proving contact permission persistence, proving the reminder channel changes to `Email`, checking modernized Portal/Calendar rendering, and deleting/restoring all temporary data. Slice 601 verifies appointment reminder retry readiness by dispatching the deterministic local reminder for `MOD-PAT-0191`, retrying it from the modernized Calendar workflow, proving the retry audit event references the previous dispatch with retry attempt metadata, and comparing the source expectation with the legacy reminder facts. Slice 600 verifies appointment waitlist deferral readiness by creating a cleanup-backed portal appointment request, proving the appointment starts and remains in waiting status (`^`), deferring the linked `Patient Reminders` row to `Deferred` with staff follow-up body evidence, proving modernized Calendar waitlist rendering, and deleting the temporary appointment and reminder. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
