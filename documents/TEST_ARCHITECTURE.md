...
Slice 219 adds patient portal appointment list readiness with the `workflow-patient-portal-appointments` suite and `slice-219-patient-portal-appointments-readiness` plan. The suite signs in as `MOD-PAT-0004`, compares the signed-in patient's upcoming and past appointment lists against the legacy portal home appointment card behavior, verifies modernized Portal rendering, and keeps the per-test reset contract used by the prior patient-portal slices. Slice 218 remains available for patient portal document list/download readiness with the `workflow-patient-portal-documents` suite and `slice-218-patient-portal-documents-readiness` plan.
...
Slice 219 extends these allow-lists with `workflow-patient-portal-appointments` and `slice-219-patient-portal-appointments-readiness`; Slice 218 remains available with `workflow-patient-portal-documents` and `slice-218-patient-portal-documents-readiness`.
