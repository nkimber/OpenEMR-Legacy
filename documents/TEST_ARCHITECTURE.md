...
Slice 220 adds patient portal appointment-request readiness with the `workflow-patient-portal-appointment-request` suite and `slice-220-patient-portal-appointment-request-readiness` plan. The suite signs in as `MOD-PAT-0004`, creates a pending `^` portal appointment request plus provider reminder note, verifies database/API/UI behavior on both targets, and cleans up the temporary appointment and reminder rows. Slice 219 remains available for patient portal appointment-list readiness with the `workflow-patient-portal-appointments` suite and `slice-219-patient-portal-appointments-readiness` plan, and Slice 218 remains available for patient portal document list/download readiness with the `workflow-patient-portal-documents` suite and `slice-218-patient-portal-documents-readiness` plan.
...
Slice 220 extends these allow-lists with `workflow-patient-portal-appointment-request` and `slice-220-patient-portal-appointment-request-readiness`; Slice 219 remains available with `workflow-patient-portal-appointments` and `slice-219-patient-portal-appointments-readiness`; Slice 218 remains available with `workflow-patient-portal-documents` and `slice-218-patient-portal-documents-readiness`.
