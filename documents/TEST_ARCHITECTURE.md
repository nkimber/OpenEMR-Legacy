...
The modernized target now exists and includes implemented workflow slices through Slice 579. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 579 extends the prescribing surface by authorizing an additional refill on an active prescription while preserving active/end-date state and rendering the updated refill evidence in Lists.

The slice readiness plans from `slice-1-readiness` through `slice-579-prescription-refill-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 579 verifies prescription refill authorization readiness by creating a cleanup-backed active prescription for `MOD-PAT-0008`, adding one refill without ending the prescription, proving refill count, modified-date, note, and active/end-date state, and proving the modernized Lists `Refill` action renders the updated refill evidence. Slice 578 verifies medication duplicate detection readiness by creating two cleanup-backed active medication-list rows for `MOD-PAT-0006`, grouping them by normalized medication title, proving the modernized clinical-list API exposes duplicate count/source IDs/date range/diagnosis evidence, and proving the modernized Lists page renders the duplicate summary. Slice 577 verifies appointment reminder dispatch readiness by deriving the due reminder expectation from the `MOD-PAT-0191` preventive-care appointment, proving `POST /api/appointments/{appointmentId}/reminders/dispatch` returns deterministic queue/audit/template/external-reference evidence, proving `GET /api/appointments/reminders/dispatch-history` returns the saved audit row, and proving the modernized Calendar `Dispatch reminder` action renders dispatch and history summaries without sending external messages. Slice 576 verifies appointment waitlist readiness by creating a temporary portal appointment request, proving the waiting appointment (`status = '^'`) and linked provider reminder, proving the modernized Calendar waitlist queue renders the requested slot, reason, reminder, and priority context, promoting the same request to OpenEMR pending status (`~`), and cleaning up the temporary appointment plus reminder. Slice 575 verifies appointment availability validation readiness by creating a temporary blocker appointment, proving `POST /api/appointments/availability/validate` reports shared provider/facility bookable-window status plus active provider, patient, and room conflicts without creating a second appointment, and proving the modernized Calendar `Check availability` action renders conflict and outside-hours evidence. Slice 574 verifies statement email outbox readiness by queuing deterministic local email records for email-ready top statement batch candidates, proving `POST /api/billing/statements/batch/email-outbox`, persisted modernized `statement_email_outbox` rows, and the modernized Fees `Email Outbox` action without sending live SMTP messages. Slice 573 verifies patient portal statement delivery readiness by publishing deterministic statement PDFs for portal-enabled top batch candidates into the `Invoices` patient-document category, proving `POST /api/billing/statements/batch/portal-delivery`, the modernized Fees `Portal Delivery` action, and patient portal Documents rendering for the delivered statement. Slice 572 verifies statement dispatch history readiness by persisting deterministic local email/print queue audit events for the top statement batch candidates, reading them back through `GET /api/billing/statements/batch/dispatch-history`, and proving the modernized Fees `Dispatch History` action renders the saved dispatch IDs, audit IDs, queue names, statuses, references, and timestamps. Slice 571 verifies statement dispatch handoff readiness by comparing deterministic local email/print queue evidence for the top statement batch candidates, including dispatch audit IDs, queue names, dispatch statuses, local external references, destinations, and statement file names, while proving the modernized Fees `Dispatch Handoff` action renders the same handoff without sending external messages. Slice 570 verifies statement delivery manifest readiness by comparing queued handoff metadata for the top statement batch candidates, including destination, file name, balance, due date, delivery method, and email-ready/print-ready counts, while proving the modernized Fees `Delivery Manifest` action renders the same deterministic handoff without sending or mutating delivery state. Slice 569 verifies focused payment-create contract readiness by proving modernized payment creation is exposed through the focused patient-payment, patient-refund, insurance-payment, insurance-reversal, and adjustment-reversal operations, retiring broad `POST /api/billing/payments` creation with a 405 Method Not Allowed response even for admin sessions, and keeping billing mutation authorization checks on the focused insurance-payment endpoint. Slice 568 verifies focused patient-payment server-side posting readiness by creating a cleanup-backed positive `patient_payment` posting against the seeded `MOD-PAT-0005` billing encounter `1000052`, driving the modernized Fees `Patient` payment source through the backend `POST /api/billing/payments/patient-payments` operation or equivalent legacy patient-payment workflow, proving patient payer identity, positive payment amount, zero adjustment, blank reason/account codes, blank payer claim number, account balance and ledger movement, void rollback, modernized rendering, and cleanup while moving patient payment capture rules out of the React UI. Slice 567 verifies focused insurance-payment server-side posting readiness by creating a cleanup-backed positive `insurance_payment` posting against the seeded `MOD-PAT-0005` billing encounter `1000052` and Northstar HMO payer, driving the modernized Fees `Insurance` payment source through the backend `POST /api/billing/payments/insurance-payments` operation or equivalent legacy payment workflow, proving insurer payer identity, positive payment amount, non-negative adjustment amount, reason/account-code handling, payer claim number, account balance and ledger movement, void rollback, modernized rendering, and cleanup while moving insurance payment posting rules out of the React UI. Slice 566 through Slice 525 continue to cover the existing focused revenue-cycle mutation and readiness contracts, including reversal/refund posting, EOB import, claim adjudication, claim lifecycle actions, claim scrubbing edge cases, account rollups, statement readiness, and patient-payment receipt export. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
