...
The modernized target now exists and currently includes thirty-four read-only slices plus sixty-eight mutation-capable slices through Slice 102. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, insurance coverage, immunization history, scheduling, encounter, encounter document attachment, encounter billing linkage, encounter claim linkage, encounter procedure order linkage, encounter diagnosis coding linkage, clinical-list, messaging, completed procedure-result, pending procedure-order, billing, claim status, payment posting, account balance, account aging, account ledger, account statement readiness, patient statement generation, patient statement PDF export, statement batch candidates, statement batch package export, collections work queue, administration, access-control, reporting, report export, patient document, patient document content, patient document preview, patient document image thumbnail, patient document PDF inline preview, patient document lifecycle timeline, patient scanned attachment, patient document revision, encounter-attached document, encounter billing linkage, encounter procedure order linkage, and encounter diagnosis coding workflows, and perform safe cleanup-backed mutation lifecycles for patient contact, patient demographics, patient registration, insurance coverage, appointments, appointment reschedules, appointment arrivals, appointment check-outs, appointment no-shows, appointment categories, appointment pending statuses, appointment provider reassignments, appointment facility reassignments, appointment billing-location reassignments, appointment comment updates, encounters, encounter metadata, encounter billing linkage visibility, encounter diagnosis coding visibility, encounter fee-sheet entry, encounter procedure-order entry, encounter procedure-result entry, encounter sign-off, encounter document upload, encounter binary document upload, encounter document sign-off, encounter document denial, encounter document metadata refiling, encounter document movement, encounter document content replacement, encounter document archive/restore, encounter document lifecycle timeline rendering, encounter external-link document attachment, allergies, problem lists, medication lists, messages, message content editing, message assignment, collections follow-up tasks, text/binary/image-preview/image-thumbnail/pdf-preview/lifecycle/scanned-attachment/sign-off/denial/metadata/archive/content-replacement/replacement-revision/external-link patient documents, prescriptions, immunizations, billing lines, billing diagnosis lines, billing correction lines, billing modifier lines, claim statuses, payment postings, patient payments, procedures, facilities, users, ACL permission assignments, and ACL user group memberships.

The slice readiness plans from `slice-1-readiness` through `slice-102-appointment-comments-readiness` prove the same normalized database facts, browser-visible behavior, mutation post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. The newest appointment comments plan creates a temporary future appointment on `MOD-PAT-0003`, updates its scheduling comments, renders the comments through the modernized Calendar edit form and detail panel, verifies the legacy `form_comments` control, deletes the temporary appointment, and returns seeded appointment counts clean; the appointment billing-location reassignment plan creates a temporary future appointment on `MOD-PAT-0003`, keeps service facility `10`, updates billing location from `10` to `11`, renders the reassigned billing facility through the modernized Calendar edit form, verifies the legacy `facility` and `billing_facility` controls, deletes the temporary appointment, and returns seeded appointment counts clean; the appointment facility reassignment plan creates a temporary future appointment on `MOD-PAT-0003`, updates its facility from `10` to `11`, renders the reassigned facility through the modernized Calendar edit form, verifies the legacy `facility` control, deletes the temporary appointment, and returns seeded appointment counts clean; the appointment provider reassignment plan creates a temporary future appointment on `MOD-PAT-0003`, updates its provider from `101` to `102`, renders the reassigned provider through the modernized Calendar edit form, verifies the legacy provider control, deletes the temporary appointment, and returns seeded appointment counts clean; the appointment pending-status plan creates a temporary future appointment on `MOD-PAT-0003`, updates it to OpenEMR-compatible pending status `~`, renders the pending state through the modernized Calendar edit form, verifies the legacy `form_apptstatus` control, deletes the temporary appointment, and returns seeded appointment counts clean; the appointment category plan creates a temporary future appointment on `MOD-PAT-0003`, preserves category `13` Preventive Care Services, updates it to category `10` New Patient, renders the category state through the modernized Calendar, verifies the legacy `form_category` control, deletes the temporary appointment, and returns seeded appointment counts clean; the appointment no-show plan creates a temporary future appointment on `MOD-PAT-0003`, marks it no-show with status `?`, renders the no-show state through the modernized Calendar `Mark no-show` action, deletes the temporary appointment, and returns seeded appointment counts clean; the appointment check-out plan creates a temporary future appointment on `MOD-PAT-0003`, marks it arrived with status `@`, checks it out with status `>`, renders the checked-out state through the modernized Calendar `Mark checked out` action, deletes the temporary appointment, and returns seeded appointment counts clean; earlier plan details remain stable for workflow mutation, revenue-cycle, document, reporting, administration, clinical-list, messaging, insurance, immunization, procedure, appointment lifecycle, and patient-chart slices.
