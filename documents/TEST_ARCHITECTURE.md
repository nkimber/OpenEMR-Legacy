...
- `mutation-isolated` runs legacy workflow mutations and shared patient contact, appointment, encounter, clinical-list allergy, problem-list, medication-list, message, document, binary document, insurance, prescription, immunization, billing, procedure, admin-facility, admin-user, access-permission, and user-group-membership mutation suites with per-test resets for strongest mutation isolation.
