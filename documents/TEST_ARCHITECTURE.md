...
Slice 223 adds patient portal lab-results readiness with the `workflow-patient-portal-lab-results` suite and `slice-223-patient-portal-lab-results-readiness` plan. The suite signs in as `MOD-PAT-0004`, verifies lab order, report, and final result facts through workflow/API surfaces, then verifies the visible legacy and modernized portal lab-results surfaces. Slice 222 remains available for patient portal clinical-summary readiness with the `workflow-patient-portal-clinical-summary` suite and `slice-222-patient-portal-clinical-summary-readiness` plan, Slice 221 remains available for patient portal appointment request-options readiness with the `workflow-patient-portal-appointment-options` suite and `slice-221-patient-portal-appointment-options-readiness` plan, Slice 220 remains available for patient portal appointment-request readiness with the `workflow-patient-portal-appointment-request` suite and `slice-220-patient-portal-appointment-request-readiness` plan, Slice 219 remains available for patient portal appointment-list readiness with the `workflow-patient-portal-appointments` suite and `slice-219-patient-portal-appointments-readiness` plan, and Slice 218 remains available for patient portal document list/download readiness with the `workflow-patient-portal-documents` suite and `slice-218-patient-portal-documents-readiness` plan.
...
Slice 223 extends these allow-lists with `workflow-patient-portal-lab-results` and `slice-223-patient-portal-lab-results-readiness`; Slice 222 remains available with `workflow-patient-portal-clinical-summary` and `slice-222-patient-portal-clinical-summary-readiness`; Slice 221 remains available with `workflow-patient-portal-appointment-options` and `slice-221-patient-portal-appointment-options-readiness`; Slice 220 remains available with `workflow-patient-portal-appointment-request` and `slice-220-patient-portal-appointment-request-readiness`; Slice 219 remains available with `workflow-patient-portal-appointments` and `slice-219-patient-portal-appointments-readiness`; Slice 218 remains available with `workflow-patient-portal-documents` and `slice-218-patient-portal-documents-readiness`.
...
The modernized target now exists and includes implemented workflow slices through Slice 223. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, duplicate detection, registration validation, deceased status, guardian contact, guardian details, social details, employer details, provider assignment, care-team membership, patient history/lifestyle, insurance coverage and subscriber details, patient portal account/reset/access/authentication/session/home/clinical-summary/lab-results/appointment/request/secure-message/document readiness, immunization history, scheduling, encounter, revenue-cycle, procedure lab-provider ownership, directory, lifecycle, configuration, address-book, order-catalog, and clinical order queue views, document, administration, access-control, reporting, and clinical-list workflows, create/validate/end an admin session, record login audit evidence, enforce protected login-audit access, enforce protected administration directory access, enforce ACL-backed administration authorization, enforce protected operational reports and CSV export access, enforce ACL-backed operational reports authorization, enforce protected patient search/chart access, enforce ACL-backed patient chart authorization, enforce patient duplicate detection, enforce patient registration validation row safety, enforce protected clinical-list access, enforce ACL-backed clinical-list authorization, enforce protected appointment search access, enforce ACL-backed appointment authorization, enforce view/write appointment mutation authorization, enforce protected encounter search access, enforce ACL-backed encounter authorization, enforce encounter amendment history projection, enforce protected patient document access, enforce protected patient message access, enforce ACL-backed patient message authorization, enforce protected billing access, enforce ACL-backed billing authorization, enforce protected procedure access, enforce ACL-backed procedure authorization, enforce add/write/sign procedure mutation authorization, enforce add/write patient mutation authorization, enforce view/write encounter mutation authorization, enforce add/write/delete document mutation authorization, enforce add/write message mutation authorization, enforce view/write billing mutation authorization, and perform safe cleanup-backed mutation lifecycles for patient, patient portal reset, patient portal access, patient portal session logout, patient portal secure-message composition/reply/thread view/archive/read-status/batch-archive/all-folder, patient portal document list/download, patient portal appointment list, patient portal appointment request, patient portal appointment request options, patient portal clinical summary, patient portal lab results, patient deceased status, patient guardian contact, patient guardian details, patient social details, patient employer details, patient provider assignment, patient care-team membership, patient contact-backed care-team membership, patient insurance subscriber coverage, patient-message reply/content/assignment, scheduling, encounter, document, revenue-cycle, procedure lab-provider lifecycle/configuration/address-book, procedure order catalog lifecycle/import, procedure order queue/transmit, procedure report bulk sign-off/reopen review, administration, and ACL workflow slices.

The slice readiness plans from `slice-1-readiness` through `slice-223-patient-portal-lab-results-readiness` prove the same normalized database facts, browser-visible behavior, mutation/session/audit post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. Slice 223 verifies patient portal lab-results readiness on both targets; the legacy side reads signed-in lab order, report, and result rows through `portal/get_lab_results.php`, while the modernized side exposes the same facts through `GET /api/patient-portal/lab-results` and Portal lab-results rendering. Slice 222 verifies patient portal clinical-summary readiness on both targets; the legacy side reads signed-in patient problem, allergy, medication-list, and active-prescription pages through `portal/get_problems.php`, `portal/get_allergies.php`, `portal/get_medications.php`, and `portal/get_prescriptions.php`, while the modernized side exposes the same facts through `GET /api/patient-portal/clinical-summary` and Portal clinical-summary rendering. Slice 221 verifies patient portal appointment request-options readiness on both targets; the legacy side reads active request categories, authorized providers, provider-derived facilities, and request defaults from the same tables used by `portal/add_edit_event_user.php`, while the modernized side exposes the same behavior through `GET /api/patient-portal/appointments/request-options` and Portal dropdown rendering. Slice 220 verifies patient portal appointment-request readiness on both targets; the legacy side creates a pending portal request through the same appointment/reminder storage contract as `portal/add_edit_event_user.php`, while the modernized side exposes the same behavior through `POST /api/patient-portal/appointments/requests`, Portal request-form rendering, and cleanup-backed appointment/reminder row checks. Slice 219 verifies patient portal appointment list readiness on both targets; the legacy side reads the signed-in patient's future and past appointment cards from the portal home, while the modernized side exposes the same facts through `/api/patient-portal/appointments` and Portal upcoming/past appointment rendering. Slice 218 verifies patient portal document list/download readiness on both targets; the legacy side reads active patient-owned `documents` rows through `portal/get_patient_documents.php` and the selected-document download package, while the modernized side exposes the same facts through `/api/patient-portal/documents`, `/api/patient-portal/documents/download`, and Portal document rendering. Slice 217 verifies patient portal secure-message All-folder readiness on both targets; the legacy side reads active patient-owned `onsite_mail` rows through the portal All tab and verifies archived-row exclusion, while the modernized side exposes the same active owner-scoped view through `/api/patient-portal/messages` `allMessages` and the Portal `All` folder. Slice 216 verifies patient portal secure-message batch-archive readiness on both targets; the legacy side archives selected patient-owned `onsite_mail` rows through the portal Actions menu and verifies active-folder removal plus Archive visibility, while the modernized side exposes the same behavior through `POST /api/patient-portal/messages/archive` and Portal selected-message rendering. Slice 215 verifies patient portal secure-message read-status readiness on both targets; the legacy side updates patient-owned `onsite_mail` rows from `New` to `Read` while keeping them in the active inbox, while the modernized side exposes the same behavior through `PUT /api/patient-portal/messages/{messageId}/read` and Portal `Mark read` rendering. Slice 214 verifies patient portal secure-message archive readiness on both targets; the legacy side soft-deletes patient-owned `onsite_mail` rows with `message_status = Delete`, hides the temporary message from Sent, and surfaces it in Archive, while the modernized side exposes the same behavior through `DELETE /api/patient-portal/messages/{messageId}` and Portal `Archive message` rendering. Slice 213 verifies patient portal secure-message thread view readiness on both targets; the legacy side reads patient-visible `onsite_mail` thread rows after a temporary reply, while the modernized side exposes the same conversation through `GET /api/patient-portal/messages/{messageId}/thread` and Portal `View thread` rendering. Slice 212 verifies patient portal secure-message reply readiness on both targets; the legacy side creates paired `onsite_mail` reply rows with `reply_mail_chain` tied to the seeded inbox message and verifies the portal Sent folder, while the modernized side creates paired `portal_mailbox_messages` rows through `POST /api/patient-portal/messages/{messageId}/reply` and verifies the Portal workspace inline-reply/sent UI. Slice 211 verifies patient portal secure-message compose readiness on both targets; the legacy side creates paired `onsite_mail` rows and verifies the portal Sent folder, while the modernized side creates paired `portal_mailbox_messages` rows through `POST /api/patient-portal/messages` and verifies the Portal workspace compose/sent UI. Slice 210 verifies patient portal secure-message inbox readiness on both targets; the legacy side reads seeded `onsite_mail` rows and verifies the portal secure-messaging page after sign-in, while the modernized side exposes the same inbox through PostgreSQL-backed `/api/patient-portal/messages` and Portal workspace secure-message cards. Slice 209 verifies patient portal home readiness on both targets; the legacy side verifies browser-visible portal home entry after portal sign-in, while the modernized side exposes the same portal identity, secure-message counts, and upcoming appointment facts through PostgreSQL-backed `/api/patient-portal/home` and the first-class Portal workspace. Slice 208 verifies patient portal session readiness on both targets; the legacy side verifies browser-visible portal logout behavior, while the modernized side verifies active-session read, `DELETE /api/patient-portal/session`, `patient_portal_sessions.ended_at`, Patient/Client Portal Account end-session rendering, and inactive reuse rejection. Slice 207 verifies patient portal authentication readiness on both targets; the legacy side verifies the OpenEMR-compatible seeded bcrypt credential, enabled portal runtime globals, and portal login state from `globals` / `patient_access_onsite` / `patient_data`, while the modernized side exposes the same observable behavior through PostgreSQL `patient_portal_accounts`, `patient_portal_sessions`, `/api/patient-portal/login`, `/api/patient-portal/session`, chart summaries, and the Patient/Client Portal Account sign-in readiness form. The shared `workflow-patient-portal-authentication` suite uses `MOD-PAT-0004`, `mod-pat-0004@example.test`, and the synthetic password `PortalPass207!`; it proves valid sign-in plus invalid-password, disabled-access, and pending-reset rejection paths, then restores reset/access state. Slice 206 verifies patient portal access readiness on both targets; the legacy side mutates and restores `patient_data.allow_patient_portal`, while the modernized side exposes the same access state through PostgreSQL `patients.portal_enabled`, `/api/patients/{patientId}/portal-account/access`, chart summaries, and the Patient/Client Portal Account panel while preserving CMS login and onsite account facts. Slice 205 verifies patient portal reset readiness on both targets; the legacy side mutates and restores `patient_access_onsite.portal_pwd_status` and `portal_onetime`, while the modernized side exposes the same reset state through PostgreSQL `patient_portal_accounts`, `/api/patients/{patientId}/portal-account/reset`, chart summaries, and the Patient/Client Portal Account panel. Slice 204 verifies patient portal account readiness on both targets; the legacy side reads and renders `patient_data.cmsportal_login` plus `patient_access_onsite` username/status facts, while the modernized side exposes the same facts through PostgreSQL `patients.cms_portal_login`, `patient_portal_accounts`, chart summaries, the `portalAccount` API object, and the Patient/Client Portal Account panel. Slice 203 verifies patient insurance subscriber behavior on both targets; the legacy side reads and renders `insurance_data` subscriber identity, DOB, sex, relationship, address, phone, and employer fields, while the modernized side exposes the same facts through PostgreSQL `insurance_records`, chart summaries, the Patient/Client insurance panel, and create/update/delete API-backed mutation behavior. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
