...
The modernized target now exists and includes implemented workflow slices through Slice 604. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 604 extends encounter operations with SOAP note template application.

The slice readiness plans from `slice-1-readiness` through `slice-604-encounter-soap-template-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 604 verifies encounter SOAP template readiness by creating a cleanup-backed temporary encounter for `MOD-PAT-0002`, applying the selected diabetes follow-up SOAP template to the modernized SOAP form, saving the resulting note, comparing the saved SOAP payload with the legacy form model, and deleting the temporary rows. Slice 603 verifies appointment reminder template administration by using the existing `MOD-PAT-0191` due reminder anchor, proving the protected catalog contains default and alternate compatible templates, proving incompatible template selection returns `400`, dispatching the selected arrival-instructions template, and checking modernized Calendar selected-template rendering. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
