...
The modernized target now exists and includes implemented workflow slices through Slice 523. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 523 extends the scheduling/billing boundary by verifying appointment-to-billing starter charge conversion from a converted appointment encounter.

The slice readiness plans from `slice-1-readiness` through `slice-523-appointment-billing-conversion-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 523 verifies appointment-to-billing conversion by creating a cleanup-backed appointment, converting it into an encounter, creating a starter `CPT4 99213` fee-sheet line on that encounter, confirming the modernized Calendar changes the action to `Charge created`, and deleting the temporary billing line before deleting the encounter and appointment. Slice 522 verifies appointment-to-encounter conversion by creating a cleanup-backed appointment, converting it into an encounter, proving the encounter inherits patient, provider, date, facility, billing facility, reason, and source appointment linkage, confirming the modernized Calendar changes the action to `Encounter created`, and deleting the temporary encounter before deleting the appointment. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
