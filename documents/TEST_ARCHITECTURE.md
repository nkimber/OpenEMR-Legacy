...
The modernized target now exists and currently includes thirty-six read-only slices plus eighty-one mutation-capable slices through Slice 117. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, insurance coverage, immunization history, scheduling, encounter, revenue-cycle, document, administration, access-control, reporting, and clinical-list workflows, and perform safe cleanup-backed mutation lifecycles for patient, scheduling, encounter, document, revenue-cycle, procedure, administration, and ACL workflow slices.

The slice readiness plans from `slice-1-readiness` through `slice-117-appointment-provider-overlap-readiness` prove the same normalized database facts, browser-visible behavior, mutation post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. The newest appointment provider overlap plan creates two temporary appointments for `MOD-PAT-0003` and `MOD-PAT-0004` at `2026-12-04 09:00` with provider `102`, verifies both targets allow and render the double-booked appointments, verifies modernized Calendar detail reports one provider overlap for each appointment, and deletes both temporary appointments during cleanup. Earlier appointment plans remain stable for create/cancel/delete, reschedule/update, arrival/check-in, check-out, no-show, category, pending-status, provider reassignment, facility reassignment, billing-location reassignment, comments behavior, recurrence metadata behavior, recurring-series read expansion, recurrence-exception read expansion, occurrence-cancel mutation, occurrence-restore mutation, occurrence-reschedule mutation, recurrence exception-list editing, recurring root title/time propagation, recurring root metadata propagation, monthly recurrence, recurrence unit matrix behavior, days-of-week recurrence behavior, monthly repeat-on recurrence behavior, and seeded series recurrence update behavior.

For the one-hundred-seventeenth modernized appointment provider overlap readiness slice, compare the current side-by-side readiness plan with:

```powershell
npm run compare -- --left-target legacy-openemr --right-target modernized-openemr --plan slice-117-appointment-provider-overlap-readiness
```
