...
The modernized target now exists and includes implemented workflow slices through Slice 251. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, duplicate detection, registration validation, deceased status, guardian contact, guardian details, social details, employer details, provider assignment, care-team membership, patient history/lifestyle, insurance coverage and subscriber details, patient portal account/reset/access/authentication/session/home/profile/home immunization health-snapshot/clinical-summary allergy/problem and medication date-column/prescription modified-date/start-date/end-date filtering/lab-results/medical-report/generated-report/appointment/request/secure-message/document readiness, immunization history, scheduling, encounter, revenue-cycle, procedure lab-provider ownership, directory, lifecycle, configuration, address-book, order-catalog, and clinical order queue views, document, administration, access-control, reporting, and clinical-list workflows, create/validate/end an admin session, record login audit evidence, enforce protected login-audit access, enforce protected administration directory access, enforce ACL-backed administration authorization, enforce protected operational reports and CSV export access, enforce ACL-backed operational reports authorization, enforce protected patient search/chart access, enforce ACL-backed patient chart authorization, enforce patient duplicate detection, enforce patient registration validation row safety, enforce protected clinical-list access, enforce ACL-backed clinical-list authorization, enforce protected appointment search access, enforce ACL-backed appointment authorization, enforce view/write appointment mutation authorization, enforce protected encounter search access, enforce ACL-backed encounter authorization, enforce encounter amendment history projection, enforce protected patient document access, enforce protected patient message access, enforce ACL-backed patient message authorization, enforce protected billing access, enforce ACL-backed billing authorization, enforce protected procedure access, enforce ACL-backed procedure authorization, enforce add/write/sign procedure mutation authorization, enforce add/write patient mutation authorization, enforce view/write encounter mutation authorization, enforce add/write/delete document mutation authorization, enforce add/write message mutation authorization, enforce view/write billing mutation authorization, and perform safe cleanup-backed mutation lifecycles for patient, patient portal reset, patient portal access, patient portal session logout, patient portal profile change requests, patient portal secure-message composition/recipient-directory/subject-presets/HTML body rendering/pagination/mark-all-read/reply/thread view/archive/read-status/batch-archive/all-folder/lifecycle audit/encrypted-body protection/forward-to-practice/deleted-folder, patient portal document list/download, patient portal appointment list, patient portal appointment request, patient portal appointment request options, patient portal profile, patient portal home immunization health-snapshot, patient portal clinical-summary allergy/problem and medication date-column and prescription modified-date/start-date/end-date filtering, patient portal lab results, patient portal medical report, patient portal generated report, patient portal generated report PDF export, patient portal generated report issue selection, patient portal generated report encounter-form selection, patient portal generated report procedure-order selection, patient portal generated report procedure-order artifact delivery, patient portal generated report printable template metadata, patient portal generated report package delivery, patient portal generated report lifecycle audit, patient deceased status, patient guardian contact, patient guardian details, patient social details, patient employer details, patient provider assignment, patient care-team membership, patient contact-backed care-team membership, patient insurance subscriber coverage, patient-message reply/content/assignment, scheduling, encounter, document, revenue-cycle, procedure lab-provider lifecycle/configuration/address-book, procedure order catalog lifecycle/import, procedure order queue/transmit, procedure report bulk sign-off/reopen review, administration, and ACL workflow slices.

The slice readiness plans from `slice-1-readiness` through `slice-251-patient-portal-profile-change-request-readiness` prove the same normalized database facts, browser-visible behavior, mutation/session/audit/encrypted-body/forwarding/deleted-folder/selected-procedure-order, selected-artifact, recipient-routing, subject-preset, sanitized HTML body, folder-pagination, browser-only mark-all-read post-state, patient portal profile demographics/insurance and profile pending-review expectations, patient portal home immunization health-snapshot expectations, and patient portal allergy/problem/medication date-column plus prescription modified-date/start-date/end-date filtering expectations where applicable, cleanup, and restoration expectations against both legacy and modernized targets. Slice 251 verifies patient portal profile change request readiness on both targets; the legacy side creates and cleans `onsite_portal_activity` waiting review rows and checks `portal/get_profile.php` renders `Edit Pending Changes.`, while the modernized side persists cleanup-backed rows in `patient_portal_profile_change_requests` and renders the same pending-review state in the signed-in Portal profile card. Slice 250 verifies patient portal profile readiness on both targets; the legacy side checks OpenEMR's `portal/get_profile.php` `Profile From Medical Records` fragment with demographics and insurance, while the modernized side renders a matching `Profile From Medical Records` card in the signed-in Portal workspace. Slice 249 verifies patient portal home immunization readiness on both targets; the legacy side checks OpenEMR's portal home `Health Snapshot (Medical Lists)` / `Patient Immunization` card while still rendering a cleanup-backed entered-in-error immunization row, and the modernized side projects the same portal-home immunization rows separately from the clinician clinical-list filter and renders a matching `Patient Immunization` card. Slice 248 verifies patient portal allergy date-column readiness on both targets; the legacy side checks OpenEMR's portal allergies table exposes `Title`, `Reported Date`, `Start Date`, `End Date`, and `Referrer` while still rendering a cleanup-backed ended allergy row, and the modernized side includes ended allergy rows in the portal clinical summary and renders matching `Reported Date`, `Start Date`, `End Date`, and `Referrer` card labels. Slice 247 verifies patient portal problem date-column readiness on both targets; the legacy side checks OpenEMR's portal problems table exposes `Title`, `Reported Date`, `Start Date`, and `End Date` while still rendering a cleanup-backed ended problem-list row, and the modernized side includes ended problem-list rows in the portal clinical summary and renders matching `Reported Date`, `Start Date`, and `End Date` card labels. Slice 246 verifies patient portal medication date-column readiness on both targets; the legacy side checks OpenEMR's portal medications table exposes `Drug`, `Start Date`, `Last Modified`, and `End Date` while still rendering a cleanup-backed ended medication-list row, and the modernized side projects `medications.modified_date`, includes ended medication-list rows in the portal clinical summary, and renders matching `Last Modified` and `End Date` card labels. Slice 245 verifies patient portal prescription end-date filtering readiness on both targets; the legacy side checks OpenEMR's portal prescriptions table exposes the `End Date` column while excluding a cleanup-backed ended prescription, and the modernized side keeps ended prescriptions out of the active prescription cards while rendering active null end dates as active status. Slice 244 verifies patient portal prescription start-date readiness on both targets; the legacy side checks OpenEMR's portal prescriptions table exposes `date_added` through the `Start Date` column, while the modernized side projects portal prescription `StartDate` from `prescriptions.date_added` and renders matching `Start` timestamps in Portal clinical-summary prescription cards. Slice 243 verifies patient portal prescription modified-date readiness on both targets; the legacy side checks OpenEMR's portal prescriptions table exposes `Last Modified` values for active prescriptions, while the modernized side projects `ModifiedDate` from `prescriptions.modified_date` and renders matching `Modified` facts in Portal clinical-summary prescription cards. Slice 242 verifies patient portal secure-message mark-all-read readiness on both targets; the legacy side checks OpenEMR's Actions menu updates visible inbox rows to `Read` without persisting `onsite_mail.message_status`, while the modernized side renders `Mark all as read`, applies local read-state overrides, clears them on refresh, and leaves the mailbox status `New`. Slice 241 verifies patient portal secure-message pagination readiness on both targets; the legacy side checks OpenEMR's 20-message page size and next-page button in the portal message list, while the modernized side renders bounded Inbox/Sent/All/Deleted pagers with the same page size and proves the second-page inbox message becomes visible only after paging forward. Slice 240 verifies patient portal secure-message HTML body rendering readiness on both targets; the legacy side checks OpenEMR's DOMPurify-rendered message detail body, while the modernized side renders message cards and thread bodies through DOMPurify with the same HTML profile and `a`/`img` stripping policy while preserving raw message bodies at the workflow boundary. Slice 239 verifies patient portal secure-message subject preset readiness on both targets; the legacy side checks OpenEMR's compose `#title` input and `#listid` datalist values, while the modernized side calls `GET /api/patient-portal/messages/compose-options`, renders the same datalist-backed subject input in the Portal secure-message compose form, and preserves free-text subject entry. Slice 238 verifies patient portal secure-message recipient-directory readiness on both targets; the legacy side normalizes OpenEMR's portal `authrecips` lookup from active `users.portal_user` rows with admin fallback and checks the legacy compose select, while the modernized side calls `GET /api/patient-portal/messages/recipients`, validates compose routing against that directory, and renders the same selected recipient in the Portal secure-message compose selector. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
