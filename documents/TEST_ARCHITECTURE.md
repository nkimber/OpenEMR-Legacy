...
The modernized target now exists and currently includes thirty-seven read-only slices plus eighty-three mutation-capable slices through Slice 120. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, insurance coverage, immunization history, scheduling, encounter, revenue-cycle, document, administration, access-control, reporting, and clinical-list workflows, and perform safe cleanup-backed mutation lifecycles for patient, scheduling, encounter, document, revenue-cycle, procedure, administration, and ACL workflow slices.

The slice readiness plans from `slice-1-readiness` through `slice-120-appointment-reminders-readiness` prove the same normalized database facts, browser-visible behavior, mutation post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. The newest appointment reminders plan uses seeded patient `MOD-PAT-0191` and appointment `APPT-MOD-PAT-0191-3` on `2026-06-25`, exactly seven days after dataset base date `2026-06-18`, to verify due-now reminder readiness, `SMS + Email` channel selection, `(619) 555-1191 / mod-pat-0191@example.test` contact rendering, and a seven-day lead in both database probes and browser-visible Calendar detail. Earlier appointment plans remain stable for create/cancel/delete, reschedule/update, arrival/check-in, check-out, no-show, category, pending-status, provider reassignment, facility reassignment, billing-location reassignment, comments behavior, recurrence metadata behavior, recurring-series read expansion, recurrence-exception read expansion, occurrence-cancel mutation, occurrence-restore mutation, occurrence-reschedule mutation, recurrence exception-list editing, recurring root title/time propagation, recurring root metadata propagation, monthly recurrence, recurrence unit matrix behavior, days-of-week recurrence behavior, monthly repeat-on recurrence behavior, seeded series recurrence update behavior, provider-overlap tolerance behavior, patient-overlap tolerance behavior, and room-overlap tolerance behavior.

For the one-hundred-twentieth modernized appointment reminder readiness slice, compare the current side-by-side readiness plan with:

```powershell
npm run compare -- --left-target legacy-openemr --right-target modernized-openemr --plan slice-120-appointment-reminders-readiness
```
