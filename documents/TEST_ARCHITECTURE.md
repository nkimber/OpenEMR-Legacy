...
The modernized target now exists and includes implemented workflow slices through Slice 202. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, duplicate detection, registration validation, deceased status, guardian contact, guardian details, social details, employer details, provider assignment, care-team membership, patient history/lifestyle, insurance coverage, immunization history, scheduling, encounter, revenue-cycle, procedure lab-provider ownership, directory, lifecycle, configuration, address-book, order-catalog, and clinical order queue views, document, administration, access-control, reporting, and clinical-list workflows, create/validate/end an admin session, record login audit evidence, enforce protected login-audit access, enforce protected administration directory access, enforce ACL-backed administration authorization, enforce protected operational reports and CSV export access, enforce ACL-backed operational reports authorization, enforce protected patient search/chart access, enforce ACL-backed patient chart authorization, enforce patient duplicate detection, enforce patient registration validation row safety, enforce protected clinical-list access, enforce ACL-backed clinical-list authorization, enforce protected appointment search access, enforce ACL-backed appointment authorization, enforce view/write appointment mutation authorization, enforce protected encounter search access, enforce ACL-backed encounter authorization, enforce encounter amendment history projection, enforce protected patient document access, enforce protected patient message access, enforce ACL-backed patient message authorization, enforce protected billing access, enforce ACL-backed billing authorization, enforce protected procedure access, enforce ACL-backed procedure authorization, enforce add/write/sign procedure mutation authorization, enforce add/write patient mutation authorization, enforce view/write encounter mutation authorization, enforce add/write/delete document mutation authorization, enforce add/write message mutation authorization, enforce view/write billing mutation authorization, and perform safe cleanup-backed mutation lifecycles for patient, patient deceased status, patient guardian contact, patient guardian details, patient social details, patient employer details, patient provider assignment, patient care-team membership, patient contact-backed care-team membership, patient-message reply/content/assignment, scheduling, encounter, document, revenue-cycle, procedure lab-provider lifecycle/configuration/address-book, procedure order catalog lifecycle/import, procedure order queue/transmit, procedure report bulk sign-off/reopen review, administration, and ACL workflow slices.

The slice readiness plans from `slice-1-readiness` through `slice-202-patient-history-readiness` prove the same normalized database facts, browser-visible behavior, mutation/session/audit post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. Slice 202 verifies read-only patient history/lifestyle behavior on both targets; the legacy side reads and renders `history_data` through OpenEMR's History and Lifestyle page, while the modernized side exposes the same row through PostgreSQL `patient_histories`, patient chart summaries, the `history` API object, and the Patient/Client History and Lifestyle panel. Slice 201 verifies mixed provider/contact-backed patient care-team behavior on both targets; the legacy side reads and renders `care_team_member.user_id` plus `care_team_member.contact_id` rows joined to `users`, `facility`, `contact`, and `person`, while the modernized side exposes the same member type, provider, facility, contact, role, note, and status facts through PostgreSQL `patient_care_team_members`, `patient_related_contacts`, `/api/patients/{patientId}/care-team`, `/api/patients/{patientId}/care-team-options`, chart summaries, and the Patient/Client Care Team panel, then restores the original anchor members. Slice 200 verifies multi-member patient care-team behavior on both targets; the legacy side reads and renders two `care_team_member` rows joined to `users` and `facility`, while the modernized side exposes the same member/provider/facility facts through PostgreSQL `patient_care_team_members`, the `members` collection accepted by `/api/patients/{patientId}/care-team`, chart summaries, and the multi-member Patient/Client Care Team panel, then restores the original anchor members. Slice 199 verifies patient care-team lead-member behavior on both targets; the legacy side reads and renders `care_teams` / `care_team_member` joined to `users` and `facility`, while the modernized side exposes the same team/member/provider/facility facts through PostgreSQL `patient_care_teams` / `patient_care_team_members`, `/api/patients/{patientId}/care-team`, chart summaries, and the Patient/Client Care Team panel, then restores the original anchor value. Slice 198 verifies patient primary-provider assignment behavior on both targets; the legacy side reads and renders `patient_data.providerID` joined to `users`, while the modernized side exposes the same provider ID/name through PostgreSQL `patients.provider_id`, `/api/patients/provider-options`, `/api/patients/{patientId}/provider-assignment`, chart summaries, and the Patient/Client Primary Provider panel, then restores the original anchor value. Slice 197 verifies patient employer behavior on both targets; the legacy side reads and renders `employer_data.name`, `street`, `city`, `state`, `postal_code`, and `country`, while the modernized side exposes the same fields through PostgreSQL `patient_employers`, `/api/patients/{patientId}/employer`, chart summaries, and the Patient/Client Employer panel, then restores the original anchor values. Slice 196 verifies patient social-detail behavior on both targets; the legacy side reads and renders `patient_data.race`, `ethnicity`, `interpreter`, `family_size`, `monthly_income`, `homeless`, and `financial_review`, while the modernized side exposes the same fields through PostgreSQL, `/api/patients/{patientId}/demographics`, chart summaries, and the Patient/Client Demographics panel, then restores the original anchor values. Slice 195 verifies patient guardian demographic/address behavior on both targets; the legacy side reads and renders `patient_data.guardiansex`, `guardianaddress`, `guardiancity`, `guardianstate`, `guardianpostalcode`, `guardiancountry`, and `guardianworkphone`, while the modernized side exposes the same fields through PostgreSQL, `/api/patients/{patientId}/guardian-contact`, chart summaries, and the Patient/Client Guardian Contact panel, then restores the original anchor values. Slice 194 remains available for mother and guardian contact behavior on both targets. Slice 193 verifies patient deceased date/reason behavior on both targets; the legacy side reads and renders `patient_data.deceased_date` / `deceased_reason`, while the modernized side exposes the same fields through PostgreSQL, `/api/patients/{patientId}/deceased-status`, chart summaries, and the Patient/Client Deceased Status panel, then restores the original anchor values. Slice 192 verifies the legacy OpenEMR PatientValidator source contract for registration fields and proves the modernized API/UI returns structured validation errors for invalid registration drafts without creating a patient row. Slice 191 verifies duplicate candidate detection on both targets; the legacy side reads matching `patient_data` rows, while the modernized side exposes scored duplicate candidates through `/api/patients/duplicates`, patient chart summaries, and Duplicate Detection UI panels after creating and cleaning up a temporary duplicate registration. Slice 190 verifies signature-derived encounter amendment history on both targets; the legacy side reads OpenEMR `esign_signatures` amendment rows, while the modernized side exposes the same nonblank amendment rows through the encounter detail API and a dedicated Encounter Amendment History panel, preserving newest-first order and cleanup. Slice 189 verifies mutation-specific ACL-backed Appointment write authorization on both targets; the legacy side keeps the reference scheduler surface visible, while the modernized side temporarily downgrades Clinicians from `patients:appt write` to `patients:appt view`, proves clinician appointment reads still succeed, proves create, update, status, recurrence restore, occurrence reschedule, and delete mutations receive 403 requiring `patients:appt write`, and restores the permanent write grant after the run. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
