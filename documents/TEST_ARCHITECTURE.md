...
Slice 95 extends these allow-lists with `workflow-appointment-checkout` and `slice-95-appointment-checkout-readiness`, while the Slice 93 appointment reschedule and Slice 94 appointment arrival entries remain available.

## Test Management

The test manifest now has two selection layers:

- Suites: layer-level groups such as database, HTTP, UI, workflow, patient-chart slice parity, scheduling slice parity, encounter slice parity, encounter metadata mutation parity, encounter billing linkage mutation parity, encounter diagnosis coding mutation parity, encounter fee-sheet entry parity, encounter procedure-order entry parity, encounter procedure-result entry parity, encounter sign-off parity, encounter document upload parity, encounter binary document upload parity, encounter document sign-off parity, encounter document denial parity, encounter document metadata parity, encounter document move parity, encounter document content replacement parity, encounter document archive restore parity, encounter document lifecycle timeline parity, encounter external-link document parity, clinical-list slice parity, messaging slice parity, patient-message assignment parity, patient-message content parity, encounter document attachment parity, encounter billing linkage parity, encounter claim linkage parity, encounter procedure order linkage parity, procedure-result slice parity, pending procedure-order slice parity, fee-sheet billing slice parity, fee-sheet diagnosis coding parity, fee-sheet charge correction parity, claim status parity, claim status mutation parity, payment posting parity, payment posting mutation parity, patient payment capture parity, collections follow-up task parity, account balance parity, account aging parity, account ledger parity, account statement parity, patient statement generation parity, patient statement PDF export parity, statement batch candidate parity, statement batch package parity, collections work queue parity, insurance coverage parity, insurance mutation parity, immunization history parity, administration directory slice parity, operational reports slice parity, reports export slice parity, patient documents slice parity, patient document content parity, patient document preview parity, patient image document preview parity, patient image document thumbnail parity, patient PDF document preview parity, patient document lifecycle timeline parity, patient scanned attachment parity, appointment reschedule parity, appointment arrival parity, appointment check-out parity, patient document revision parity, patient document replacement revision parity, patient document mutation parity, binary patient-document mutation parity, patient document sign-off parity, patient document external-link parity, patient document denial parity, patient document metadata parity, patient document archive restore parity, patient document content replacement parity, problem-list mutation parity, medication-list mutation parity, and immunization mutation parity.
...
The modernized target now exists and currently includes thirty-four read-only slices plus sixty-one mutation-capable slices through Slice 95. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, insurance coverage, immunization history, scheduling, encounter, encounter document attachment, encounter billing linkage, encounter claim linkage, encounter procedure order linkage, encounter diagnosis coding linkage, clinical-list, messaging, completed procedure-result, pending procedure-order, billing, claim status, payment posting, account balance, account aging, account ledger, account statement readiness, patient statement generation, patient statement PDF export, statement batch candidates, statement batch package export, collections work queue, administration, access-control, reporting, report export, patient document, patient document content, patient document preview, patient document image thumbnail, patient document PDF inline preview, patient document lifecycle timeline, patient scanned attachment, patient document revision, encounter-attached document, encounter billing linkage, encounter procedure order linkage, and encounter diagnosis coding workflows, and perform safe cleanup-backed mutation lifecycles for patient contact, patient demographics, patient registration, insurance coverage, appointments, appointment reschedules, appointment arrivals, appointment check-outs, encounters, encounter metadata, encounter billing linkage visibility, encounter diagnosis coding visibility, encounter fee-sheet entry, encounter procedure-order entry, encounter procedure-result entry, encounter sign-off, encounter document upload, encounter binary document upload, encounter document sign-off, encounter document denial, encounter document metadata refiling, encounter document movement, encounter document content replacement, encounter document archive/restore, encounter document lifecycle timeline rendering, encounter external-link document attachment, allergies, problem lists, medication lists, messages, message content editing, message assignment, collections follow-up tasks, text/binary/image-preview/image-thumbnail/pdf-preview/lifecycle/scanned-attachment/sign-off/denial/metadata/archive/content-replacement/replacement-revision/external-link patient documents, prescriptions, immunizations, billing lines, billing diagnosis lines, billing correction lines, billing modifier lines, claim statuses, payment postings, patient payments, procedures, facilities, users, ACL permission assignments, and ACL user group memberships.

The slice readiness plans from `slice-1-readiness` through `slice-95-appointment-checkout-readiness` prove the same normalized database facts, browser-visible behavior, mutation post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. The newest appointment check-out plan creates a temporary future appointment on `MOD-PAT-0003`, marks it arrived with status `@`, checks it out with status `>`, renders the checked-out state through the modernized Calendar `Mark checked out` action, deletes the temporary appointment, and returns seeded appointment counts clean; the appointment arrival plan creates a temporary future appointment on `MOD-PAT-0003`, marks it arrived with status `@`, renders the arrived state through the modernized Calendar `Mark arrived` action, deletes the temporary appointment, and returns seeded appointment counts clean; the appointment reschedule plan creates a temporary future appointment on `MOD-PAT-0003`, updates date/time/duration/status/room, renders the changed schedule in the modernized Calendar, deletes the temporary appointment, and returns seeded appointment counts clean; the patient scanned attachment plan creates a temporary scanned PDF document on `MOD-PAT-0001`, verifies `Scanned attachment`, `front-desk scanner`, one scanned page, `OCR pending`, active/archive state, and modernized Documents card/viewer scan-readiness rendering, deletes the temporary document, and returns seeded patient document counts clean; the patient document lifecycle timeline plan creates a temporary text document on `MOD-PAT-0001`, verifies filed/current-version/review-pending/active lifecycle state, signs it, verifies review-approved lifecycle state, archives it, verifies archived lifecycle state, restores it, verifies active lifecycle state and modernized Documents card/viewer lifecycle rendering, deletes the temporary document, and returns seeded patient document counts clean; the patient PDF document preview plan creates a temporary `application/pdf` document on `MOD-PAT-0001`, verifies `pdf`, `Inline PDF preview`, `PDF`, inline-readiness, content/download bytes, active/archive state, and modernized Documents viewer iframe rendering, deletes the temporary document, and returns seeded patient document counts clean; the patient image document thumbnail plan creates a temporary `image/svg+xml` document on `MOD-PAT-0001`, verifies `thumbnailDataUri` from stored image bytes, active/archive state, and modernized Documents card thumbnail rendering, deletes the temporary document, and returns seeded patient document counts clean; the patient image document preview plan creates a temporary `image/svg+xml` document on `MOD-PAT-0001`, verifies `image`, `Inline image preview`, `IMG`, inline-readiness, content/download bytes, active/archive state, and modernized Documents viewer image rendering, deletes the temporary document, and returns seeded patient document counts clean; the encounter external-link document plan creates a temporary web URL document on `MOD-PAT-0001` encounter `1000013`, verifies `text/uri-list`, `web_url`, URL, preview, active/archive state, and modernized Encounter external-link card rendering, deletes the temporary document, and returns seeded encounter document counts clean; the encounter document lifecycle timeline plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, verifies filed/current-version/review-pending/active lifecycle state, signs it, verifies review-approved lifecycle state, archives it, verifies archived lifecycle state, restores it, verifies active lifecycle state and modernized Encounter timeline rendering, deletes the temporary document, and returns seeded encounter document counts clean; the encounter document archive/restore plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, archives it, verifies active encounter detail hides it, verifies archived detail and the modernized Encounter attached-document panel can show and restore it, deletes the temporary document, and returns seeded encounter document counts clean; the encounter document content replacement plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, replaces its text payload in place, verifies replacement preview and current-version facts, renders the replacement state through the modernized Encounter attached-document panel, deletes the temporary document, and returns seeded encounter document counts clean; the encounter document move plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, moves it to same-patient encounter `1000011`, verifies source removal and target attachment, renders the moved state through the modernized Encounter attached-document panel, deletes the temporary document, and returns seeded source/target encounter document counts clean; the encounter document metadata plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, refiles name/category/date/notes while preserving the encounter link, verifies updated metadata, renders the updated state through the modernized Encounter attached-document panel, deletes the temporary document, and returns seeded document counts clean; the encounter document denial plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, verifies pending review state, denies it as `admin`, verifies denied review metadata, renders the denied state through the modernized Encounter attached-document panel with both Sign and Deny disabled, deletes the temporary document, and returns seeded document counts clean; the encounter document sign-off plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, verifies pending review state, signs it as `admin`, verifies approved review metadata, renders the approved state through the modernized Encounter attached-document panel, deletes the temporary document, and returns seeded document counts clean; the encounter binary document upload plan creates a temporary PDF document on `MOD-PAT-0001` encounter `1000013`, verifies legacy `documents.encounter_id` and modernized encounter document facts, renders legacy document-category evidence and the modernized Encounter attached-document panel, downloads matching binary content, deletes the temporary document, and returns seeded document counts clean; the encounter document upload plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, verifies legacy `documents.encounter_id` and modernized encounter document facts, renders legacy document-category evidence and the modernized Encounter attached-document panel, deletes the temporary document, and returns seeded document counts clean; the encounter sign-off plan creates a temporary encounter for `MOD-PAT-0002`, records an `admin` attestation, verifies legacy `esign_signatures` and modernized `encounter_signatures` facts, renders the modernized Encounter Sign-Off panel, deletes the signature, and deletes the temporary encounter so the seeded encounter/signature baseline returns clean; earlier plan details remain stable for workflow mutation, revenue-cycle, document, reporting, administration, clinical-list, messaging, insurance, immunization, procedure, and patient-chart slices.
