...
Slice 126 extends these allow-lists with `workflow-encounter-document-scanned-attachment` and `slice-126-encounter-document-scanned-attachment-readiness`; Slice 123 encounter document replacement revision, Slice 122 encounter document revision, and Slice 121 encounter co-signature remain available; Slice 120 appointment reminders, Slice 119 appointment room overlap, Slice 118 appointment patient overlap, Slice 117 appointment provider overlap, Slice 116 appointment series recurrence update, Slice 115 appointment monthly repeat-on recurrence, Slice 114 appointment days-of-week recurrence, Slice 103 appointment recurrence metadata, Slice 104 appointment recurring-series, Slice 105 appointment recurrence-exceptions, Slice 106 appointment occurrence-cancel, Slice 107 appointment occurrence-restore, Slice 108 appointment occurrence-reschedule, Slice 109 appointment recurrence exception-list edit, Slice 110 appointment series root update, Slice 111 appointment series root metadata, Slice 112 appointment monthly recurrence, Slice 113 appointment recurrence unit matrix, and Slice 93 through Slice 102 appointment scheduling entries remain available.
...
The modernized target now exists and currently includes thirty-eight read-only slices plus eighty-six mutation-capable slices through Slice 126. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, insurance coverage, immunization history, scheduling, encounter, revenue-cycle, document, administration, access-control, reporting, and clinical-list workflows, and perform safe cleanup-backed mutation lifecycles for patient, scheduling, encounter, document, revenue-cycle, procedure, administration, and ACL workflow slices.

The slice readiness plans from `slice-1-readiness` through `slice-126-encounter-document-scanned-attachment-readiness` prove the same normalized database facts, browser-visible behavior, mutation post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. The newest encounter scanned attachment plan creates a temporary PDF on encounter `1000013`, derives scan status, capture source, scanned page count, and OCR status from document notes/metadata, confirms modernized API/card rendering, and hard-deletes the document. Slice 123 encounter document replacement revision remains available for temporary current-revision mutation parity, Slice 122 encounter document revision remains available for seeded `DOC-MOD-PAT-0001-1` and `DOC-MOD-PAT-0001-2` current-version metadata, and Slice 121 encounter co-signature plus Slice 120 appointment reminders remain available. Earlier appointment plans remain stable for create/cancel/delete, reschedule/update, arrival/check-in, check-out, no-show, category, pending-status, provider reassignment, facility reassignment, billing-location reassignment, comments behavior, recurrence metadata behavior, recurring-series read expansion, recurrence-exception read expansion, occurrence-cancel mutation, occurrence-restore mutation, occurrence-reschedule mutation, recurrence exception-list editing, recurring root title/time propagation, recurring root metadata propagation, monthly recurrence, recurrence unit matrix behavior, days-of-week recurrence behavior, monthly repeat-on recurrence behavior, seeded series recurrence update behavior, provider-overlap tolerance behavior, patient-overlap tolerance behavior, and room-overlap tolerance behavior.

For the one-hundred-twenty-sixth modernized encounter scanned attachment readiness slice, compare the current side-by-side readiness plan with:

```powershell
npm run compare -- --left-target legacy-openemr --right-target modernized-openemr --plan slice-126-encounter-document-scanned-attachment-readiness
```
