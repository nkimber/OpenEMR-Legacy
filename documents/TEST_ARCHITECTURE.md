...
For the eighty-second modernized encounter document metadata readiness slice, compare the current side-by-side readiness plan with:

```powershell
npm run compare -- --left-target legacy-openemr --right-target modernized-openemr --plan slice-82-encounter-document-metadata-readiness
```
