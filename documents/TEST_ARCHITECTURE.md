...
Slice 218 adds patient portal document list/download readiness with the `workflow-patient-portal-documents` suite and `slice-218-patient-portal-documents-readiness` plan. The suite signs in as `MOD-PAT-0004`, compares active patient-owned document categories and metadata against the legacy portal document page, downloads the selected document ZIP package, and keeps the per-test reset contract used by the prior patient-portal slices. Slice 217 remains available for patient portal secure-message All-folder readiness with the `workflow-patient-portal-all-messages` suite and `slice-217-patient-portal-all-messages-readiness` plan.
...
Slice 218 extends these allow-lists with `workflow-patient-portal-documents` and `slice-218-patient-portal-documents-readiness`; Slice 217 remains available with `workflow-patient-portal-all-messages` and `slice-217-patient-portal-all-messages-readiness`.
