...
The modernized target now exists and currently includes thirty-four read-only slices plus forty-six mutation-capable slices through Slice 80. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, insurance coverage, immunization history, scheduling, encounter, encounter document attachment, encounter billing linkage, encounter claim linkage, encounter procedure order linkage, encounter diagnosis coding linkage, clinical-list, messaging, completed procedure-result, pending procedure-order, billing, claim status, payment posting, account balance, account aging, account ledger, account statement readiness, patient statement generation, patient statement PDF export, statement batch candidates, statement batch package export, collections work queue, administration, access-control, reporting, report export, patient document, patient document content, patient document preview, patient document revision, encounter-attached document, encounter billing linkage, encounter procedure order linkage, and encounter diagnosis coding workflows, and perform safe cleanup-backed mutation lifecycles for patient contact, patient demographics, patient registration, insurance coverage, appointments, encounters, encounter metadata, encounter billing linkage visibility, encounter diagnosis coding visibility, encounter fee-sheet entry, encounter procedure-order entry, encounter procedure-result entry, encounter sign-off, encounter document upload, encounter binary document upload, encounter document sign-off, allergies, problem lists, medication lists, messages, message content editing, message assignment, collections follow-up tasks, text/binary/sign-off/denial/metadata/archive/content-replacement/replacement-revision/external-link patient documents, prescriptions, immunizations, billing lines, billing diagnosis lines, billing correction lines, billing modifier lines, claim statuses, payment postings, patient payments, procedures, facilities, users, ACL permission assignments, and ACL user group memberships.

The slice readiness plans from `slice-1-readiness` through `slice-80-encounter-document-signoff-readiness` prove the same normalized database facts, browser-visible behavior, mutation post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. The newest encounter document sign-off plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, verifies pending review state, signs it as `admin`, verifies approved review metadata, renders the approved state through the modernized Encounter attached-document panel, deletes the temporary document, and returns seeded document counts clean; the encounter binary document upload plan creates a temporary PDF document on `MOD-PAT-0001` encounter `1000013`, verifies legacy `documents.encounter_id` and modernized encounter document facts, renders legacy document-category evidence and the modernized Encounter attached-document panel, downloads matching binary content, deletes the temporary document, and returns seeded document counts clean; the encounter document upload plan creates a temporary text document on `MOD-PAT-0001` encounter `1000013`, verifies legacy `documents.encounter_id` and modernized encounter document facts, renders legacy document-category evidence and the modernized Encounter attached-document panel, deletes the temporary document, and returns seeded document counts clean; the encounter sign-off plan creates a temporary encounter for `MOD-PAT-0002`, records an `admin` attestation, verifies legacy `esign_signatures` and modernized `encounter_signatures` facts, renders the modernized Encounter Sign-Off panel, deletes the signature, and deletes the temporary encounter so the seeded encounter/signature baseline returns clean; the encounter procedure-result entry plan creates a temporary pending `80053` laboratory order plus reviewed final report/result on `MOD-PAT-0001` encounter `1000013`, verifies legacy Procedure Results rendering, modernized Encounter workspace result-entry behavior, Encounter detail API report/result fields, and hard-delete cleanup so the seeded procedure-order/report/result baseline returns clean; the encounter procedure-order entry plan creates a temporary pending `80053` laboratory order on `MOD-PAT-0001` encounter `1000013`, verifies legacy Procedure Orders and Reports rendering, modernized Encounter workspace entry behavior, Encounter detail API procedure-order fields, no-report pending state, and hard-delete cleanup so the seeded procedure-order baseline returns clean; the encounter fee-sheet entry plan creates temporary `CPT4 99499` and `ICD10 R73.03` rows on `MOD-PAT-0001` encounter `1000013`, verifies legacy Fee Sheet rendering, modernized Encounter workspace entry behavior, Fee Sheet Linkage rendering, Diagnosis Coding rendering, billed/inactive hiding, and hard-delete cleanup so the seeded billing baseline returns clean; the encounter diagnosis coding mutation plan creates a temporary `ICD10 R73.03` fee-sheet diagnosis row on `MOD-PAT-0001` encounter `1000013`, verifies legacy Fee Sheet rendering, modernized Encounter detail API diagnosis evidence, modernized Encounters Diagnosis Coding rendering, billed/inactive hiding, and hard-delete cleanup so the seeded billing baseline returns clean; the encounter billing linkage mutation plan creates a temporary `CPT4 99499` fee-sheet row on `MOD-PAT-0001` encounter `1000013`, verifies legacy Fee Sheet rendering, modernized Encounter detail API billing fields, modernized Encounters Fee Sheet Linkage rendering, and supporting diagnosis evidence, then deactivates and hard-deletes the row so the seeded billing baseline returns clean; the encounter diagnosis coding plan covers `MOD-PAT-0001` encounter `1000013` diagnosis `E78.5`, fee-sheet justification support from `CPT4 99214` and `CPT4 36415`, `MOD-PAT-0001` encounter `1000011` diagnosis `E11.9`, procedure order `5000001`, legacy encounter/fee-sheet/procedure-result rendering including the fee-sheet visible `E78.` justification text, modernized Encounter detail API diagnosis evidence fields, and Encounters workspace Diagnosis Coding rendering; the encounter procedure order linkage plan covers `MOD-PAT-0001` encounter `1000011`, procedure order `5000001`, report `6000001`, four final result rows including `Hemoglobin A1c` `5.7 %`, legacy procedure-result rendering, modernized Encounter detail API procedure-order fields, and Encounters workspace Procedure Orders rendering; the encounter claim linkage plan covers `MOD-PAT-0001` encounter `1000013`, claim `CLAIM-1000013-1`, payer `Acme Health`, `HCFA` target, cleared claim status facts, legacy claim row facts, modernized Encounter detail API claim fields, and Encounters workspace Claim Linkage rendering; the encounter billing linkage plan covers `MOD-PAT-0001` encounter `1000013`, two active CPT4 fee-sheet lines, legacy fee-sheet rendering, modernized Encounter detail API billing fields, and Encounters workspace Fee Sheet Linkage rendering; the encounter document attachment plan covers `MOD-PAT-0001` encounter `1000013`, two linked documents, legacy document-category rendering, modernized Encounter detail API document fields, and Encounters workspace attached-document rendering; earlier plan details remain stable for workflow mutation, revenue-cycle, document, reporting, administration, clinical-list, messaging, insurance, immunization, procedure, and patient-chart slices.
