...
- Slice 604 encounter SOAP template plan for side-by-side temporary encounter creation, selected SOAP-template payload save behavior, protected modernized catalog evidence, Encounters workspace template application, and cleanup of the temporary SOAP note plus encounter.
- Slice 605 patient merge preview plan for side-by-side temporary duplicate-patient source creation, source/target/combined workflow-count projection, non-destructive modernized merge-preview API/UI evidence, and cleanup of the temporary source chart.
...
The modernized target now exists and includes implemented workflow slices through Slice 605. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 605 extends patient administration with non-destructive duplicate-patient merge-preview evidence.

The slice readiness plans from `slice-1-readiness` through `slice-605-patient-merge-preview-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 605 verifies patient merge-preview readiness by creating a cleanup-backed duplicate source chart for `MOD-PAT-0010`, proving target/source/combined workflow-count evidence, checking modernized protected preview API output and Patient/Client merge-preview rendering, and deleting the temporary source chart. Slice 604 verifies encounter SOAP template readiness by creating a cleanup-backed temporary encounter for `MOD-PAT-0002`, applying the selected diabetes follow-up SOAP template to the modernized SOAP form, saving the resulting note, comparing the saved SOAP payload with the legacy form model, and deleting the temporary rows. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
