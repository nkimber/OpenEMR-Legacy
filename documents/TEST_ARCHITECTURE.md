...
The modernized target now exists and includes implemented workflow slices through Slice 590. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 590 extends document operations with pending-OCR queue readiness.

The slice readiness plans from `slice-1-readiness` through `slice-590-document-ocr-queue-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 590 verifies document OCR queue readiness by creating a cleanup-backed scanned PDF with `OCR pending` metadata, proving scanned pending-OCR source facts on both targets, proving the modernized protected OCR queue API projects the document as `Ready for OCR`, proving the Documents OCR Queue panel renders the pending item, and deleting the temporary document. Slice 589 verifies appointment conflict enforcement readiness by creating a cleanup-backed blocker appointment, proving provider and room conflict source facts on both targets, proving strict modernized appointment creation returns HTTP 409 with conflict details, proving the Calendar `Block conflicts` create path renders the conflict message, and confirming the blocked appointment row is not inserted. Slice 588 verifies medication reconciliation readiness by creating cleanup-backed matched, medication-list-only, and prescription-only evidence for `MOD-PAT-0008`, proving normalized active medication-list and prescription source facts on both targets, proving the modernized clinical-list API projects all three reconciliation states, proving the Lists medication panel renders the summary, and deleting all temporary rows. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
