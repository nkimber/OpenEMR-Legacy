...
The modernized target now exists and includes implemented workflow slices through Slice 524. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 524 extends the billing surface by verifying fee-sheet charge-template application from the modernized Fees page.

The slice readiness plans from `slice-1-readiness` through `slice-524-billing-charge-template-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 524 verifies charge-template readiness by loading the modernized Fees page, replacing ad hoc CPT draft values through the `Office visit` template, saving the generated `CPT4 99213` fee-sheet line, and deleting the temporary billing line after comparison. Slice 523 verifies appointment-to-billing conversion by creating a cleanup-backed appointment, converting it into an encounter, creating a starter `CPT4 99213` fee-sheet line on that encounter, confirming the modernized Calendar changes the action to `Charge created`, and deleting the temporary billing line before deleting the encounter and appointment. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
