...
The modernized target now exists and includes implemented workflow slices through Slice 522. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 522 extends the scheduling/encounter boundary by verifying appointment-to-encounter conversion with source appointment linkage.

The slice readiness plans from `slice-1-readiness` through `slice-522-appointment-encounter-conversion-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 522 verifies appointment-to-encounter conversion by creating a cleanup-backed appointment, converting it into an encounter, proving the encounter inherits patient, provider, date, facility, billing facility, reason, and source appointment linkage, confirming the modernized Calendar changes the action to `Encounter created`, and deleting the temporary encounter before deleting the appointment. Slice 521 verifies patient portal secure-message search pagination behavior by creating cleanup-backed newer nonmatching Inbox rows plus 21 matching rows, proving search-before-pagination produces a first filtered page and an older second-page match, documenting the installed legacy no-search-input baseline, and confirming the modernized Portal resets to the first filtered page when search changes. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
