...
The modernized target now exists and includes implemented workflow slices through Slice 588. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, portal, scheduling, encounter, revenue-cycle, procedure, document, administration, access-control, reporting, and clinical-list workflows, enforce the implemented session/ACL/mutation policies, and perform safe cleanup-backed mutation lifecycles. Slice 588 extends the prescribing surface with medication reconciliation evidence that compares active medication-list rows with active prescriptions.

The slice readiness plans from `slice-1-readiness` through `slice-588-medication-reconciliation-readiness` prove normalized database facts, browser-visible behavior, mutation/session/audit and cleanup expectations against both legacy and modernized targets where applicable. Slice 588 verifies medication reconciliation readiness by creating cleanup-backed matched, medication-list-only, and prescription-only evidence for `MOD-PAT-0008`, proving normalized active medication-list and prescription source facts on both targets, proving the modernized clinical-list API projects all three reconciliation states, proving the Lists medication panel renders the summary, and deleting all temporary rows. Slice 587 verifies prescription diagnosis interaction readiness by creating a cleanup-backed active problem and two prescriptions for `MOD-PAT-0008`, proving one diagnosis matches an active problem and one diagnosis remains unmatched, proving the modernized clinical-list API projects both interaction states, proving the Lists prescription panel renders the summary, and deleting all temporary rows. Slice 586 verifies medication vocabulary readiness by searching for Metformin, proving RxNorm `860975`, strength, dose, frequency, duration, and controlled-substance schedule evidence through the shared workflow adapter, and proving the modernized Lists prescription form can render and select the vocabulary result without creating a patient-specific row. Slice 585 verifies prescription structured-dose readiness by creating a cleanup-backed prescription for `MOD-PAT-0008`, proving dose amount/unit, frequency, and duration evidence through the shared workflow adapter, proving the modernized Lists prescription card renders the structured dose line, and deleting the temporary prescription. Slice 584 verifies prescription audit-history readiness by creating a cleanup-backed prescription for `MOD-PAT-0008`, refilling it, routing it to deterministic Northstar pharmacy evidence, comparing create/refill/route audit events across legacy-derived workflow evidence and the modernized audit table, proving the modernized Lists `History` action renders the event stream, and deleting the temporary prescription plus legacy-only pharmacy fixture. Slice 583 verifies prescription controlled-substance readiness by creating a cleanup-backed Oxycodone prescription for `MOD-PAT-0008`, proving ordinary pharmacy routing is blocked before pharmacy/eRx transmit fields are stamped, proving the modernized Lists warning renders, and deleting the temporary prescription plus legacy-only pharmacy fixture. Slice 582 verifies prescription pharmacy routing readiness by creating a cleanup-backed active prescription for `MOD-PAT-0008`, using deterministic Northstar pharmacy evidence, routing the prescription, proving pharmacy ID/name/NCPDP, sent timestamp, note, and local eRx payload evidence, proving the modernized Lists `Route` action renders and transmits the route, and deleting the temporary prescription plus legacy-only pharmacy fixture. Slice 581 verifies prescription refill request approval readiness by creating a cleanup-backed active prescription for `MOD-PAT-0004`, submitting a portal refill request, proving the staff queue projection, approving the request, proving refill count, modified-date, note, `Done` mailbox status, and queue removal, proving the modernized Lists `Refill Requests` action renders and approves the request, and deleting the temporary prescription plus mailbox rows. Slice 580 verifies patient portal prescription refill request readiness by creating a cleanup-backed active prescription for `MOD-PAT-0004`, submitting a refill request through secure messaging, proving title/body/sender/recipient/status/relation mailbox evidence, proving the modernized Portal `Request refill` action renders and sends the request, and deleting the temporary prescription plus mailbox rows. Slice 579 verifies prescription refill authorization readiness by creating a cleanup-backed active prescription for `MOD-PAT-0008`, adding one refill without ending the prescription, proving refill count, modified-date, note, and active/end-date state, and proving the modernized Lists `Refill` action renders the updated refill evidence. Slice 578 verifies medication duplicate detection readiness by creating two cleanup-backed active medication-list rows for `MOD-PAT-0006`, grouping them by normalized medication title, proving the modernized clinical-list API exposes duplicate count/source IDs/date range/diagnosis evidence, and proving the modernized Lists page renders the duplicate summary. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
