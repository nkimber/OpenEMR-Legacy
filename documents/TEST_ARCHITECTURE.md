...
For the eighty-fourth modernized encounter document content replacement readiness slice, compare the current side-by-side readiness plan with:

```powershell
npm run compare -- --left-target legacy-openemr --right-target modernized-openemr --plan slice-84-encounter-document-content-replace-readiness
```
