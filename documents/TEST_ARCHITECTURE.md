...
The modernized target now exists and includes implemented workflow slices through Slice 188. The smoke test proves that the target can run, consume the shared gold dataset, retrieve deterministic anchors across patient, insurance coverage, immunization history, scheduling, encounter, revenue-cycle, procedure lab-provider ownership, directory, lifecycle, configuration, address-book, order-catalog, and clinical order queue views, document, administration, access-control, reporting, and clinical-list workflows, create/validate/end an admin session, record login audit evidence, enforce protected login-audit access, enforce protected administration directory access, enforce ACL-backed administration authorization, enforce protected operational reports and CSV export access, enforce ACL-backed operational reports authorization, enforce protected patient search/chart access, enforce ACL-backed patient chart authorization, enforce protected clinical-list access, enforce ACL-backed clinical-list authorization, enforce protected appointment search access, enforce ACL-backed appointment authorization, enforce protected encounter search access, enforce ACL-backed encounter authorization, enforce protected patient document access, enforce protected patient message access, enforce ACL-backed patient message authorization, enforce protected billing access, enforce ACL-backed billing authorization, enforce protected procedure access, enforce ACL-backed procedure authorization, enforce add/write/sign procedure mutation authorization, enforce add/write patient mutation authorization, enforce view/write encounter mutation authorization, enforce add/write/delete document mutation authorization, enforce add/write message mutation authorization, enforce view/write billing mutation authorization, and perform safe cleanup-backed mutation lifecycles for patient, patient-message reply/content/assignment, scheduling, encounter, document, revenue-cycle, procedure lab-provider lifecycle/configuration/address-book, procedure order catalog lifecycle/import, procedure order queue/transmit, procedure report bulk sign-off/reopen review, administration, and ACL workflow slices.

The slice readiness plans from `slice-1-readiness` through `slice-188-billing-mutation-authorization-policy-readiness` prove the same normalized database facts, browser-visible behavior, mutation/session/audit post-state where applicable, cleanup, and restoration expectations against both legacy and modernized targets. Slice 188 verifies mutation-specific ACL-backed billing write authorization on both targets; the legacy side keeps the reference fee-sheet surface visible, while the modernized side temporarily grants Clinicians `acct:bill view`, proves clinician billing reads succeed, proves fee-sheet line, claim, payment, and collections follow-up mutations receive 403 requiring `acct:bill write`, and revokes the temporary grant after the run. Slice 187 verifies mutation-specific patient-message authorization on both targets; the shared ACL matrix proves Administrators have Patient Notes write while Clinicians have permanent Patient Notes add-only without write authority, and the modernized side proves the clinician can read seeded patient messages and create a temporary message but receives 403 for status, content, assignment, reply, archive, and hard-delete operations before admin cleanup. Slice 186 verifies mutation-specific document authorization on both targets; the shared ACL matrix proves Administrators have Documents write and Documents Delete write while Clinicians have permanent Documents add-only without write/delete authority, and the modernized side proves the clinician can read existing document facts and file a temporary document but receives 403 for metadata, content, sign-off, archive, and hard-delete operations before admin cleanup. Slice 185 verifies mutation-specific encounter authorization on both targets; the shared ACL matrix proves Administrators have Authorize Any Encounter write while Clinicians only receive a temporary Authorize Any Encounter view grant, and the modernized side proves the clinician can read encounter search/detail but receives 403 for create, update, sign, and delete before the temporary grant is revoked. Slice 184 verifies mutation-specific patient authorization on both targets; the shared ACL matrix proves Administrators and Front Office have Demographics write while Clinicians have Demographics add-only but not write, and the modernized side proves a clinician can read and register a temporary patient but receives 403 for existing chart contact, demographics, and delete operations while admin can clean up the temporary patient. Slice 183 verifies mutation-specific procedure authorization on both targets; the shared ACL matrix proves Administrators have Lab Results write and Sign Lab Results write while Clinicians have Lab Results add-only but not write/sign, and the modernized side proves a clinician can read and create procedure orders/reports but receives 403 for complete, sign, and delete while admin can complete, sign, and delete the temporary order tree. Slice 182 verifies ACL-backed procedure authorization on both targets; the legacy side proves the ACL matrix grants Lab Results access to Administrators and not Front Office, while the modernized side proves a front-desk demo session authenticates but receives 403 for procedure result retrieval, order-catalog retrieval, and procedure-order create, and an admin session still loads procedure API data and the Procedures UI. Slice 181 verifies ACL-backed billing authorization on both targets; the legacy side proves the ACL matrix grants Billing access to Administrators and Back Office but not Front Office, while the modernized side proves a front-desk demo session authenticates but receives 403 for billing retrieval, statement batch, and billing-line create, and an admin session still loads billing API data and the Fees UI. Slice 180 verifies ACL-backed patient message authorization on both targets; the legacy side proves the ACL matrix grants Patient Notes access to Administrators and not Front Office, while the modernized side proves a front-desk demo session authenticates but receives 403 for message list/create, and an admin session still loads message API data and the Messages UI. Slice 179 verifies ACL-backed patient document authorization on both targets; the legacy side proves the ACL matrix grants Documents access to Administrators and not Front Office, while the modernized side proves a front-desk demo session authenticates but receives 403 for document list/content/create, and an admin session still loads document API data and the Documents UI. Slice 178 verifies ACL-backed patient chart authorization on both targets; the legacy side proves the ACL matrix grants Demographics access to Administrators and Front Office, while the modernized side proves both admin and front-desk sessions can load patient search/chart API data and the Patient/Client UI. Slice 177 verifies ACL-backed encounter authorization on both targets; the legacy side proves the ACL matrix grants Authorize Any Encounter access to Administrators and not Front Office, while the modernized side proves a front-desk demo session authenticates but receives 403 for encounter search and mutation, and an admin session still loads encounter API data and the Encounters UI. Slice 176 verifies ACL-backed appointment authorization on both targets; the legacy side proves the ACL matrix grants Appointment access to Administrators and Front Office, while the modernized side proves the seeded front-desk demo session authenticates, carries Front Office membership, and loads appointment search/detail data through the API and Calendar UI. Slice 175 verifies ACL-backed clinical-list authorization on both targets; the legacy side proves the ACL matrix grants Medical/History access to Administrators and not Front Office, while the modernized side proves a front-desk demo session authenticates but receives 403 for clinical-list retrieval and allergy mutation, and an admin session still loads clinical-list data and the Lists UI. Slice 174 verifies ACL-backed operational reports authorization on both targets; the legacy side proves the ACL matrix grants Patient Report access to Administrators and not Front Office, while the modernized side proves a front-desk demo session authenticates but receives 403 for operational report JSON and CSV export, and an admin session still loads report JSON, CSV, and the Reports UI. Slice 173 verifies ACL-backed administration authorization on both targets; the legacy side proves the ACL matrix grants `admin:acl write` to Administrators and not Front Office, while the modernized side proves a front-desk demo session authenticates but receives 403 for administration directory and mutation access, and an admin session still loads the directory and Admin UI. Earlier workflow plans remain stable for the implemented patient, scheduling, encounter, document, revenue-cycle, lab/procedure, administration, ACL, reporting, and clinical-list slices.
