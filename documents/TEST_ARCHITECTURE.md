...
For the eighty-third modernized encounter document move readiness slice, compare the current side-by-side readiness plan with:

```powershell
npm run compare -- --left-target legacy-openemr --right-target modernized-openemr --plan slice-83-encounter-document-move-readiness
```
