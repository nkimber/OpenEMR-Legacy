...
Read this when working on the parity test harness, database/API/UI/workflow test layers, named run plans, reset strategy, test-run artifacts, comparison artifacts, Workbench test orchestration, or future side-by-side modernized target testing.
...
| `TEST_ARCHITECTURE.md` | Defines the TypeScript/Playwright parity test harness, database/HTTP/UI/workflow layers, named run plans, reset modes, artifact model, comparison runner, Workbench orchestration, and future modernized target strategy. | Writing or running parity tests, adding test suites or plans, changing reset behavior, integrating tests with the Workbench, or preparing side-by-side comparison. |
