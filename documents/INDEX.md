...
Read this when working on the installed original OpenEMR baseline, Docker Compose runtime, source checkout, smoke test, native PHPUnit runner, or environment verification.
...
Read this when working on the native PHPUnit lane, parity test harness, database/API/UI/workflow test layers, named run plans, reset strategy, test-run artifacts, comparison artifacts, Workbench test orchestration, or future side-by-side modernized target testing.

It explains how the legacy-native PHPUnit lane is exposed, how `parity-tests/` is structured, and how parity tests should remain reusable across the legacy and modernized targets.
...
| `LEGACY_OPENEMR_BASELINE.md` | Documents the installed legacy OpenEMR baseline, pinned source/image versions, Docker runtime, local URLs, smoke test, native PHPUnit runner, and verified status. | Starting, stopping, testing, resetting, inspecting, or changing the original OpenEMR baseline. |
...
| `TEST_ARCHITECTURE.md` | Defines the native PHPUnit lane, TypeScript/Playwright parity test harness, database/HTTP/UI/workflow layers, named run plans, reset modes, artifact model, comparison runner, Workbench orchestration, and future modernized target strategy. | Writing or running native or parity tests, adding test suites or plans, changing reset behavior, integrating tests with the Workbench, or preparing side-by-side comparison. |
