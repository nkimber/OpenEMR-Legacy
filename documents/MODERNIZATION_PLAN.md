...
- ERA/EOB import, payer remittance reconciliation, refund workflows beyond focused patient-refund reversal, audit history, batch posting, claim adjudication state transitions, and statement delivery workflows remain future revenue-cycle slices. Focused patient payment capture is covered by Slice 58, and printable patient statement generation is covered by Slice 59.
...
- Refund workflows beyond focused patient-refund reversal, online card processing integration, statement delivery, collections, and revenue-cycle audit history remain future billing slices. Printable patient statement generation is covered by Slice 59, deterministic statement PDF export is covered by Slice 60, and statement batch candidate readiness is covered by Slice 61.
...
- The three-hundred-forty-third implementation slice extends normalized database payload attachments into the Slice 82 encounter document metadata suite. It records the MOD-PAT-0001 anchor patient, encounter 1000013, baseline encounter document list, temporary text attachment, original Medical Record metadata, refiled Advance Directive metadata, legacy Documents category rendering facts, modernized encounter-detail API metadata rows, Encounters attached-document metadata UI anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 82 comparison.
- The three-hundred-forty-fourth implementation slice extends normalized database payload attachments into the Slice 83 encounter document move suite. It records the MOD-PAT-0001 anchor patient, source encounter 1000013, target encounter 1000011, baseline source/target encounter document lists, temporary text attachment, source-to-target movement facts, legacy Documents category rendering facts, modernized source/target encounter-detail API rows, Encounters attached-document move UI anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 83 comparison.
- The three-hundred-forty-fifth implementation slice extends normalized database payload attachments into the Slice 84 encounter document content replacement suite. It records the MOD-PAT-0001 anchor patient, encounter 1000013, baseline encounter document list, temporary original text attachment, replacement text payload, original/replaced content and revision facts, legacy Documents category rendering facts, modernized encounter-detail API replacement rows, Encounters attached-document replacement UI anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 84 comparison.
- The three-hundred-forty-sixth implementation slice extends normalized database payload attachments into the Slice 85 encounter document archive/restore suite. It records the MOD-PAT-0001 anchor patient, encounter 1000013, baseline encounter document list, temporary text attachment, archived hidden-content state, restored active document facts, legacy Documents category rendering facts, modernized active/archived encounter-detail API rows, Encounters attached-document Archive/Restore UI anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 85 comparison.
- The three-hundred-forty-seventh implementation slice extends normalized database payload attachments into the Slice 86 encounter document lifecycle timeline suite. It records the MOD-PAT-0001 anchor patient, encounter 1000013, baseline encounter document list, temporary text attachment, filed/current-version/review-pending/active create facts, review-approved sign-off facts, archived lifecycle facts, restored active lifecycle facts, legacy Documents category rendering facts, modernized encounter-detail lifecycle API rows, Encounters attached-document lifecycle UI anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 86 comparison.
- The three-hundred-forty-eighth implementation slice extends normalized database payload attachments into the Slice 87 encounter external-link document suite. It records the MOD-PAT-0001 anchor patient, encounter 1000013, baseline encounter document list, temporary URL-backed `text/uri-list` document payload, `web_url` storage metadata, legacy Documents category rendering facts, modernized encounter-detail API external-link projection, Encounters attached-document Open Link UI anchors, archive state, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 87 comparison.
- The three-hundred-forty-ninth implementation slice extends normalized database payload attachments into the Slice 88 patient image document preview suite. It records the MOD-PAT-0001 anchor patient, baseline document counts, temporary SVG binary document payload, `image/svg+xml` storage and preview metadata, legacy Documents category rendering facts, modernized Documents card and viewer inline image UI anchors, byte-preserving download facts, archive state, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 88 comparison.
- The three-hundred-fiftieth implementation slice extends normalized database payload attachments into the Slice 89 patient image document thumbnail suite. It records the MOD-PAT-0001 anchor patient, baseline document counts, temporary SVG binary document payload, `image/svg+xml` storage and thumbnail metadata, legacy Documents category rendering facts, modernized Documents card thumbnail image anchors, archive state, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 89 comparison.
- The three-hundred-fifty-first implementation slice extends normalized database payload attachments into the Slice 90 patient PDF document inline-preview suite. It records the MOD-PAT-0001 anchor patient, baseline document counts, temporary PDF binary document payload, `application/pdf` storage and inline-preview metadata, legacy Documents category rendering facts, modernized Documents card and viewer iframe UI anchors, byte-preserving download facts, archive state, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 90 comparison.
- The three-hundred-fifty-second implementation slice extends normalized database payload attachments into the Slice 91 patient document lifecycle timeline suite. It records the MOD-PAT-0001 anchor patient, baseline document counts, temporary text document payload, filed/current-version/review-pending/active create facts, review-approved sign-off facts, archived lifecycle facts, restored active lifecycle facts, legacy Documents category rendering facts, modernized Documents lifecycle API/card/viewer anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 91 comparison.
- The three-hundred-fifty-third implementation slice extends normalized database payload attachments into the Slice 92 patient scanned attachment suite. It records the MOD-PAT-0001 anchor patient, baseline document counts, temporary scanned PDF binary document payload, `application/pdf` storage and preview metadata, scan-readiness facts, legacy Documents category rendering facts, modernized Documents card and viewer scan metadata anchors, archive state, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 92 comparison.
- The three-hundred-fifty-fourth implementation slice extends normalized database payload attachments into the Slice 93 appointment reschedule suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary future appointment create payload, rescheduled title/date/time/duration/status/room facts, legacy appointment edit rendering facts, modernized Calendar list/detail rendering anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 93 comparison.
- The three-hundred-fifty-fifth implementation slice extends normalized database payload attachments into the Slice 94 appointment arrival suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary scheduled appointment create payload, arrived status/title facts, legacy appointment edit rendering facts, modernized Calendar arrival action/rendering anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 94 comparison.
- The three-hundred-fifty-sixth implementation slice extends normalized database payload attachments into the Slice 95 appointment check-out suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary scheduled appointment create payload, arrived `@` state, checked-out `>` state, legacy appointment edit rendering facts, modernized Calendar arrival/check-out action anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 95 comparison.
- The three-hundred-fifty-seventh implementation slice extends normalized database payload attachments into the Slice 96 appointment no-show suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary scheduled appointment create payload, no-show `?` status/title facts, legacy appointment edit rendering facts, modernized Calendar no-show action/rendering anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 96 comparison.
- The three-hundred-fifty-eighth implementation slice extends normalized database payload attachments into the Slice 97 appointment category suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary Preventive Care Services appointment create payload, New Patient category update facts, legacy appointment edit rendering facts, modernized Calendar category selector/rendering anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 97 comparison.
- The three-hundred-fifty-ninth implementation slice extends normalized database payload attachments into the Slice 98 appointment pending-status suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary scheduled appointment create payload, pending `~` status/title facts, legacy appointment edit rendering facts, modernized Calendar pending-status selector/rendering anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 98 comparison.
- The three-hundred-sixtieth implementation slice extends normalized database payload attachments into the Slice 99 appointment provider reassignment suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary scheduled appointment create payload, original and alternate provider IDs, reassigned title/provider facts, legacy appointment edit rendering facts, modernized Calendar provider selector/rendering anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 99 comparison.
- The three-hundred-sixty-first implementation slice extends normalized database payload attachments into the Slice 100 appointment facility reassignment suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary scheduled appointment create payload, original and reassigned service/billing facility IDs, reassigned title/facility facts, legacy appointment edit rendering facts, modernized Calendar facility selector/rendering anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs. The slice also aligns the modernized Calendar save path with legacy behavior by carrying the billing facility along when service and billing facilities originally matched and the user changes only the service facility, while preserving the separate Slice 101 billing-location edit path.
- The three-hundred-sixty-second implementation slice extends normalized database payload attachments into the Slice 101 appointment billing-location reassignment suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary scheduled appointment create payload, preserved service facility ID, original and reassigned billing facility IDs, reassigned title/billing-location facts, legacy appointment edit rendering facts, modernized Calendar billing facility selector/rendering anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 101 comparison.
- The three-hundred-sixty-third implementation slice extends normalized database payload attachments into the Slice 102 appointment comments suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary scheduled appointment create payload, initial and updated comments, updated title/comment facts, legacy appointment edit rendering facts, modernized Calendar comments control/rendering anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 102 comparison.
- The three-hundred-sixty-fourth implementation slice extends normalized database payload attachments into the Slice 103 appointment recurrence metadata suite. It records the MOD-PAT-0003 anchor patient, baseline appointment counts, temporary recurring appointment create payload, initial weekly cadence/end-date facts, updated two-week cadence/end-date facts, legacy appointment repeat-control rendering facts, modernized Calendar repeat-control/rendering anchors, and final cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 103 comparison.
- The three-hundred-sixty-fifth implementation slice extends normalized database payload attachments into the Slice 104 appointment recurring-series suite. It records the MOD-PAT-0003 anchor patient, seeded recurrence search window, expected generated occurrence dates and numbers, normalized recurring-series workflow rows, modernized Calendar generated-occurrence rendering anchors, and legacy workflow projection facts as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 104 comparison.
- The three-hundred-sixty-sixth implementation slice extends normalized database payload attachments into the Slice 105 appointment recurrence-exceptions suite. It records the MOD-PAT-0013 anchor patient, seeded skipped occurrence date, expected generated occurrence dates and numbers after exception filtering, normalized recurrence-exdate workflow rows, modernized Calendar skipped-date rendering anchors, and legacy workflow projection facts as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 105 comparison.
- The three-hundred-sixty-seventh implementation slice extends normalized database payload attachments into the Slice 106 appointment occurrence-cancel suite. It records the MOD-PAT-0013 anchor patient, seeded recurring-series state, generated occurrence selected for cancellation, recurrence-exdate mutation, hidden cancelled occurrence expansion, modernized Calendar skip-occurrence rendering anchors, and cleanup restoration as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 106 comparison.
- The three-hundred-sixty-eighth implementation slice extends normalized database payload attachments into the Slice 107 appointment occurrence-restore suite. It records the MOD-PAT-0013 anchor patient, generated occurrence selected for restoration, temporary skipped occurrence state, recurrence-exdate removal, restored occurrence expansion, modernized Calendar restore-occurrence rendering anchors, and cleanup restoration as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 107 comparison.
- The three-hundred-sixty-ninth implementation slice extends normalized database payload attachments into the Slice 108 appointment occurrence-reschedule suite. It records the MOD-PAT-0013 anchor patient, generated occurrence selected for movement, recurring-root exception mutation, standalone moved appointment, modernized Calendar reschedule rendering anchors, and cleanup restoration as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 108 comparison.
- The three-hundred-seventieth implementation slice extends normalized database payload attachments into the Slice 109 appointment recurrence exception-list edit suite. It records the MOD-PAT-0013 anchor patient, recurring root, seeded skipped date, added skipped date, edited occurrence expansion, modernized Calendar skipped-date edit rendering anchors, and cleanup restoration as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 109 comparison.
- The three-hundred-seventy-first implementation slice extends normalized database payload attachments into the Slice 110 appointment series root update suite. It records the MOD-PAT-0013 anchor patient, recurring root title/time update, generated occurrence propagation, seeded exception preservation, modernized Calendar root edit rendering anchors, and cleanup restoration as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 110 comparison.
- The three-hundred-seventy-second implementation slice extends normalized database payload attachments into the Slice 111 appointment series root metadata suite. It records the MOD-PAT-0013 anchor patient, recurring root provider/facility/billing/category/status/room/comment metadata update, generated occurrence propagation, seeded exception preservation, modernized Calendar generated-occurrence rendering anchors, and cleanup restoration as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 111 comparison.
- The three-hundred-seventy-third implementation slice extends normalized database payload attachments into the Slice 112 appointment monthly recurrence suite. It records the MOD-PAT-0003 anchor patient, temporary monthly recurring root creation, every-two-month update, generated occurrence expansion, legacy repeat-control rendering facts, modernized Calendar monthly recurrence rendering anchors, and cleanup deletion as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 112 comparison.
- The three-hundred-seventy-fourth implementation slice extends normalized database payload attachments into the Slice 113 appointment recurrence unit matrix suite. It records the MOD-PAT-0003 anchor patient, temporary daily, workday, and yearly recurring root creation, generated occurrence expansion, legacy repeat-control rendering facts, modernized Calendar recurrence-unit rendering anchors, and cleanup deletion as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 113 comparison.
- The three-hundred-seventy-fifth implementation slice extends normalized database payload attachments into the Slice 114 appointment days-of-week recurrence suite. It records the MOD-PAT-0003 anchor patient, temporary Monday/Wednesday/Friday recurring root creation, weekday selection metadata, generated occurrence expansion, legacy REPEAT_DAYS rendering facts, modernized Calendar weekday rendering anchors, and cleanup deletion as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 114 comparison.
- The three-hundred-seventy-sixth implementation slice extends normalized database payload attachments into the Slice 115 appointment monthly repeat-on recurrence suite. It records the MOD-PAT-0003 anchor patient, temporary second-Tuesday and last-Friday recurring root creation, repeat-on ordinal/weekday metadata, generated occurrence expansion, legacy REPEAT_ON rendering facts, modernized Calendar repeat-on rendering anchors, and cleanup deletion as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 115 comparison.
- The three-hundred-seventy-seventh implementation slice extends normalized database payload attachments into the Slice 116 appointment series recurrence update suite. It records the MOD-PAT-0013 seeded recurring root, original every-two-weeks cadence, temporary every-three-weeks update, recurrence end-date change, skipped-date preservation, generated occurrence expansion, legacy repeat-control rendering facts, modernized Calendar recurrence update anchors, and cleanup restoration as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 116 comparison.
- The three-hundred-seventy-eighth implementation slice extends normalized database payload attachments into the Slice 117 appointment provider overlap suite. It records the MOD-PAT-0003 and MOD-PAT-0004 seeded patient anchors, shared provider/date/time precondition, temporary same-provider overlapping appointment rows, overlap query results, legacy appointment rendering facts, modernized provider-overlap rendering anchors, count deltas, and cleanup deletion as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 117 comparison.
- The three-hundred-seventy-ninth implementation slice extends normalized database payload attachments into the Slice 118 appointment patient overlap suite. It records the MOD-PAT-0003 seeded patient anchor, primary and secondary provider IDs, shared date/time precondition, temporary same-patient overlapping appointment rows, overlap query results, legacy appointment rendering facts, modernized patient-overlap rendering anchors, count deltas, and cleanup deletion as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 118 comparison.
- The three-hundred-eightieth implementation slice extends normalized database payload attachments into the Slice 119 appointment room overlap suite. It records the MOD-PAT-0003 and MOD-PAT-0004 seeded patient anchors, primary and secondary provider IDs, shared room/date/time precondition, temporary same-room overlapping appointment rows, overlap query results, legacy appointment rendering facts, modernized room-overlap rendering anchors, count deltas, and cleanup deletion as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 119 comparison.
- The three-hundred-eighty-first implementation slice extends normalized database payload attachments into the Slice 120 appointment reminders suite. It records the MOD-PAT-0191 seeded patient anchor, future preventive-care appointment, contact-consent and reminder-channel derivation, legacy appointment rendering facts, and modernized reminder-detail rendering anchors as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 120 comparison.
- The three-hundred-eighty-second implementation slice extends normalized database payload attachments into the Slice 121 encounter co-signature suite. It records the MOD-PAT-0002 seeded patient anchor, temporary encounter, ordered admin/provider signature rows, lock/hash/amendment facts, legacy patient-summary rendering facts, modernized sign-off panel rendering anchors, signature deletion, and encounter cleanup as path-backed JSON evidence on both legacy and modernized runs while preserving a matched side-by-side Slice 121 comparison.
- The three-hundred-eighty-third implementation slice extends normalized database payload attachments into the Slice 122 encounter document revision suite. It records the MOD-PAT-0001 seeded patient anchor, encounter 1000013, two encounter-attached current-version documents, revision timestamps, revision-hash parity, modernized encounter-detail API document projection, and modernized attached-document card rendering anchors as path-backed JSON evidence while preserving a matched side-by-side Slice 122 comparison.
- The three-hundred-eighty-fourth implementation slice extends normalized database payload attachments into the Slice 123 encounter document replacement revision suite. It records the MOD-PAT-0001 seeded patient anchor, encounter 1000013, temporary original document creation, replacement content and hash movement, preserved single-current-version metadata, legacy Documents rendering facts, modernized replacement form/API/card rendering anchors, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 123 comparison.
- The three-hundred-eighty-fifth implementation slice extends normalized database payload attachments into the Slice 126 encounter scanned attachment suite. It records the MOD-PAT-0001 seeded patient anchor, encounter 1000013, temporary scanned application/pdf payload, normalized PDF preview metadata, scan/OCR-readiness facts, legacy Documents rendering facts, modernized encounter detail API/card rendering anchors, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 126 comparison.
- The three-hundred-eighty-sixth implementation slice extends normalized database payload attachments into the Slice 127 encounter binary document content replacement suite. It records the MOD-PAT-0001 seeded patient anchor, encounter 1000013, temporary original PDF attachment, replacement PDF bytes, preview/download metadata, revision hash movement, legacy Documents rendering facts, modernized binary replacement form/API/card/download anchors, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 127 comparison.
- The three-hundred-eighty-seventh implementation slice extends normalized database payload attachments into the Slice 128 patient binary document content replacement suite. It records the MOD-PAT-0001 seeded patient anchor, temporary original PDF document, replacement PDF bytes, preview/download metadata, revision hash movement, legacy Documents rendering facts, modernized Documents binary replacement form/API/card/download anchors, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 128 comparison.
- The three-hundred-eighty-eighth implementation slice extends normalized database payload attachments into the Slice 129 procedure result correction suite. It records the MOD-PAT-0009 seeded patient anchor, temporary encounter/order/report/result creation, corrected result value/range/abnormal/status movement, legacy Procedure Results rendering facts, modernized Procedures correction UI facts, patient procedure projection, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 129 comparison.

- The three-hundred-eighty-ninth implementation slice extends normalized database payload attachments into the Slice 130 procedure specimen suite. It records the MOD-PAT-0009 seeded patient anchor, temporary encounter/order/report/result creation, collected date and specimen number facts, legacy Procedure Results rendering facts, modernized Procedures specimen UI facts, patient procedure projection, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 130 comparison.
- The three-hundred-ninetieth implementation slice extends normalized database payload attachments into the Slice 131 procedure specimen detail suite. It records the MOD-PAT-0009 seeded patient anchor, temporary encounter/order/specimen-detail creation, specimen identifier/accession/type/collection/location/volume/condition facts, legacy database projection facts, modernized Procedures specimen-detail UI facts, patient procedure projection, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 131 comparison.
- The three-hundred-ninety-first implementation slice extends normalized database payload attachments into the Slice 132 procedure order correction suite. It records the MOD-PAT-0009 seeded patient anchor, temporary encounter/order creation, initial and corrected order metadata, follow-up report/result projection, legacy Procedure Results rendering facts, modernized Procedures correction UI facts, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 132 comparison.
- The three-hundred-ninety-second implementation slice extends normalized database payload attachments into the Slice 133 procedure report correction suite. It records the MOD-PAT-0009 seeded patient anchor, temporary encounter/order/report/result creation, corrected collected-date/report-date/specimen/status/review/notes facts, preserved linked result projection, legacy Procedure Results rendering facts, modernized Procedures report-correction UI facts, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 133 comparison.
- The three-hundred-ninety-third implementation slice extends normalized database payload attachments into the Slice 134 procedure report sign-off suite. It records the MOD-PAT-0009 seeded patient anchor, temporary encounter/order/report/result creation, pending-review report state, reviewed-by/reviewed-at sign-off facts, preserved linked result projection, legacy Procedure Results rendering facts, modernized Procedures sign-off UI facts, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 134 comparison.
- The three-hundred-ninety-fourth implementation slice extends normalized database payload attachments into the Slice 135 procedure report review queue suite. It records the MOD-PAT-0009 seeded patient anchor, temporary encounter/order/report creation, unreviewed queue membership, reviewed queue movement after sign-off, legacy review queue rendering facts, modernized Reports queue rendering facts, and cleanup removal from both queues as path-backed JSON evidence while preserving a matched side-by-side Slice 135 comparison.
- The three-hundred-ninety-fifth implementation slice extends normalized database payload attachments into the Slice 136 procedure report review queue filter suite. It records the MOD-PAT-0009 seeded patient anchor, temporary encounter/order/report creation, patient/date filter inclusion, outside-date exclusion, reviewed filter movement after sign-off, legacy filtered queue rendering facts, modernized Reports filter rendering facts, and cleanup removal from filtered queues as path-backed JSON evidence while preserving a matched side-by-side Slice 136 comparison.
- The three-hundred-ninety-sixth implementation slice extends normalized database payload attachments into the Slice 137 procedure report review queue provider filter suite. It records the MOD-PAT-0009 seeded patient anchor, temporary encounter/order/report creation, ordering-provider filter inclusion, alternate-provider exclusion, reviewed provider-filter movement after sign-off, legacy provider-filtered queue rendering facts, modernized Reports provider filter rendering facts, and cleanup removal from provider-filtered queues as path-backed JSON evidence while preserving a matched side-by-side Slice 137 comparison.
- The three-hundred-ninety-seventh implementation slice extends normalized database payload attachments into the Slice 138 procedure report review queue lab filter suite. It records the MOD-PAT-0009 seeded patient anchor, temporary lab-provider and alternate-lab-provider creation, temporary encounter/order/report creation, processing-lab filter inclusion, alternate-lab exclusion, reviewed lab-filter movement after sign-off, legacy lab-filtered queue rendering facts, modernized Reports lab filter rendering facts, and cleanup removal from lab-filtered queues plus temporary lab providers as path-backed JSON evidence while preserving a matched side-by-side Slice 138 comparison.
- The three-hundred-ninety-eighth implementation slice extends normalized database payload attachments into the Slice 139 procedure lab provider catalog suite. It records the MOD-PAT-0009 seeded patient anchor, permanent reviewed report ownership by Pacific Women's Health Laboratory, outside-lab exclusion, legacy reviewed queue rendering facts, and modernized Reports lab catalog rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 139 comparison.
- The three-hundred-ninety-ninth implementation slice extends normalized database payload attachments into the Slice 140 procedure lab provider directory suite. It records the permanent five-provider gold-data directory, active/inactive provider counts, per-provider order/report/future-order counts, legacy Procedure Providers rendering facts, and modernized Reports directory rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 140 comparison.
- The four-hundredth implementation slice extends normalized database payload attachments into the Slice 141 procedure lab provider lifecycle suite. It records temporary provider create inputs, active row/rendering facts, deactivated include-inactive row/rendering facts, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 141 comparison.
- The four-hundred-first implementation slice extends normalized database payload attachments into the Slice 142 procedure lab provider configuration suite. It records initial provider configuration, updated SFTP transport settings, browser/API rendering facts, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 142 comparison.
- The four-hundred-second implementation slice extends normalized database payload attachments into the Slice 144 procedure lab provider address-book suite. It records temporary order-service organizations, provider name derivation from linked organizations, relinked provider state, browser/API rendering facts, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 144 comparison.
- The four-hundred-third implementation slice extends normalized database payload attachments into the Slice 145 procedure order catalog suite. It records the permanent gold-data catalog root, provider groups, Pacific Women's Health Laboratory anchor panels, modernized API counts, legacy AJAX rendering facts, and modernized Reports catalog rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 145 comparison.
- The four-hundred-fourth implementation slice extends normalized database payload attachments into the Slice 147 procedure order catalog lifecycle suite. It records temporary catalog item create inputs, created and updated rows, rendered create/update facts, deactivation facts, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 147 comparison.
- The four-hundred-fifth implementation slice extends normalized database payload attachments into the Slice 148 procedure vendor compendium import suite. It records temporary import group inputs, first and second PathGroup CSV payloads, created/updated/deactivated/reactivated order and result rows, rendering facts, and cleanup deletion as path-backed JSON evidence while preserving a matched side-by-side Slice 148 comparison.
- The four-hundred-sixth implementation slice extends normalized database payload attachments into the Slice 149 procedure order queue suite. It records temporary encounter/order/report inputs, ready-to-send queue membership, ready rendering facts, reported queue transition, reported rendering facts, and cleanup count restoration as path-backed JSON evidence while preserving a matched side-by-side Slice 149 comparison.
- The four-hundred-seventh implementation slice extends normalized database payload attachments into the Slice 151 procedure order transmit suite. It records temporary encounter/order inputs, ready-to-send queue precondition, transmit action state, sent-awaiting-results queue transition, browser/API rendering facts, and cleanup count restoration as path-backed JSON evidence while preserving a matched side-by-side Slice 151 comparison.
...
- The four-hundred-twelfth implementation slice extends normalized database payload attachments into the Slice 158 patient-message update metadata suite. It records temporary pnotes-compatible message creation, edited title/body state, OpenEMR-compatible updated-by and updated-at metadata, browser/API rendering facts, soft-delete state, and cleanup count restoration as path-backed JSON evidence while preserving a matched side-by-side Slice 158 comparison.
- The four-hundred-thirteenth implementation slice extends normalized probe payload attachments into the Slice 159 admin login readiness suite. It records redacted configured-credential preconditions, legacy HTTP success and invalid-password rejection markers, modernized API success and rejection responses, and modernized Admin-page login rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 159 comparison.
- The four-hundred-fourteenth implementation slice extends normalized probe payload attachments into the Slice 160 admin login audit readiness suite. It records audit-table boundary preconditions, legacy login log rows, modernized auth audit event rows, success/failure row summaries, and modernized Admin-page login audit rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 160 comparison.

- The four-hundred-fifteenth implementation slice extends normalized probe payload attachments into the Slice 161 admin session readiness suite. It records redacted session preconditions, legacy session cookie and logout markers, modernized login/current-session/logout API facts, auth session persistence rows, and modernized Admin-page session rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 161 comparison.
- The four-hundred-sixteenth implementation slice extends normalized probe payload attachments into the Slice 162 admin audit protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API blocking facts, authenticated legacy/API visibility facts, ended-session rejection facts, and modernized Admin-page audit rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 162 comparison.
- The four-hundred-seventeenth implementation slice extends normalized probe payload attachments into the Slice 163 admin directory protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API directory blocking facts, unauthenticated modernized mutation rejection facts, authenticated legacy/API directory visibility facts, and modernized Admin-page directory rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 163 comparison.
- The four-hundred-eighteenth implementation slice extends normalized probe payload attachments into the Slice 164 operational reports protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API report blocking facts, unauthenticated modernized CSV export rejection facts, authenticated legacy/API report visibility facts, authenticated CSV export facts, and modernized Reports-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 164 comparison.
- The four-hundred-nineteenth implementation slice extends normalized probe payload attachments into the Slice 165 patient chart protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API patient chart blocking facts, authenticated legacy/API patient search and chart visibility facts, and modernized Patient/Client-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 165 comparison.

- The four-hundred-twentieth implementation slice extends normalized probe payload attachments into the Slice 166 clinical-list protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API clinical-list blocking facts, unauthenticated modernized mutation rejection facts, authenticated legacy/API list visibility facts, and modernized Lists-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 166 comparison.

- The four-hundred-twenty-first implementation slice extends normalized probe payload attachments into the Slice 167 appointment protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API appointment blocking facts, unauthenticated modernized create rejection facts, authenticated legacy/API appointment search/detail visibility facts, and modernized Calendar-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 167 comparison.

- The four-hundred-twenty-second implementation slice extends normalized probe payload attachments into the Slice 168 encounter protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API encounter blocking facts, unauthenticated modernized create rejection facts, authenticated legacy/API encounter search/detail visibility facts, and modernized Encounters-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 168 comparison.

- The four-hundred-twenty-third implementation slice extends normalized probe payload attachments into the Slice 169 document protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API document list/content/create blocking facts, authenticated legacy/API document visibility facts, and modernized Documents-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 169 comparison.
- The four-hundred-twenty-fourth implementation slice extends normalized probe payload attachments into the Slice 170 message protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API message list/create blocking facts, authenticated legacy/API message visibility facts, and modernized Messages-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 170 comparison.
- The four-hundred-twenty-fifth implementation slice extends normalized probe payload attachments into the Slice 171 billing protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API billing summary/batch/create blocking facts, authenticated legacy/API fee-sheet visibility facts, and modernized Fees-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 171 comparison.
- The four-hundred-twenty-sixth implementation slice extends normalized probe payload attachments into the Slice 172 procedure protection readiness suite. It records redacted protection preconditions, unauthenticated legacy/API procedure result/catalog/create blocking facts, authenticated legacy/API procedure result visibility facts, and modernized Procedures-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 172 comparison.
- The four-hundred-twenty-seventh implementation slice extends normalized probe payload attachments into the Slice 173 administration authorization-policy readiness suite. It records redacted ACL preconditions, legacy ACL matrix rendering facts, modernized front-desk forbidden directory/mutation facts, admin directory allow facts, and modernized Admin-page retry rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 173 comparison.
- The four-hundred-twenty-eighth implementation slice extends normalized probe payload attachments into the Slice 174 operational reports authorization-policy readiness suite. It records redacted ACL preconditions, legacy operational report rendering facts, modernized front-desk forbidden report/export facts, admin report/export allow facts, and modernized Reports-page retry rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 174 comparison.
- The four-hundred-twenty-ninth implementation slice extends normalized probe payload attachments into the Slice 175 clinical-list authorization-policy readiness suite. It records redacted ACL preconditions, legacy patient-summary clinical-list rendering facts, modernized front-desk forbidden clinical-list retrieval and allergy-mutation facts, admin clinical-list allow facts, and modernized Lists-page retry rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 175 comparison.
- The four-hundred-thirtieth implementation slice extends normalized probe payload attachments into the Slice 176 appointment authorization-policy readiness suite. It records redacted ACL preconditions, legacy appointment edit rendering facts, modernized front-desk login, appointment search, and appointment detail allow facts, and modernized Calendar-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 176 comparison.
- The four-hundred-thirty-first implementation slice extends normalized probe payload attachments into the Slice 177 encounter authorization-policy readiness suite. It records redacted ACL preconditions, legacy encounter rendering facts, modernized front-desk forbidden encounter search and mutation facts, admin encounter search and detail allow facts, and modernized Encounters-page retry rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 177 comparison.
- The four-hundred-thirty-second implementation slice extends normalized probe payload attachments into the Slice 178 patient chart authorization-policy readiness suite. It records redacted Demographics ACL preconditions, legacy patient-summary rendering facts, modernized unauthenticated patient-search rejection facts, front-desk patient search/chart allow facts, admin chart allow facts, and modernized Patient/Client-page rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 178 comparison.
- The four-hundred-thirty-third implementation slice extends normalized probe payload attachments into the Slice 179 patient document authorization-policy readiness suite. It records redacted Documents ACL preconditions, legacy document-list rendering facts, modernized front-desk forbidden document list/content/create facts, admin document list/content allow facts, and modernized Documents-page retry rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 179 comparison.
- The four-hundred-thirty-fourth implementation slice extends normalized probe payload attachments into the Slice 180 patient message authorization-policy readiness suite. It records redacted Patient Notes ACL preconditions, legacy patient-notes rendering facts, modernized front-desk forbidden message list/create facts, admin message list allow facts, and modernized Messages-page retry rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 180 comparison.
- The four-hundred-thirty-fifth implementation slice extends normalized probe payload attachments into the Slice 181 billing authorization-policy readiness suite. It records redacted Billing ACL preconditions, legacy fee-sheet rendering facts, modernized front-desk forbidden billing summary, statement-batch, and billing-line create facts, admin billing summary allow facts, and modernized Fees-page retry rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 181 comparison.
- The four-hundred-thirty-sixth implementation slice extends normalized probe payload attachments into the Slice 182 procedure authorization-policy readiness suite. It records redacted Lab Results ACL preconditions, legacy procedure-result rendering facts, modernized front-desk forbidden procedure-result, order-catalog, and procedure-order create facts, admin procedure-result allow facts, and modernized Procedures-page retry rendering facts as path-backed JSON evidence while preserving a matched side-by-side Slice 182 comparison.
- The four-hundred-thirty-seventh implementation slice extends normalized probe payload attachments into the Slice 183 procedure mutation authorization-policy readiness suite. It records redacted Lab Results and Sign Lab Results ACL preconditions, legacy procedure-result rendering facts, clinician add-only procedure order/report creation facts, clinician Lab Results write and Sign Lab Results denial facts, admin status/sign/delete cleanup facts, and final cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 183 comparison.
- The four-hundred-thirty-eighth implementation slice extends normalized probe payload attachments into the Slice 184 patient mutation authorization-policy readiness suite. It records redacted Demographics ACL preconditions, legacy Demographics edit rendering facts, clinician add-only patient registration facts, clinician Demographics/contact/delete denial facts, admin delete cleanup facts, and final cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 184 comparison.
- The four-hundred-thirty-ninth implementation slice extends normalized probe payload attachments into the Slice 185 encounter mutation authorization-policy readiness suite. It records redacted Authorize Any Encounter ACL preconditions, legacy encounter rendering facts, temporary clinician view-grant facts, clinician read allow facts, clinician create/update/sign/delete denial facts, admin detail allow facts, and final grant cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 185 comparison.
- The four-hundred-fortieth implementation slice extends normalized probe payload attachments into the Slice 186 document mutation authorization-policy readiness suite. It records redacted Documents/Documents Delete ACL preconditions, legacy Documents rendering facts, modernized admin and clinician login facts, clinician read and add-only create facts, clinician metadata/content/sign/archive/delete denial facts, admin hard-delete cleanup facts, and final document-count cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 186 comparison.
- The four-hundred-forty-first implementation slice extends normalized probe payload attachments into the Slice 187 patient message mutation authorization-policy readiness suite. It records redacted Patient Notes ACL preconditions, legacy Patient Notes rendering facts, admin and clinician login facts, clinician read and add-only create facts, clinician status/content/assignment/reply/archive/delete denial facts, admin hard-delete cleanup facts, and final message-count cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 187 comparison.
- The four-hundred-forty-second implementation slice extends normalized probe payload attachments into the Slice 188 billing mutation authorization-policy readiness suite. It records redacted Billing ACL preconditions, legacy Fee Sheet rendering facts, admin login facts, temporary clinician Billing view-grant facts, clinician login and read facts, clinician billing-line/collections/claim/payment denial facts, grant cleanup facts, and final revenue-cycle count cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 188 comparison.
- The four-hundred-forty-third implementation slice extends normalized probe payload attachments into the Slice 189 appointment mutation authorization-policy readiness suite. It records redacted Appointment ACL preconditions, legacy appointment edit rendering facts, admin login facts, temporary clinician Appointment view-downgrade facts, clinician login and read facts, clinician create/update/status/recurrence-restore/occurrence-reschedule/delete denial facts, write-grant restoration facts, and final appointment-count cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 189 comparison.
- The four-hundred-forty-fourth implementation slice extends normalized probe payload attachments into the Slice 190 encounter amendment history readiness suite. It records anchor-patient preconditions, temporary encounter creation, signature and amendment-history rows, legacy patient-summary reachability, modernized API/UI rendering facts, signature cleanup, and final encounter/signature cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 190 comparison.
- The four-hundred-forty-fifth implementation slice extends normalized probe payload attachments into the Slice 191 patient duplicate detection readiness suite. It records anchor-patient/contact preconditions, temporary duplicate registration creation, database duplicate-candidate rows, legacy database-surface facts, modernized duplicate-search/chart/UI facts, and final temporary-patient cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 191 comparison.
- The four-hundred-forty-sixth implementation slice extends normalized probe payload attachments into the Slice 192 patient registration validation readiness suite. It records legacy PatientValidator/source-rule facts, invalid registration draft inputs, structured modernized API validation problems, modernized Patient/Client validation-panel facts, and no-row database guards as path-backed JSON evidence while preserving a matched side-by-side Slice 192 comparison.
- The four-hundred-forty-seventh implementation slice extends normalized probe payload attachments into the Slice 193 patient deceased-status readiness suite. It records anchor/original deceased-status preconditions, temporary deceased date/reason update facts, legacy demographics edit rendering facts, modernized patient chart API/UI deceased-status facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 193 comparison.
- The four-hundred-forty-eighth implementation slice extends normalized probe payload attachments into the Slice 194 patient guardian-contact readiness suite. It records anchor/original mother and guardian contact preconditions, temporary contact update facts, legacy demographics edit rendering facts, modernized patient chart API/UI Guardian Contact facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 194 comparison.
- The four-hundred-forty-ninth implementation slice extends normalized probe payload attachments into the Slice 195 patient guardian demographic/address readiness suite. It records anchor/original guardian detail preconditions, temporary sex/address/country/work-phone update facts, legacy demographics edit rendering facts, modernized patient chart API/UI Guardian Contact facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 195 comparison.
- The four-hundred-fiftieth implementation slice extends normalized probe payload attachments into the Slice 196 patient social-detail readiness suite. It records anchor/original demographics preconditions, temporary race/ethnicity/interpreter/family-size/income/homeless/review-date update facts, legacy demographics edit rendering facts, modernized patient chart API/UI demographics facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 196 comparison.
- The four-hundred-fifty-first implementation slice extends normalized probe payload attachments into the Slice 197 patient employer core readiness suite. It records anchor/original employer preconditions, temporary employer identity/address update facts, legacy demographics/employer rendering facts, modernized patient chart API/UI Employer facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 197 comparison.
- The four-hundred-fifty-second implementation slice extends normalized probe payload attachments into the Slice 198 patient provider assignment readiness suite. It records anchor/original primary-provider preconditions, temporary provider ID/name update facts, legacy demographics provider rendering facts, modernized patient chart API/UI Primary Provider facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 198 comparison.
- The four-hundred-fifty-third implementation slice extends normalized probe payload attachments into the Slice 199 patient care-team readiness suite. It records anchor/original care-team preconditions, temporary lead-member provider/facility/role update facts, legacy patient-summary care-team rendering facts, modernized patient chart API/UI Care Team facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 199 comparison.
- The four-hundred-fifty-fourth implementation slice extends normalized probe payload attachments into the Slice 200 patient multi-member care-team readiness suite. It records anchor/original care-team member preconditions, temporary two-member provider/facility/role update facts, legacy patient-summary care-team rendering facts, modernized patient chart API/UI Care Team facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 200 comparison.
- The four-hundred-fifty-fifth implementation slice extends normalized probe payload attachments into the Slice 201 patient contact-backed care-team readiness suite. It records anchor/original provider/contact care-team preconditions, temporary provider plus patient-contact update facts, legacy patient-summary care-team rendering facts, modernized patient chart API/UI Care Team facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 201 comparison.
- The four-hundred-fifty-sixth implementation slice extends normalized probe payload attachments into the Slice 202 patient history and lifestyle readiness suite. It records anchor/history/lifestyle preconditions, legacy history/lifestyle rendering facts, modernized patient chart API facts, and modernized Patient/Client history panel facts as path-backed JSON evidence while preserving a matched side-by-side Slice 202 comparison.
- The four-hundred-fifty-seventh implementation slice extends normalized probe payload attachments into the Slice 203 patient insurance subscriber readiness suite. It records seeded secondary subscriber preconditions, temporary tertiary subscriber create/update facts, legacy insurance browse rendering facts, modernized Patient/Client insurance panel facts, and cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 203 comparison.
- The four-hundred-fifty-eighth implementation slice extends normalized probe payload attachments into the Slice 204 patient portal account readiness suite. It records seeded CMS login and portal-account preconditions, legacy demographics edit rendering facts, modernized patient chart API facts, and modernized Patient/Client Portal Account panel facts as path-backed JSON evidence while preserving a matched side-by-side Slice 204 comparison.
- The four-hundred-fifty-ninth implementation slice extends normalized probe payload attachments into the Slice 205 patient portal reset readiness suite. It records original portal reset preconditions, temporary one-time reset issue/clear facts, legacy demographics edit rendering facts, modernized patient chart API facts, modernized Portal Account panel facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 205 comparison.
- The four-hundred-sixtieth implementation slice extends normalized probe payload attachments into the Slice 206 patient portal access readiness suite. It records original portal access preconditions, temporary revoke/grant facts, legacy demographics edit rendering facts, modernized patient chart API facts, modernized Portal Account panel facts, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 206 comparison.
- The four-hundred-sixty-first implementation slice extends normalized probe payload attachments into the Slice 207 patient portal authentication readiness suite. It records original account/access/reset preconditions, valid credential authentication, invalid password rejection, disabled-access rejection, pending-reset rejection, target-specific portal UI readiness rendering, and restore cleanup facts as path-backed JSON evidence while preserving a matched side-by-side Slice 207 comparison.
- The four-hundred-sixty-second implementation slice extends normalized probe payload attachments into the Slice 208 patient portal session readiness suite. It records original account/access/reset preconditions, portal login facts, modernized active-session read facts, modernized session logout facts, inactive reuse rejection facts, modernized end-session UI rendering, and legacy browser-cookie logout rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 208 comparison.
- The four-hundred-sixty-third implementation slice extends normalized probe payload attachments into the Slice 209 patient portal home readiness suite. It records enabled portal-account preconditions, authenticated home-summary identity, message-count, latest-message, upcoming-appointment facts, legacy portal home rendering, and modernized Portal home rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 209 comparison.
- The four-hundred-sixty-fourth implementation slice extends normalized probe payload attachments into the Slice 210 patient portal secure-message inbox readiness suite. It records anchor-patient preconditions, authenticated mailbox identity, seeded inbox message details, legacy portal Secure Messaging rendering, and modernized Portal Secure Messages rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 210 comparison.
- The four-hundred-sixty-fifth implementation slice extends normalized probe payload attachments into the Slice 211 patient portal secure-message compose readiness suite. It records anchor-patient preconditions, pre-compose mailbox state, cleanup-backed compose result, post-cleanup mailbox state, legacy portal Sent-folder rendering, and modernized Portal Sent-folder rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 211 comparison.
- The four-hundred-sixty-sixth implementation slice extends normalized probe payload attachments into the Slice 212 patient portal secure-message reply readiness suite. It records anchor-patient preconditions, seeded original-message state, cleanup-backed threaded reply result, post-cleanup mailbox state, legacy portal Sent-folder rendering, and modernized Portal reply/Sent-folder rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 212 comparison.
- The four-hundred-sixty-seventh implementation slice extends normalized probe payload attachments into the Slice 213 patient portal secure-message thread view readiness suite. It records anchor-patient preconditions, seeded original-message/thread anchor state, cleanup-backed reply/thread result, post-cleanup thread state, legacy portal thread-adjacent mailbox rendering, and modernized Portal thread rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 213 comparison.
- The four-hundred-sixty-eighth implementation slice extends normalized probe payload attachments into the Slice 214 patient portal secure-message archive readiness suite. It records anchor-patient preconditions, pre-archive mailbox state, cleanup-backed sent-message creation/archive result, post-cleanup mailbox state, legacy portal Sent/Archive folder rendering, and modernized Portal archive-action rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 214 comparison.
- The four-hundred-sixty-ninth implementation slice extends normalized probe payload attachments into the Slice 215 patient portal secure-message read-status readiness suite. It records anchor-patient preconditions, pre-read mailbox/home-summary state, cleanup-backed inbound message creation/read result, post-cleanup mailbox state, legacy portal click-driven read rendering, and modernized Portal Mark read rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 215 comparison.
- The four-hundred-seventieth implementation slice extends normalized probe payload attachments into the Slice 216 patient portal secure-message batch archive readiness suite. It records anchor-patient preconditions, pre-batch mailbox state, cleanup-backed multi-message creation/archive result, post-cleanup mailbox state, legacy portal selected-message Archive rendering, and modernized Portal Archive selected rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 216 comparison.
- The four-hundred-seventy-first implementation slice extends normalized probe payload attachments into the Slice 217 patient portal secure-message All-folder readiness suite. It records anchor-patient preconditions, pre-All-folder mailbox state, cleanup-backed inbound/sent message creation plus archived-row exclusion, post-cleanup mailbox state, legacy portal All tab rendering, and modernized Portal All secure messages rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 217 comparison.
- The four-hundred-seventy-second implementation slice extends normalized probe payload attachments into the Slice 218 patient portal document list/download readiness suite. It records anchor-patient preconditions, active portal document/category projection, selected-document ZIP package metadata, legacy portal document download rendering, and modernized Portal document download rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 218 comparison.
- The four-hundred-seventy-third implementation slice extends normalized probe payload attachments into the Slice 219 patient portal appointment list readiness suite. It records anchor-patient preconditions, future and past appointment projection, legacy portal appointment rendering, and modernized Portal appointment rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 219 comparison.
- The four-hundred-seventy-fourth implementation slice extends normalized probe payload attachments into the Slice 220 patient portal appointment request readiness suite. It records anchor-patient preconditions, cleanup-backed pending appointment/provider reminder creation, cleanup state, legacy portal requested-appointment rendering, and modernized Portal requested-appointment rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 220 comparison.
- The four-hundred-seventy-fifth implementation slice extends normalized probe payload attachments into the Slice 221 patient portal appointment request-options readiness suite. It records anchor-patient preconditions, category/provider/facility/default option projection, legacy portal request-form option rendering, and modernized Portal request-form option rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 221 comparison.
- The four-hundred-seventy-sixth implementation slice extends normalized probe payload attachments into the Slice 222 patient portal clinical-summary readiness suite. It records anchor-patient preconditions, problem/allergy/medication/prescription projection, legacy portal clinical-list page rendering, and modernized Portal clinical-summary rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 222 comparison.
- The four-hundred-seventy-seventh implementation slice extends normalized probe payload attachments into the Slice 223 patient portal lab-results readiness suite. It records anchor-patient preconditions, lab order/report/result projection, legacy portal lab-results rendering, and modernized Portal lab-results rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 223 comparison.
- The four-hundred-seventy-eighth implementation slice extends normalized probe payload attachments into the Slice 224 patient portal medical-report readiness suite. It records anchor-patient preconditions, report-builder section/issue/encounter/procedure-order projection, legacy portal report-builder rendering, and modernized Portal medical-report rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 224 comparison.
- The four-hundred-seventy-ninth implementation slice extends normalized probe payload attachments into the Slice 225 patient portal generated medical-report readiness suite. It records anchor-patient preconditions, generated report section/billing/procedure projection, legacy generated-report rendering, and modernized Portal generated-report rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 225 comparison.
- The four-hundred-eightieth implementation slice extends normalized probe payload attachments into the Slice 226 patient portal generated medical-report PDF readiness suite. It records anchor-patient preconditions, PDF readiness projection, legacy PDF export response metadata, and modernized PDF API/download-control rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 226 comparison.
- The four-hundred-eighty-first implementation slice extends normalized probe payload attachments into the Slice 227 patient portal generated medical-report issue-selection readiness suite. It records anchor-patient preconditions, selected issue/report projection, legacy issue-checkbox generated-report rendering, and modernized Portal issue-selection rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 227 comparison.
- The four-hundred-eighty-second implementation slice extends normalized probe payload attachments into the Slice 228 patient portal generated medical-report encounter-form selection readiness suite. It records anchor-patient preconditions, selected encounter-form/report projection, legacy encounter/form checkbox generated-report rendering, and modernized Portal encounter-form selection rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 228 comparison.
- The four-hundred-eighty-third implementation slice extends normalized probe payload attachments into the Slice 229 patient portal generated medical-report printable-template readiness suite. It records anchor-patient preconditions, printable template metadata projection, legacy printable generated-report rendering, and modernized Portal template rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 229 comparison.
- The four-hundred-eighty-fourth implementation slice extends normalized probe payload attachments into the Slice 230 patient portal generated medical-report package readiness suite. It records anchor-patient preconditions, package metadata projection, legacy printable/PDF source artifact rendering, and modernized package API/Portal rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 230 comparison.
- The four-hundred-eighty-fifth implementation slice extends normalized probe payload attachments into the Slice 231 patient portal generated medical-report lifecycle audit readiness suite. It records anchor-patient preconditions, generated-report audit projection, legacy printable/PDF source lifecycle evidence, and modernized audit API/Portal rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 231 comparison.
- The four-hundred-eighty-sixth implementation slice extends normalized probe payload attachments into the Slice 232 patient portal secure-message lifecycle audit readiness suite. It records anchor-patient preconditions, normalized compose/reply/read/single-archive/selected-archive audit projection, legacy secure-message source/UI evidence, and modernized message-audit API/Portal rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 232 comparison.
- The four-hundred-eighty-seventh implementation slice extends normalized probe payload attachments into the Slice 233 patient portal secure-message encrypted-body readiness suite. It records anchor-patient preconditions, protected encrypted-message projection, legacy encrypted-message portal evidence, and modernized protected-placeholder Portal rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 233 comparison.
- The four-hundred-eighty-eighth implementation slice extends normalized probe payload attachments into the Slice 234 patient portal secure-message forward-to-practice readiness suite. It records anchor-patient/original inbox-message preconditions, cleanup-backed forward result/projection, legacy Secure Messaging forward source evidence, and modernized forward API/Portal rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 234 comparison.
- The four-hundred-eighty-ninth implementation slice extends normalized probe payload attachments into the Slice 235 patient portal secure-message Deleted-folder readiness suite. It records anchor-patient preconditions, mailbox baseline/result/cleanup projections, legacy Archive-tab evidence, and modernized Deleted secure messages rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 235 comparison.
- The four-hundred-ninetieth implementation slice extends normalized probe payload attachments into the Slice 236 patient portal generated medical-report procedure-order selection readiness suite. It records anchor-patient/encounter preconditions, temporary procedure-order report-builder/result/cleanup projections, legacy custom-report source evidence, and modernized selected procedure-order generated-report rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 236 comparison.
- The four-hundred-ninety-first implementation slice extends normalized probe payload attachments into the Slice 237 patient portal generated medical-report procedure-order artifact readiness suite. It records anchor-patient/encounter preconditions, selected procedure-order delivery metadata, cleanup projections, legacy printable/PDF source evidence, and modernized PDF, ZIP package, manifest, summary, packaged-PDF, and Portal rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 237 comparison.
- The four-hundred-ninety-second implementation slice extends normalized probe payload attachments into the Slice 238 patient portal secure-message recipient-directory readiness suite. It records anchor-patient preconditions, normalized recipient-directory routing evidence, legacy compose selector evidence, and modernized recipient selector rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 238 comparison.
- The four-hundred-ninety-third implementation slice extends normalized probe payload attachments into the Slice 239 patient portal secure-message subject-preset readiness suite. It records anchor-patient preconditions, normalized compose-options subject/recipient evidence, legacy subject input and datalist rendering, and modernized Portal subject preset rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 239 comparison.
- The four-hundred-ninety-fourth implementation slice extends normalized probe payload attachments into the Slice 240 patient portal secure-message HTML-body rendering readiness suite. It records anchor-patient preconditions, raw HTML body preservation, cleanup state, legacy sanitized detail rendering, and modernized sanitized message-card rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 240 comparison.
- The four-hundred-ninety-fifth implementation slice extends normalized probe payload attachments into the Slice 241 patient portal secure-message pagination readiness suite. It records anchor-patient preconditions, first-page/second-page mailbox ordering, cleanup state, legacy next-page rendering, and modernized pager rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 241 comparison.
- The four-hundred-ninety-sixth implementation slice extends normalized probe payload attachments into the Slice 242 patient portal secure-message mark-all-read readiness suite. It records anchor-patient preconditions, browser-only visible status transitions, persisted unread mailbox state after refresh, cleanup state, legacy toolbar rendering, and modernized Portal rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 242 comparison.
- The four-hundred-ninety-seventh implementation slice extends normalized probe payload attachments into the Slice 243 patient portal prescription modified-date readiness suite. It records anchor-patient preconditions, active prescription modified-date projection, legacy portal Last Modified table rendering, and modernized Portal prescription-card rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 243 comparison.
- The four-hundred-ninety-eighth implementation slice extends normalized probe payload attachments into the Slice 244 patient portal prescription start-date readiness suite. It records anchor-patient preconditions, active prescription date-added/start-date projection, legacy portal Start Date table rendering, and modernized Portal prescription-card rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 244 comparison.
- The four-hundred-ninety-ninth implementation slice extends normalized probe payload attachments into the Slice 245 patient portal prescription end-date filtering readiness suite. It records anchor-patient preconditions, temporary ended-prescription lifecycle facts, active-prescription filtering, cleanup state, legacy portal End Date table rendering, and modernized Portal prescription-card rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 245 comparison.
- The five-hundredth implementation slice extends normalized probe payload attachments into the Slice 246 patient portal medication date-column readiness suite. It records anchor-patient preconditions, temporary ended-medication lifecycle facts, active and ended medication date-column projections, cleanup state, legacy portal Start Date/Last Modified/End Date table rendering, and modernized Portal medication-card rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 246 comparison.
- The five-hundred-first implementation slice extends normalized probe payload attachments into the Slice 247 patient portal problem date-column readiness suite. It records anchor-patient preconditions, temporary ended-problem lifecycle facts, active and ended problem date-column projections, cleanup state, legacy portal Reported Date/Start Date/End Date table rendering, and modernized Portal problem-card rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 247 comparison.
- The five-hundred-second implementation slice extends normalized probe payload attachments into the Slice 248 patient portal allergy date-column readiness suite. It records anchor-patient preconditions, temporary ended-allergy lifecycle facts, active and ended allergy date/referrer/reaction/severity projections, cleanup state, legacy portal Reported Date/Start Date/End Date/Referrer table rendering, and modernized Portal allergy-card rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 248 comparison.
- The five-hundred-third implementation slice extends normalized probe payload attachments into the Slice 249 patient portal home immunization readiness suite. It records anchor-patient preconditions, temporary entered-in-error immunization lifecycle facts, portal home immunization health-snapshot projections, cleanup state, legacy Health Snapshot rendering, and modernized Portal immunization-region rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 249 comparison.
- The five-hundred-fourth implementation slice extends normalized probe payload attachments into the Slice 250 patient portal profile readiness suite. It records anchor-patient preconditions, authenticated medical-record demographics, primary and secondary insurance projections, legacy Profile From Medical Records rendering, and modernized Portal profile-region rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 250 comparison.
- The five-hundred-fifth implementation slice extends normalized probe payload attachments into the Slice 251 patient portal profile change request readiness suite. It records anchor-patient preconditions, clean profile state, submitted waiting-review profile edit facts, refreshed pending-change projection, cleanup state, legacy Edit Pending Changes rendering, and modernized Portal pending-review rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 251 comparison.
- The five-hundred-sixth implementation slice extends normalized probe payload attachments into the Slice 252 patient portal profile review queue readiness suite. It records anchor-patient preconditions, clean staff review queue state, waiting review request projections, cleanup state, legacy normalized review-queue workflow facts, and modernized Admin review-queue rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 252 comparison.
- The five-hundred-seventh implementation slice extends normalized probe payload attachments into the Slice 253 patient portal profile review accept readiness suite. It records anchor-patient preconditions, original chart demographics, queued review request state, legacy staff accept action facts, modernized Admin Commit to Chart rendering, post-accept queue state, accepted chart demographics, and restoration cleanup as path-backed JSON evidence while preserving a matched side-by-side Slice 253 comparison.
- The five-hundred-eighth implementation slice extends normalized probe payload attachments into the Slice 254 patient portal profile review revert readiness suite. It records anchor-patient preconditions, original chart demographics, queued review request state, legacy staff revert action facts, modernized Admin Revert Edits rendering, post-revert queue state, unchanged chart demographics, and restoration cleanup as path-backed JSON evidence while preserving a matched side-by-side Slice 254 comparison.
- The five-hundred-ninth implementation slice extends normalized probe payload attachments into the Slice 255 patient portal secure-message notification readiness suite. It records anchor-patient notification preconditions, cleanup-backed notification creation, Inbox/All notification projections, cleanup state, legacy portal UI-absence evidence, and modernized read-only notification-row rendering as path-backed JSON evidence while preserving a matched side-by-side Slice 255 comparison.
- The five-hundred-tenth implementation slice adds patient portal secure-message attachment metadata readiness. It extends the modernized portal message DTO/API and Portal UI with explicit `attachmentCount` / `attachments` metadata, normalizes the current legacy-compatible zero-attachment state for portal mailbox rows, and adds a cleanup-backed parity plan proving composed secure messages expose no attachments on both targets while the modernized Portal renders `Attachments 0` on message cards.
- The five-hundred-eleventh implementation slice extends normalized probe payload attachments into the Slice 510 patient portal secure-message attachment metadata readiness suite. It records anchor-patient attachment preconditions, cleanup-backed composed-message creation facts, normalized mailbox attachment projections, cleanup state, legacy sent-surface evidence, modernized sent-card rendering, and UI-test cleanup as path-backed JSON evidence while preserving the matched Slice 510 comparison.
- The five-hundred-twelfth implementation slice adds patient portal secure-message unsupported attachment policy readiness. It introduces explicit attachment-submission request metadata for modernized compose/reply APIs, rejects non-empty payloads before mailbox inserts because legacy OpenEMR has no active portal secure-message upload control, and adds a shared parity plan proving the rejected submission creates no sent, inbox, or All-folder mailbox rows on either target.
- The five-hundred-thirteenth implementation slice adds patient portal secure-message folder search readiness. It adds a modernized Portal search field that filters Inbox, Sent, All, and Deleted message folders with the shared case-insensitive substring model before pagination, resets folder pages when the query changes, and adds a cleanup-backed shared parity plan proving the data-level filter semantics on both targets while recording that the installed legacy portal template does not expose an active rendered search input.
- The five-hundred-fourteenth implementation slice extends patient portal secure-message search readiness across Sent, All, and Deleted folder projections. It uses cleanup-backed sent and archived mailbox rows for `MOD-PAT-0004`, proves Sent/All query matching and Deleted query matching at the workflow layer, verifies the modernized Portal search field filters those rendered folder regions, and keeps the installed legacy portal no-input baseline explicit.
- The five-hundred-fifteenth implementation slice adds patient portal secure-message empty-search readiness. It makes the modernized Portal render deterministic folder-specific empty-result text for no-hit secure-message searches, keeps clear-search restoration behavior explicit, and adds a shared parity plan proving cleanup-backed Inbox, Sent, All, and Deleted rows return zero matches for a no-hit query while the installed legacy portal remains documented as having no active rendered search input.
- The five-hundred-sixteenth implementation slice adds patient portal secure-message search-count readiness. It makes the modernized Portal render a visible search-count summary with filtered and total counts for Inbox, Sent, All, and Deleted folders and adds a shared parity plan proving cleanup-backed Inbox, Sent, All, and Deleted rows produce the expected per-folder count projections while the installed legacy portal remains documented as having no active rendered search input.
- The five-hundred-seventeenth implementation slice adds patient portal secure-message clear-search readiness. It makes the modernized Portal expose a Clear search action beside the secure-message search summary, disables that action when no query is active, clears the query, resets folder paging, and restores Inbox rows; the shared parity plan records the legacy no-active-clear-search-control baseline and proves both targets restore the cleanup-backed rows after the search filter is cleared.
- The five-hundred-eighteenth implementation slice adds patient portal secure-message search-normalization readiness. It exposes the modernized Portal search-count summary as a polite live status and adds a shared parity plan proving padded mixed-case Inbox search text is trimmed, matched case-insensitively, and reported with the normalized visible query while the installed legacy portal remains documented as having no active search input or live search summary.
- The five-hundred-nineteenth implementation slice adds patient portal secure-message search-selection readiness. It scopes modernized secure-message batch selections to the currently filtered mailbox rows, clears selected message IDs when search changes or clears, and adds a shared parity plan proving a hidden selected row no longer keeps the Archive selected action enabled while the installed legacy portal remains documented as having no active search filter.
- The five-hundred-twentieth implementation slice adds patient portal secure-message search mark-all-read readiness. It scopes the modernized Portal Mark all as read action to currently filtered secure-message rows so hidden unread rows remain New when search is cleared, while the shared parity plan records the installed legacy no-active-search-filter baseline.
- The five-hundred-twenty-first implementation slice adds patient portal secure-message search pagination readiness. It resets modernized Portal secure-message folder pagers to the first filtered page when search text changes, while the shared parity plan proves search-before-pagination ordering with cleanup-backed `MOD-PAT-0004` mailbox rows and records the installed legacy no-active-search-input baseline.
- The five-hundred-twenty-second implementation slice adds appointment-to-encounter conversion readiness. The modernized Calendar appointment detail can create an encounter from a selected appointment, preserve patient/provider/facility/billing/date/time/comment context, link the encounter back to the source appointment, and show the converted encounter ID while the shared parity plan verifies cleanup-backed conversion behavior on both targets.
- The five-hundred-twenty-third implementation slice adds appointment-to-billing conversion readiness. The modernized Calendar appointment detail can create a starter `CPT4 99213` fee-sheet charge from a converted appointment encounter, expose the active converted-charge count, prevent duplicate starter charges, refresh the billing read model, and verify cleanup-backed appointment, encounter, and billing-line behavior through the shared parity plan on both targets.
- The five-hundred-twenty-fourth implementation slice adds fee-sheet charge-template readiness. The modernized Fees page can apply an office-visit template to the CPT line form, replacing ad hoc draft values with a known `CPT4 99213` office-visit payload, save the generated line through the existing billing API, and verify cleanup-backed template-created charge behavior through the shared parity plan on both targets.
- The five-hundred-twenty-fifth implementation slice adds patient payment receipt readiness. The modernized billing API can generate a deterministic PDF receipt from an active payment posting, the Fees page exposes a `Receipt` action on each payment row, and the shared parity plan creates a cleanup-backed patient payment, verifies the receipt contract against both targets, downloads and inspects the modernized PDF, confirms the UI action is visible, and removes the temporary payment.
- The five-hundred-twenty-sixth implementation slice adds patient payment refund readiness. The modernized billing API accepts explicit `patient_refund` postings as negative patient payments, the Fees page exposes a `Patient refund` posting mode and labels refund rows/statement lines clearly, and the shared parity plan posts a cleanup-backed refund reversal, verifies payment-session/activity increments, balance movement, positive refund ledger impact, modernized UI rendering, and final cleanup on both targets.
- The five-hundred-twenty-seventh implementation slice adds insurance payment reversal readiness. The modernized billing API accepts explicit `insurance_reversal` postings as negative insurer-backed payments, the Fees page exposes an `Insurance reversal` posting mode and labels reversal rows distinctly from patient refunds, and the shared parity plan posts a cleanup-backed reversal for Northstar HMO, verifies payer identity, payment-session/activity increments, net payment reduction, balance increase, positive reversal ledger impact, modernized UI rendering, and final cleanup on both targets.
- The five-hundred-twenty-eighth implementation slice adds adjustment reversal readiness. The modernized billing API accepts explicit `adjustment_reversal` postings as negative insurer-backed adjustments with no payment amount, the Fees page exposes an `Adjustment reversal` posting mode and labels reversed adjustments clearly, and the shared parity plan posts a cleanup-backed Northstar HMO adjustment reversal, verifies payer identity, payment-session/activity increments, net adjustment reduction, balance increase, positive reversal ledger impact, modernized UI rendering, and final cleanup on both targets.
- The five-hundred-twenty-ninth implementation slice adds claim denial readiness. The modernized Fees claim card exposes a `Deny` action that records denied status, X12 denial metadata, a deterministic denial response file, and a submitted-claim payload, while the shared parity plan creates a temporary queued Northstar HMO claim, updates it to denied, verifies payer/status/file/payload/count facts, renders the modernized Fees denial state, and hard-deletes the temporary row on both targets.
- The five-hundred-thirtieth implementation slice adds claim adjudication readiness. The modernized Fees claim card exposes an `Adjudicate` action that posts a linked insurer EOB payment/adjustment, stamps the claim with cleared X12 835 metadata, and refreshes the billing view; the shared parity plan creates a temporary generated Northstar HMO claim, adjudicates it with payment, adjustment, reason-code, and payer-claim metadata, verifies balance and ledger effects, renders the modernized UI action/result, and removes both the temporary payment and claim on both targets.
- The five-hundred-thirty-first implementation slice adds starter EOB batch import readiness. The modernized Fees payment panel exposes an `Import EOB` action that posts a deterministic two-line Northstar HMO electronic remittance batch through the existing payment posting service; the shared parity plan imports or creates the same temporary batch on both targets, verifies payer identity, payment method, payer-claim numbers, reason codes, payment/adjustment balance movement, ledger rows, modernized UI rendering, and removes both payment rows so starting payment-session and payment-activity counts are restored.
- The five-hundred-thirty-second implementation slice adds focused 837P claim generation readiness. The modernized Fees claim card `Generate` action now creates a deterministic claim file name and 837P-style submitted-claim payload from the claim, payer, encounter, and patient anchor; the shared parity plan creates a temporary queued Northstar HMO claim, drives the modernized UI action or applies the equivalent legacy update, verifies X12 target, process-file metadata, submitted-claim segments, UI rendering, claim-count stability, and hard-delete cleanup.
- The five-hundred-thirty-third implementation slice adds focused claim scrubbing readiness. The modernized Fees claim card exposes a `Scrub` action that evaluates the encounter fee-sheet lines for payer presence, CPT line availability, diagnosis pointers, positive fees, and positive units, writes a deterministic local scrub report to submitted-claim metadata, and refreshes the billing view; the shared parity plan creates a temporary queued Northstar HMO claim, drives the modernized UI action or applies the equivalent legacy update, verifies the scrub report, process-file metadata, UI rendering, claim-count stability, and hard-delete cleanup.
- The five-hundred-thirty-fourth implementation slice adds focused claim resubmission readiness. The modernized Fees claim card exposes a `Resubmit` action for denied claims that requeues the claim for billing with X12 target metadata, deterministic resubmission process-file naming, and a compact resubmission payload; the shared parity plan creates a temporary denied Northstar HMO claim, drives the modernized UI action or applies the equivalent legacy update, verifies queued status, resubmission payload content, UI rendering, claim-count stability, and hard-delete cleanup.
- The five-hundred-thirty-fifth implementation slice adds expanded charge-template catalog readiness. The modernized Fees CPT form now includes a `Complex follow-up` template with CPT `99214`, modifier `25`, two units, fee, description, and diagnosis pointer values; the shared parity plan applies the template in the modernized UI or creates the equivalent legacy billing row, verifies the modifier-bearing multi-unit fee-sheet row, count movement, UI rendering, and hard-delete cleanup.
- The five-hundred-thirty-sixth implementation slice adds focused claim modifier-scrubbing readiness. The modernized Fees claim `Scrub` action now checks CPT modifiers against a small supported modifier catalog and emits deterministic `invalid-modifier:<value>` issues for unsupported values; the shared parity plan creates a temporary unsupported-modifier CPT line plus queued Northstar HMO claim, drives the modernized UI action or applies the equivalent legacy update, verifies `SCRUB-FAIL` content, claim and line count stability, UI rendering, and hard-delete cleanup.
- The five-hundred-thirty-seventh implementation slice adds focused claim diagnosis-pointer scrubbing readiness. The modernized Fees claim `Scrub` action now compares CPT diagnosis pointers with active ICD10 diagnosis lines on the same encounter and emits deterministic `invalid-diagnosis-pointer:<value>` issues for unsupported pointers; the shared parity plan creates a temporary ICD10 diagnosis line, a CPT line with an unsupported pointer, and a queued Northstar HMO claim, then verifies `SCRUB-FAIL` content, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-thirty-eighth implementation slice adds focused claim modifier-compatibility readiness. The modernized Fees claim `Scrub` action now tokenizes comma/space-separated CPT modifiers, validates each token against the supported modifier catalog, and emits deterministic `duplicate-modifier:<value>` issues when the same modifier appears more than once on a CPT line; the shared parity plan creates a temporary ICD10 diagnosis line, a CPT line with duplicate supported modifier `25,25`, and a queued Northstar HMO claim, then verifies `SCRUB-FAIL` content, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-thirty-ninth implementation slice adds focused claim modifier-count readiness. The modernized Fees claim `Scrub` action now emits deterministic `modifier-count-exceeded:<count>` issues when a CPT line has more than four modifier tokens and can parse compact two-character modifier sequences; the shared parity plan creates a temporary ICD10 diagnosis line, a CPT line with five supported modifiers encoded as `2559767795`, and a queued Northstar HMO claim, then verifies `SCRUB-FAIL` content without invalid- or duplicate-modifier misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-fortieth implementation slice adds focused claim missing-diagnosis-code readiness. The modernized Fees claim `Scrub` action now emits deterministic `missing-diagnosis-code` issues when CPT lines carry diagnosis pointers but the encounter has no active ICD10 diagnosis rows; the shared parity plan creates a temporary billing-only encounter with a CPT line and queued Northstar HMO claim but no ICD10 line, then verifies `SCRUB-FAIL` content without missing-pointer or invalid-pointer misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-forty-first implementation slice adds focused claim multi-diagnosis-pointer readiness. The modernized Fees claim `Scrub` action now tokenizes comma/space-separated CPT diagnosis pointers before validating them against active encounter ICD10 diagnosis rows; the shared parity plan creates a temporary encounter with ICD10 lines for `K21.9` and `E11.9`, a CPT line justified by `K21.9,E11.9`, and a queued Northstar HMO claim, then verifies deterministic `SCRUB-PASS` content, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-forty-second implementation slice adds focused claim diagnosis-pointer-count readiness. The modernized Fees claim `Scrub` action now flags CPT service lines with more than four diagnosis pointer tokens as deterministic `diagnosis-pointer-count-exceeded:<count>` scrub issues; the shared parity plan creates a temporary encounter with five active ICD10 diagnosis lines, a CPT line justified by all five pointers, and a queued Northstar HMO claim, then verifies deterministic `SCRUB-FAIL` content without invalid-pointer or missing-code misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-forty-third implementation slice adds focused duplicate diagnosis-pointer readiness. The modernized Fees claim `Scrub` action now flags repeated CPT diagnosis pointer tokens on the same service line as deterministic `duplicate-diagnosis-pointer:<code>` scrub issues; the shared parity plan creates a temporary encounter with one active ICD10 diagnosis line, a CPT line justified by the same pointer twice, and a queued Northstar HMO claim, then verifies deterministic `SCRUB-FAIL` content without invalid-pointer, missing-code, or pointer-count misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-forty-fourth implementation slice adds focused duplicate diagnosis-code readiness. The modernized Fees claim `Scrub` action now flags duplicate active ICD10 diagnosis rows on the encounter as deterministic `duplicate-diagnosis-code:<code>` scrub issues; the shared parity plan creates a temporary encounter with two active `K21.9` ICD10 lines, a CPT line justified by `K21.9`, and a queued Northstar HMO claim, then verifies deterministic `SCRUB-FAIL` content without invalid-pointer, missing-code, or duplicate-pointer misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-forty-fifth implementation slice adds focused invalid CPT-code readiness. The modernized Fees claim `Scrub` action now flags CPT4 service lines whose code is not exactly five digits as deterministic `invalid-cpt-code:<code>` scrub issues; the shared parity plan creates a temporary encounter with a valid ICD10 diagnosis, a malformed CPT4 code `99A14`, and a queued Northstar HMO claim, then verifies deterministic `SCRUB-FAIL` content without diagnosis-pointer or missing-code misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-forty-sixth implementation slice adds focused invalid ICD10 diagnosis-code readiness. The modernized Fees claim `Scrub` action now flags ICD10 diagnosis rows whose code does not match the supported ICD10 format as deterministic `invalid-diagnosis-code:<code>` scrub issues; the shared parity plan creates a temporary encounter with malformed diagnosis code `BADCODE`, a valid CPT4 `99214` line justified by that diagnosis, and a queued Northstar HMO claim, then verifies deterministic `SCRUB-FAIL` content without diagnosis-pointer, missing-code, or CPT-code misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-forty-seventh implementation slice adds focused future service-date readiness. The modernized Fees claim `Scrub` action now flags active CPT4 service lines whose billing date is after the claim-scrub business date as deterministic `future-service-date:<date>` scrub issues; the shared parity plan creates a temporary future-dated encounter with valid ICD10 diagnosis `K21.9`, valid CPT4 `99214`, and a queued Northstar HMO claim, then verifies deterministic `SCRUB-FAIL` content without CPT-code, diagnosis-code, diagnosis-pointer, or missing-code misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-forty-eighth implementation slice adds focused incompatible modifier-combination readiness. The modernized Fees claim `Scrub` action now flags active CPT4 service lines that combine individually supported but incompatible modifiers `25` and `59` as deterministic `incompatible-modifier-combination:25+59` scrub issues; the shared parity plan creates a temporary encounter with valid ICD10 diagnosis `K21.9`, valid CPT4 `99214`, modifier text `25,59`, and a queued Northstar HMO claim, then verifies deterministic `SCRUB-FAIL` content without invalid-modifier, duplicate-modifier, modifier-count, CPT-code, diagnosis-pointer, or missing-code misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-forty-ninth implementation slice adds focused missing-payer readiness. The modernized billing claim API now permits `payerId` `0` so a queued claim can represent the same missing-payer state the legacy baseline permits, while the Fees claim `Scrub` action treats zero/blank payer metadata as deterministic `missing-payer` scrub issues; the shared parity plan creates a temporary encounter with valid ICD10 diagnosis `K21.9`, valid CPT4 `99214`, modifier `25`, positive fee and units, and a queued claim with no payer selected, then verifies deterministic `SCRUB-FAIL` content without CPT-code, diagnosis-code, diagnosis-pointer, modifier, fee, unit, future-date, or missing-code misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-fiftieth implementation slice adds focused invalid-fee readiness. The modernized Fees claim `Scrub` action now uses explicit numeric validation for fee and unit checks so zero or non-numeric CPT fee values fail deterministically, while preserving valid-payer, valid-code, valid-diagnosis, valid-modifier, and valid-unit behavior; the shared parity plan creates a temporary encounter with valid ICD10 diagnosis `K21.9`, valid CPT4 `99214`, supported modifier `25`, zero CPT fee, and a queued Northstar HMO claim, then verifies deterministic `SCRUB-FAIL` / `invalid-fee` content without payer, CPT-code, diagnosis-code, diagnosis-pointer, modifier, unit, future-date, or missing-code misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-fifty-first implementation slice adds focused invalid-units readiness. The modernized billing line create/update API now permits zero units so invalid quantity values can be represented and caught by claim scrubbing while negative units remain rejected; the shared parity plan creates a temporary encounter with valid ICD10 diagnosis `K21.9`, valid CPT4 `99214`, supported modifier `25`, positive fee, zero CPT units, and a queued Northstar HMO claim, then verifies deterministic `SCRUB-FAIL` / `invalid-units` content without payer, fee, CPT-code, diagnosis-code, diagnosis-pointer, modifier, future-date, or missing-code misclassification, claim and line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-fifty-second implementation slice adds focused missing-CPT-line readiness. The shared parity plan creates a temporary encounter with valid ICD10 diagnosis `K21.9`, intentionally omits every CPT4 service line, and queues a Northstar HMO claim, then verifies deterministic `SCRUB-FAIL` / `missing-cpt-line` content without payer, fee, unit, CPT-code, diagnosis-code, diagnosis-pointer, modifier, future-date, or missing-code misclassification, claim and single diagnosis-line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-fifty-third implementation slice adds focused missing-diagnosis-pointer readiness. The modernized billing line create/update API now permits blank diagnosis pointers so claim scrubbing can flag them instead of rejecting the charge before review; the shared parity plan creates a temporary encounter with valid ICD10 diagnosis `K21.9`, valid CPT4 `99214`, supported modifier `25`, positive fee and units, but a blank CPT diagnosis pointer, then queues a Northstar HMO claim and verifies deterministic `SCRUB-FAIL` / `missing-diagnosis-pointer` content without payer, CPT-line, fee, unit, CPT-code, diagnosis-code, invalid-pointer, modifier, future-date, or missing-code misclassification, claim and billing-line count stability, UI rendering, and hard-delete cleanup on both targets.
- The five-hundred-fifty-fourth implementation slice adds focused inactive-CPT-line readiness. The shared parity plan creates a temporary encounter with valid ICD10 diagnosis `K21.9`, valid CPT4 `99214`, supported modifier `25`, positive fee and units, marks the CPT row billed/inactive before claim scrubbing, then queues a Northstar HMO claim and verifies active-line projection produces deterministic `SCRUB-FAIL` / `missing-cpt-line` content without payer, fee, unit, CPT-code, diagnosis-code, diagnosis-pointer, modifier, future-date, or missing-code misclassification, while proving the inactive CPT row is still present until cleanup and is ignored for scrub eligibility on both targets.
- The five-hundred-fifty-fifth implementation slice adds focused inactive-diagnosis-line readiness. The shared parity plan creates a temporary encounter with ICD10 diagnosis `K21.9`, marks that diagnosis row billed/inactive before claim scrubbing, creates an active valid CPT4 `99214` line justified by `K21.9`, queues a Northstar HMO claim, and verifies active-line projection produces deterministic `SCRUB-FAIL` / `missing-diagnosis-code` content without payer, CPT-line, fee, unit, CPT-code, diagnosis-pointer, modifier, future-date, or invalid-pointer misclassification, while proving the inactive diagnosis row is still present until cleanup and is ignored for scrub eligibility on both targets.
- The five-hundred-fifty-sixth implementation slice moves focused claim scrubbing from the React UI into the modernized billing API. The Fees claim card now calls `POST /api/billing/claims/{claimId}/scrub`, the backend reads the claim plus active encounter billing lines, computes the deterministic scrub report, persists process-file/submitted-claim metadata, and returns refreshed billing detail. The shared parity plan verifies the same temporary Northstar HMO queued claim still produces `SCRUB-PASS` / `issues=none` behavior against both targets while documenting the business-tier ownership of scrub logic.

- The five-hundred-fifty-seventh implementation slice moves focused 837P claim generation from the React UI into the modernized billing API. The Fees claim card now calls `POST /api/billing/claims/{claimId}/generate`, the backend reads the claim and patient identity, builds the deterministic 837P-style submitted-claim payload, marks the claim generated with X12 metadata, and returns refreshed billing detail. The shared parity plan verifies the same temporary Northstar HMO queued claim still produces matching process-file, submitted-claim, rendering, count, and cleanup behavior against both targets while documenting the business-tier ownership of generation logic.

- The five-hundred-fifty-eighth implementation slice moves focused claim resubmission from the React UI into the modernized billing API. The Fees claim card now calls `POST /api/billing/claims/{claimId}/resubmit`, the backend reads the denied claim and patient identity, builds the deterministic X12 resubmission payload, requeues the claim, and returns refreshed billing detail. The shared parity plan verifies the same temporary Northstar HMO denied claim still produces matching queued status, process-file, submitted-claim, rendering, count, and cleanup behavior against both targets while documenting the business-tier ownership of resubmission logic.

- The five-hundred-fifty-ninth implementation slice moves focused claim denial from the React UI into the modernized billing API. The Fees claim card now calls `POST /api/billing/claims/{claimId}/deny`, the backend reads the queued claim and patient identity, applies the deterministic X12 denial metadata and submitted-claim fallback, marks the claim denied, and returns refreshed billing detail. The shared parity plan verifies the same temporary Northstar HMO queued claim still produces matching denied status, process-file, submitted-claim, rendering, count, and cleanup behavior against both targets while documenting the business-tier ownership of denial logic.

- The five-hundred-sixtieth implementation slice moves focused claim clearing from the React UI into the modernized billing API. The Fees claim card now calls `POST /api/billing/claims/{claimId}/clear`, the backend reads the claim and patient identity, applies the HCFA cleared status transition, clears process-file/X12 partner metadata, preserves or creates the submitted-claim fallback, and returns refreshed billing detail. The shared parity plan verifies the same temporary Northstar HMO denied claim still produces matching cleared status, HCFA target, submitted-claim, rendering, count, and cleanup behavior against both targets while documenting the business-tier ownership of clearing logic.

- The five-hundred-sixty-first implementation slice moves focused claim adjudication from the React UI into the modernized billing API. The Fees claim card now calls `POST /api/billing/claims/{claimId}/adjudicate`, the backend reads the claim, patient, and encounter context, posts the deterministic Northstar HMO EOB payment and contractual adjustment, stamps the claim with X12 835 metadata, and returns refreshed billing detail. The shared parity plan verifies the same temporary generated claim still produces matching payment, adjustment, ledger, balance, claim metadata, rendering, and cleanup behavior against both targets while documenting the business-tier ownership of adjudication logic.

- The five-hundred-sixty-second implementation slice moves starter EOB batch import from the React UI into the modernized billing API. The Fees `Import EOB` action now calls `POST /api/billing/eob-batches/import`, the backend validates the anchor patient and encounter, inserts the deterministic two-line Northstar HMO remittance batch in one transaction, and returns refreshed billing detail. The shared parity plan verifies the same temporary EOB payment, adjustment, ledger, balance, rendering, and cleanup behavior against both targets while documenting the business-tier ownership of batch remittance import logic.

- The five-hundred-sixty-third implementation slice moves the focused charge-template catalog from the React UI into the modernized billing API. The Fees template buttons now call `GET /api/billing/charge-templates/{templateId}`, the backend returns the deterministic Office visit, Preventive visit, Telehealth follow-up, and Complex follow-up CPT payloads, and the existing Save CPT path persists the generated fee-sheet line. The shared parity plan verifies Office visit and Complex follow-up template application, saved line payloads, count movement, rendering, and cleanup against both targets while documenting the business-tier ownership of charge-template catalog logic.

- The five-hundred-sixty-fourth implementation slice moves patient-refund posting semantics from the React UI into the modernized billing API. The Fees `Patient refund` mode now calls `POST /api/billing/payments/patient-refunds` with the patient, encounter, reference, date, method, CPT context, memo, and refund amount; the backend owns OpenEMR-shaped refund rules by forcing patient payer identity, `patient_refund` payment type, negative pay amount, zero adjustment, blank reason/account codes, and refreshed billing detail. The shared parity plan verifies the existing temporary patient refund posting, rendering, account-balance/ledger movement, and cleanup against both targets while documenting the business-tier ownership of patient-refund posting logic.

- The five-hundred-sixty-fifth implementation slice moves insurance payment reversal semantics from the React UI into the modernized billing API. The Fees `Insurance reversal` mode now calls `POST /api/billing/payments/insurance-reversals` with the patient, encounter, payer, reference, date, method, CPT context, memo, reversal amount, and payer claim number; the backend owns OpenEMR-shaped insurer-backed reversal rules by forcing primary payer type, `insurance_reversal` payment type, negative pay amount, zero adjustment, blank reason/account codes, and refreshed billing detail. The shared parity plan verifies the temporary Northstar HMO reversal posting, rendering, account-balance/ledger movement, payer claim number, and cleanup against both targets while documenting the business-tier ownership of insurance-reversal posting logic.

- The five-hundred-sixty-sixth implementation slice moves adjustment reversal semantics from the React UI into the modernized billing API. The Fees `Adjustment reversal` mode now calls `POST /api/billing/payments/adjustment-reversals` with the patient, encounter, payer, reference, date, method, CPT context, memo, adjustment amount, and payer claim number; the backend owns OpenEMR-shaped adjustment reversal rules by forcing primary payer type, `adjustment_reversal` payment type, zero pay amount, negative adjustment amount, blank reason/account codes, and refreshed billing detail. The shared parity plan verifies the temporary Northstar HMO adjustment reversal posting, rendering, account-balance/ledger movement, payer claim number, and cleanup against both targets while documenting the business-tier ownership of adjustment-reversal posting logic.

- The five-hundred-sixty-seventh implementation slice moves insurance payment posting semantics from the React UI into the modernized billing API. The Fees `Insurance` payment source now calls `POST /api/billing/payments/insurance-payments` with the patient, encounter, payer, reference, date, method, CPT context, memo, payment amount, adjustment amount, reason code, and payer claim number; the backend owns OpenEMR-shaped insurer payment rules by forcing primary payer type, `insurance_payment` payment type, positive payment amount, non-negative adjustment amount, account-code derivation from the reason code, payer claim preservation, and refreshed billing detail. The shared parity plan verifies the temporary Northstar HMO insurance payment posting, rendering, void behavior, account-balance/ledger movement, payer claim number, and cleanup against both targets while documenting the business-tier ownership of insurance-payment posting logic.

- The five-hundred-sixty-eighth implementation slice moves patient payment capture semantics from the React UI into the modernized billing API. The Fees `Patient` payment source now calls `POST /api/billing/payments/patient-payments` with the patient, encounter, reference, date, method, CPT context, memo, and payment amount; the backend owns OpenEMR-shaped patient payment rules by forcing patient payer identity, `patient_payment` payment type, positive payment amount, zero adjustment, blank reason/account codes, blank payer claim number, and refreshed billing detail. The shared parity plan verifies the temporary patient payment posting, rendering, void behavior, account-balance/ledger movement, and cleanup against both targets while documenting the business-tier ownership of patient-payment capture logic.

- The five-hundred-sixty-ninth implementation slice hardens the billing payment-create contract after the focused payment operations are in place. The modernized API no longer exposes broad `POST /api/billing/payments` creation, the React Fees shell removes the generic payment-posting helper/fallback, the parity workflow adapter routes all temporary payment creation through the focused patient-payment, patient-refund, insurance-payment, insurance-reversal, and adjustment-reversal endpoints, and the billing mutation authorization-policy suite proves the retired broad create method returns 405 even for an admin session while payment void/delete routes remain available by ID. This keeps payment business semantics in the server-side operation selected by the payment type rather than allowing callers to author arbitrary posting fields.

- The five-hundred-seventieth implementation slice adds statement delivery manifest readiness. The modernized billing API exposes `POST /api/billing/statements/batch/delivery-manifest`, deriving deterministic queued handoff rows from the same top statement batch candidates used by Slice 61 and Slice 62. The React Fees statement batch panel can prepare and render the delivery ID, prepared timestamp, queued count, email-ready count, print-ready count, destination, file name, balance, and due date for each selected statement without sending email or printing. The shared parity plan compares the normalized legacy database projection with the modernized API/UI response, keeping production mail/print delivery integrations as future scope.

- The five-hundred-seventy-first implementation slice adds statement dispatch handoff readiness. The modernized billing API exposes `POST /api/billing/statements/batch/dispatch`, converting the deterministic delivery manifest rows into local email/print queue handoff records with dispatch audit IDs, queue names, dispatch statuses, external references, destinations, and statement file references. The React Fees statement batch panel exposes a `Dispatch Handoff` action and renders the dispatch summary/rows. The shared parity plan compares the normalized legacy-derived handoff expectation with the modernized API/UI response while keeping real SMTP, printer, mailing, portal delivery, and durable delivery-audit integrations as future scope.

- The five-hundred-seventy-second implementation slice adds durable local statement dispatch audit-history readiness. The modernized billing API now persists each `Dispatch Handoff` row into `statement_delivery_audit_events`, exposes `GET /api/billing/statements/batch/dispatch-history`, and keeps deterministic dispatch IDs, audit IDs, queue names, statuses, external references, timestamps, destinations, and file names available after the original dispatch response. The React Fees statement batch panel adds a `Dispatch History` action and renders the saved audit summary/rows. The shared parity plan compares the normalized legacy-derived history expectation with the modernized API/UI response while keeping real SMTP, printer, mailing, and portal delivery integrations as future scope.

- The five-hundred-seventy-third implementation slice adds patient portal statement delivery readiness. The modernized billing API exposes `POST /api/billing/statements/batch/portal-delivery`, finds portal-enabled recipients among the top statement batch candidates, and publishes deterministic statement PDFs into the existing `Invoices` patient-document category with idempotent document keys. The React Fees statement batch panel adds a `Portal Delivery` action and renders delivered portal document rows, while the patient portal Documents page shows the published statement document for eligible recipients. The shared parity plan compares the legacy-derived portal delivery expectation with the modernized API, Fees UI, and portal document rendering while keeping real SMTP, printer, and mailing-provider integrations as future scope.

- The five-hundred-seventy-fourth implementation slice adds statement email outbox readiness. The modernized billing API exposes `POST /api/billing/statements/batch/email-outbox`, filters the top statement batch candidates to email-ready recipients, persists deterministic local outbox messages in `statement_email_outbox`, and returns message IDs, subjects, recipients, attachment file names, queue names, statuses, and local references. The React Fees statement batch panel adds an `Email Outbox` action and renders queued message evidence. The shared parity plan compares the legacy-derived email outbox expectation with the modernized API/UI response while keeping live SMTP/provider delivery, bounce handling, and production mail credentials as future scope.

- The five-hundred-seventy-fifth implementation slice adds appointment availability validation readiness. The modernized appointment API exposes `POST /api/appointments/availability/validate`, resolves the draft patient/provider/facility context, checks the shared Monday-Friday 08:00-17:00 provider/facility bookable window, and reports active provider, patient, and room conflicts without creating an appointment. The React Calendar create form adds a `Check availability` action that renders available/unavailable status, provider/facility availability, conflict count, and validation messages. The shared parity plan creates and cleans up a temporary blocker appointment, compares conflict and outside-hours behavior against the legacy-derived scheduling projection, and keeps configurable provider schedules, waitlists, and reminder delivery as future scope.
- The five-hundred-seventy-sixth implementation slice adds appointment waitlist readiness. Pending portal appointment requests (`status = '^'`) are now exposed through `GET /api/appointments/waitlist`, joined to their provider reminder messages, and rendered in the modernized Calendar as a staff waitlist queue with requested slot, patient, reason, reminder status, and priority context. Staff can promote a waiting request to OpenEMR pending status (`~`) through the existing appointment status mutation path. The shared parity plan creates and cleans up a temporary portal appointment request, verifies the linked reminder and waiting status, checks modernized Calendar waitlist rendering, promotes the request, and confirms the same slot moves to pending status on both targets.
- The five-hundred-seventy-seventh implementation slice adds appointment reminder dispatch readiness. The modernized appointment API exposes `POST /api/appointments/{appointmentId}/reminders/dispatch` and `GET /api/appointments/reminders/dispatch-history`, derives due-reminder channel/contact/template facts from the same seeded readiness model as Slice 120, persists idempotent local dispatch audit rows, and returns deterministic queue, audit, external-reference, template, and message-preview evidence. The React Calendar detail panel adds a `Dispatch reminder` action plus dispatch and history summaries, while the shared parity plan compares the legacy-derived dispatch expectation with the modernized API/UI response and keeps real SMS/email/phone provider delivery, retry handling, and template administration as future scope.
- The five-hundred-seventy-eighth implementation slice adds medication duplicate detection readiness. The modernized clinical-list API now groups active medication-list rows by normalized medication title, exposes duplicate-count/date/diagnosis/source-ID evidence, and the Lists page renders a compact duplicate summary. The shared parity plan creates two cleanup-backed active medication rows with the same normalized title on `MOD-PAT-0006`, compares the legacy database projection with the modernized API and UI rendering, and removes both rows after the run.
- The five-hundred-seventy-ninth implementation slice adds prescription refill authorization readiness. The modernized clinical-list API exposes `PUT /api/clinical-lists/prescriptions/{prescriptionId}/refill`, increments active prescription refill count, stamps the modified date, and preserves note evidence without ending the prescription. The Lists page adds a `Refill` action on prescription cards, and the shared parity plan creates a cleanup-backed active prescription on `MOD-PAT-0008`, authorizes one additional refill, compares legacy and modernized row state, verifies modernized rendering, and deletes the temporary prescription.
- The five-hundred-eightieth implementation slice adds patient portal prescription refill request readiness. The modernized patient portal API exposes `POST /api/patient-portal/prescriptions/{prescriptionId}/refill-request`, validates that the active prescription belongs to the signed-in portal patient, creates secure-message sent and recipient mailbox evidence with `portal:prescription-refill-request` relation metadata, and records a portal message audit event. The Portal clinical summary adds a `Request refill` action on prescription cards, and the shared parity plan creates a cleanup-backed active prescription for `MOD-PAT-0004`, submits the patient-originated refill request, compares the mailbox evidence with legacy `onsite_mail`, verifies modernized rendering, and deletes the temporary prescription and message rows.
- The five-hundred-eighty-first implementation slice adds staff prescription refill request approval readiness. The modernized clinical-list API now projects pending `portal:prescription-refill-request` mailbox rows as `prescriptionRefillRequests` in the Lists patient detail and exposes `PUT /api/clinical-lists/prescription-refill-requests/{messageId}/approve` to approve one or more additional refills while marking the originating mailbox request `Done`. The Lists page renders a `Refill Requests` staff queue with patient note, current refill count, and an `Approve` action. The shared parity plan creates a cleanup-backed active prescription for `MOD-PAT-0004`, submits the portal refill request, verifies the staff queue, approves the request, compares legacy and modernized prescription/message status, verifies modernized rendering, and removes the temporary prescription and mailbox rows.
- The five-hundred-eighty-second implementation slice adds prescription pharmacy routing readiness. The modernized PostgreSQL schema now includes a focused pharmacy catalog plus prescription routing/transmit fields, the clinical-list API exposes `PUT /api/clinical-lists/prescriptions/{prescriptionId}/route-pharmacy`, and the Lists page renders a `Route` action with pharmacy, NCPDP, sent timestamp, and local eRx payload evidence. The shared parity plan creates a cleanup-backed active prescription for `MOD-PAT-0008`, routes it to deterministic Northstar pharmacy evidence, compares legacy `pharmacies` / `prescriptions` fields with modernized PostgreSQL state, verifies modernized rendering, and removes the temporary prescription plus legacy-only pharmacy fixture.
- The five-hundred-eighty-third implementation slice adds prescription controlled-substance routing guard readiness. The clinical-list API now flags controlled-substance prescriptions with schedule/review evidence, ordinary pharmacy routing returns a blocked route result before transmit fields are stamped, and the Lists page renders the EPCS review warning. The shared parity plan creates a cleanup-backed Oxycodone prescription for `MOD-PAT-0008`, verifies the legacy adapter and modernized API both block ordinary pharmacy routing, proves pharmacy/eRx fields remain empty, checks modernized warning rendering, and removes the temporary prescription plus legacy-only pharmacy fixture.
- The five-hundred-eighty-fourth implementation slice adds prescription audit history readiness. The clinical-list API now persists focused prescription audit events for create, refill, route, and deactivate operations, exposes `GET /api/clinical-lists/prescriptions/{prescriptionId}/audit-history`, and the Lists page renders a selected prescription's event history. The shared parity plan creates a cleanup-backed prescription for `MOD-PAT-0008`, refills it, routes it to deterministic Northstar pharmacy evidence, compares create/refill/route audit events across legacy-derived workflow evidence and the modernized audit table, verifies modernized history rendering, and removes the temporary prescription plus legacy-only pharmacy fixture.
...
- The five-hundred-eighty-ninth implementation slice adds appointment conflict enforcement readiness. The modernized appointment create contract accepts an explicit `enforceConflictPolicy` flag, runs the existing availability engine before insert, returns HTTP 409 with provider/room conflict evidence when strict scheduling would collide with an active appointment, and the Calendar create form exposes a `Block conflicts` toggle with the returned conflict message. The shared parity plan creates a cleanup-backed blocker appointment, proves provider and room conflict source facts on both targets, verifies the modernized API/UI rejection path, and confirms the blocked appointment row was not inserted.
- The five-hundred-ninetieth implementation slice adds document OCR queue readiness. The modernized document API exposes protected `GET /api/documents/ocr-queue`, projecting active scanned documents with `OCR pending` evidence into a staff work queue with queue status, priority, capture source, page count, and patient context. The Documents workspace renders an OCR Queue panel beside the document summary, while the shared parity plan creates a cleanup-backed scanned PDF for `MOD-PAT-0001`, proves source facts on both targets, verifies the modernized API/UI queue projection, and deletes the temporary document.
- The five-hundred-ninety-first implementation slice adds document OCR completion readiness. The modernized document API exposes protected `POST /api/documents/{documentId}/ocr/complete`, converts a queued scanned document from `OCR pending` to `OCR complete`, preserves extracted-text evidence in the document content/notes projection, returns the refreshed document plus queue, and the Documents workspace exposes `Complete OCR` from the OCR Queue panel. The shared parity plan creates a cleanup-backed scanned PDF for `MOD-PAT-0001`, proves pending-OCR source facts on both targets, verifies the modernized API/UI completion path, confirms the completed item leaves the queue, and deletes the temporary document.
- The five-hundred-ninety-second implementation slice adds generated PDF thumbnail readiness. The modernized document read model emits deterministic SVG thumbnail data URIs for small database-backed PDF rows, the Documents workspace renders those generated thumbnails through the existing document-card thumbnail slot, and the shared parity plan creates a cleanup-backed PDF for `MOD-PAT-0001`, proves comparable legacy PDF source facts, verifies the modernized API/card thumbnail evidence, archives the temporary document, and removes it during cleanup.
- The five-hundred-ninety-third implementation slice adds document routing queue readiness. The modernized document API exposes protected `GET /api/documents/routing-queue`, projecting active pending-review documents into routed work items with category, destination, priority, review status, and reason evidence. The Documents workspace renders a Routing Queue panel beside OCR Queue, and the shared parity plan creates a cleanup-backed high-priority Advance Directive PDF for `MOD-PAT-0001`, proves comparable legacy source facts, verifies the modernized API/UI queue evidence, and removes it during cleanup.
- The five-hundred-ninety-fourth implementation slice adds document retention policy readiness. The modernized document API exposes protected `GET /api/documents/retention-policy`, projecting active documents into retention-policy rows with retention class, retention years, retain-until date, disposition status, and policy-basis evidence calculated from the deterministic dataset base date. The Documents workspace renders a Retention Policy panel, and the shared parity plan creates a cleanup-backed older Patient Information PDF for `MOD-PAT-0001`, proves comparable legacy source facts, verifies modernized API/UI retention evidence, and removes it during cleanup.
- The five-hundred-ninety-fifth implementation slice adds controlled document retention disposition readiness. The modernized document API exposes protected `POST /api/documents/{documentId}/retention/dispose`, validates that the active document is eligible under the deterministic retain-until calculation, archives the document, appends disposition evidence to notes, and returns refreshed document/policy state. The Documents Retention Policy panel exposes a `Dispose` action for eligible rows, and the shared parity plan creates a cleanup-backed older Patient Information PDF for `MOD-PAT-0001`, proves comparable legacy archive behavior, verifies modernized API/UI disposition behavior, and removes it during cleanup.
...
- The six-hundredth implementation slice adds appointment waitlist deferral readiness. The modernized Calendar waitlist cards now expose a `Defer request` action that updates the linked `Patient Reminders` row to `Deferred` with a staff follow-up note while preserving the portal appointment request in OpenEMR waiting status (`^`). The shared parity plan creates a cleanup-backed portal appointment request for `MOD-PAT-0004`, verifies waiting appointment and reminder source facts, defers the request on both targets, verifies the appointment remains waiting with date/time preserved, checks modernized Calendar rendering, and deletes the temporary appointment and reminder.
- The six-hundred-first implementation slice adds appointment reminder retry readiness. The modernized appointment API now exposes a protected reminder-dispatch retry endpoint that appends deterministic retry audit events to `appointment_reminder_dispatch_audit`, and the Calendar detail panel exposes a `Retry reminder` action once dispatch history exists. The shared parity plan uses the existing `MOD-PAT-0191` reminder anchor, dispatches the local reminder, retries it from the Calendar workflow, verifies the retry metadata/history, and compares the modernized behavior with the legacy-derived reminder expectation while keeping real provider delivery integrations as future scope.
- The six-hundred-second implementation slice adds patient-facing appointment reminder delivery preferences. The modernized patient Portal profile form now shows SMS/email reminder permission state, lets signed-in patients submit SMS/email consent changes through the existing pending-review profile-change workflow, and staff profile-review acceptance persists those fields into chart contact data. The shared parity plan uses the existing `MOD-PAT-0004` portal account, creates a cleanup-backed due appointment, proves reminder derivation starts as `SMS + Email`, submits and accepts an SMS opt-out/email opt-in change, verifies contact consent persistence, proves the reminder channel changes to `Email`, checks modernized Portal and Calendar rendering, and restores the patient plus deletes the temporary appointment.
- The six-hundred-third implementation slice adds appointment reminder template administration readiness. The modernized appointment API now exposes a protected reminder-template catalog, accepts an optional compatible template id when dispatching a due reminder, rejects incompatible template selections before queue/audit persistence, and keeps no-body dispatch on the default template for prior slices. The Calendar detail controls load compatible templates for the selected reminder channel, default to the channel default, allow staff to select an alternate arrival-instructions template, and render selected-template dispatch evidence. The shared parity plan uses the existing `MOD-PAT-0191` due reminder anchor, verifies catalog contents, selected-template dispatch, invalid-template rejection, and Calendar selected-template rendering while keeping external reminder-provider integrations as future scope.
- The six-hundred-fourth implementation slice adds encounter SOAP note template readiness. The modernized encounter API now exposes a protected SOAP-note template catalog owned by the business tier, including stable follow-up, diabetes follow-up, acute respiratory, and preventive annual templates. The Encounters workspace can select a template, apply its Subjective/Objective/Assessment/Plan payload into the draft SOAP form, and save the note through the existing SOAP creation endpoint. The shared parity plan creates a cleanup-backed temporary encounter for `MOD-PAT-0002`, proves the selected diabetes template payload can be saved in the legacy SOAP form model, verifies the modernized catalog and browser apply/save behavior, and removes the temporary SOAP note and encounter.
- The six-hundred-fifth implementation slice adds patient merge-preview readiness. The modernized patient API exposes a protected non-destructive merge preview for duplicate patient charts, calculating target/source direction, match score and reasons, source/target/combined workflow counts, and explicit safeguards before any destructive merge path exists. The Patient/Client duplicate-detection panel can preview a candidate merge inline, and the shared parity plan creates a cleanup-backed duplicate source chart for `MOD-PAT-0010`, compares legacy normalized database evidence with the modernized API/UI preview, and deletes the temporary source chart.
