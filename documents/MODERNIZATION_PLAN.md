...
- This slice is read-only.
- Encounter create, update, and delete workflows are covered by Slice 12 for the focused encounter/vitals/SOAP lifecycle, and encounter search/detail/mutation access protection is covered by Slice 168. Broader encounter-adjacent workflows such as templates, amendment policy/history depth, billing expansion, authorization policy depth, and audit history remain deferred.
...
- The one-hundred-seventy-first modernized vertical slice implements billing protection readiness by requiring an active `X-OpenEMR-Session` session for `/api/billing/*`, gating the Fees workspace fee-sheet lookup, statement, batch, collections, claim, payment, and billing-line mutation controls until sign-in, replacing unauthenticated statement PDF/package links with authenticated downloads, adding Workbench billing protection plan actions, extending smoke coverage with unauthenticated billing `401` evidence, keeping older billing, claims, payments, statements, collections, and encounter fee-sheet parity suites authenticated, and comparing legacy fee-sheet protection with modernized no-session/active-session API/UI behavior. This protects the billing/revenue-cycle API surface; remaining procedure API protection, ASP.NET Core Identity, MFA, authorization policy depth, password lifecycle, and broader production session hardening remain future work.
