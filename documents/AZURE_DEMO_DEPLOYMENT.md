...
The same refresh action attempts a resource-group-scoped Azure Cost Management query for month-to-date demo cost, month-to-date average cost per day, today's posted cost, and the latest posted daily cost. Operators can run it from either **Refresh Azure status** or the Cost Tracking panel's **Refresh costs** button. Cost data is best-effort operational guidance only. Azure Cost Management data can lag behind live runtime usage and may require the signed-in account to have Cost Management read access for the selected resource group.
