  ...
  response.json({
    systems: [
      ...
      },
      {
        ...
        status: "Slice 52 account statement implemented",
...
app.get("/api/progress", async (_request, response) => {
  response.json({
    slices: [
      ...
      { id: "modernized-target", name: "Modernized OpenEMR target", status: "in-progress", detail: "Slice 52 adds statement-readiness details over seeded charges, payments, adjustments, aging, and ledger facts, API/Fees rendering, Workbench actions, smoke coverage, and side-by-side account statement parity evidence." }
    ]
  });
});
