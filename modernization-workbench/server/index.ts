  ...
  response.json({
    systems: [
      ...
      },
      {
        ...
        status: "Slice 123 encounter document replacement revision readiness implemented",
...
app.get("/api/progress", async (_request, response) => {
  ...
  response.json({
    slices: [
      ...
      { id: "test-management", name: "Parity test management", status: "verified", detail: "Named run plans are implemented through Slice 147 procedure order catalog lifecycle parity, with custom target/suite/plan/reset selection, side-by-side comparison artifact rendering, Slice 124 expandable comparison drill-ins, and Slice 125 safe links to run/comparison artifacts." },
      { id: "modernized-target", name: "Modernized OpenEMR target", status: "in-progress", detail: "Slice 147 proves procedure order catalog lifecycle readiness with temporary procedure_type/lab_order_catalog rows, create/update/deactivate/delete behavior, legacy and modernized rendering, cleanup, and matched side-by-side comparison evidence. The target now covers forty-one read-only slices plus one hundred two mutation-capable slices." }
    ],
