  ...
  response.json({
    systems: [
      ...
      },
      {
        ...
        status: "Slice 37 patient registration implemented",
...
app.get("/api/progress", async (_request, response) => {
  response.json({
    slices: [
      ...
      { id: "workflow-mutations", name: "Legacy workflow mutation suite", status: "verified", detail: "Implemented for demographics, patient registration, insurance coverage, scheduling, encounters with vitals/SOAP details, clinical lists, problem lists, medication lists, patient messages, text and binary patient documents, prescriptions, immunizations, billing, and lab procedure lifecycle coverage with pre/post database probes." },
      ...
      { id: "modernized-target", name: "Modernized OpenEMR target", status: "in-progress", detail: "Slice 37 adds patient registration create/render/delete behavior to the Patients API and React workspace, Workbench actions, smoke coverage, and side-by-side registration parity evidence." }
    ]
  });
});
