...
app.get("/api/progress", async (_request, response) => {
  ...
  response.json({
    slices: [
      ...
      { id: "test-management", name: "Parity test management", status: "verified", detail: "Named run plans are implemented through Slice 151 procedure order transmit parity, with custom target/suite/plan/reset selection, side-by-side comparison artifact rendering, Slice 124 expandable comparison drill-ins, and Slice 125 safe links to run/comparison artifacts." },
      { id: "modernized-target", name: "Modernized OpenEMR target", status: "in-progress", detail: "Slice 151 proves procedure order transmit readiness with temporary reportless lab orders, sent-awaiting-results queue membership after transmit marking, cleanup, and matched side-by-side comparison evidence. The target now covers forty-one read-only slices plus one hundred five mutation-capable slices." }
    ],
