...
app.get("/api/progress", async (_request, response) => {
  ...
  response.json({
    slices: [
      ...
      { id: "test-management", name: "Parity test management", status: "verified", detail: "Named run plans are implemented through Slice 148 procedure vendor compendium import parity, with custom target/suite/plan/reset selection, side-by-side comparison artifact rendering, Slice 124 expandable comparison drill-ins, and Slice 125 safe links to run/comparison artifacts." },
      { id: "modernized-target", name: "Modernized OpenEMR target", status: "in-progress", detail: "Slice 148 proves procedure vendor compendium import readiness with temporary catalog groups, PathGroup-style order/result CSV import, legacy-compatible deactivate/reactivate semantics, cleanup, and matched side-by-side comparison evidence. The target now covers forty-one read-only slices plus one hundred three mutation-capable slices." }
    ],
