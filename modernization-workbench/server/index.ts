...
app.get("/api/progress", async (_request, response) => {
  response.json({
    slices: [
      ...
      { id: "modernized-target", name: "Modernized OpenEMR target", status: "in-progress", detail: "Slice 46 adds fee-sheet modifier behavior to the React Fees workspace, Workbench actions, smoke coverage, and side-by-side billing modifier parity evidence." }
    ]
  });
});
