...
app.get("/api/progress", async (_request, response) => {
  response.json({
    slices: [
      ...
      { id: "modernized-target", name: "Modernized OpenEMR target", status: "in-progress", detail: "Slice 25 adds patient document metadata, React Documents visibility, Workbench actions, smoke coverage, and side-by-side document parity evidence." }
    ]
  });
});
