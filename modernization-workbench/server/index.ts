...
app.get("/api/progress", async (_request, response) => {
  response.json({
    slices: [
      ...
      { id: "workflow-mutations", name: "Legacy workflow mutation suite", status: "verified", detail: "Implemented for demographics, scheduling, encounters with vitals/SOAP details, clinical lists, patient messages, prescriptions, immunizations, billing, and lab procedure lifecycle coverage with pre/post database probes." },
      ...
      { id: "modernized-target", name: "Modernized OpenEMR target", status: "in-progress", detail: "Slice 30 adds immunization create, entered-in-error, and delete lifecycle behavior to the clinical Lists API and React UI, Workbench actions, and side-by-side immunization mutation parity evidence." }
    ]
  });
});
